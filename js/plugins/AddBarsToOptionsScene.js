...
//=============================================================================
/*:
 * @plugindesc (v1.10)[v1.3] 控件 - 选项界面的可拉动滑竿
 * @author ツミオ （Drill_up翻译+优化）
 * 
 * 
 * @param 资源-滑竿
 * @desc 滑竿的图片资源。不设置则使用默认绘制滑竿。
 * @default (需配置)选项界面滑竿-滑竿
 * @require 1
 * @dir img/Menu__ui/
 * @type file
 * 
...
(function () {
    ...
    var $bar_position_ABOS_Y = Number(param['平移-滑竿 Y'] || 40);
...
Window_Options.prototype.addBar = function(index){
    ...
    if (this.isVolumeSymbol(symbol)) {
        ...
        //タッチされたかどうか判定
        var _on_touch_button = this.isTouchButton(rect.x + this.button_position[index],rect.y + $bar_position_ABOS_Y-3 );

        if(_on_touch_button)
        {
            this.changeButtonPosition(rect.x + this.button_position[index],rect.y + $bar_position_ABOS_Y-3,index);
        }

        //this.drawText('あははははうんこだー！', rect.x, rect.y, 50 , 'left');
        this.drawBar(rect.x + $bar_position_ABOS, rect.y + $bar_position_ABOS_Y, rect.x + $bar_position_ABOS + $bar_length_ABOS,rect.y + $bar_position_ABOS_Y-3, $bar_color_ABOS, 6,index);
        this.drawButton(rect.x + this.button_position[index], rect.y + $bar_position_ABOS_Y-3, 10, $button_color_ABOS,index);
    }
};
