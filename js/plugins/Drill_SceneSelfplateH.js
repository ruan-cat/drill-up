...
　　var DrillUp = DrillUp || {}; 
    ...
    DrillUp.g_SSpH_menu_name = String(DrillUp.parameters['主菜单显示名'] || "");
	...
	DrillUp.g_SSpH_title_data_global = String(DrillUp.parameters['数据是否全局存储'] || "false") === "true";	
    DrillUp.g_SSpH_title_data_fileId = Number(DrillUp.parameters['全局存储的文件路径'] || 1);	
	...
	DrillUp.g_SSpH_locked_name = DrillUp.g_SSpH_locked_name.replace(/\\\\/g,"\\");
	DrillUp.g_SSpH_locked_context = String(DrillUp.parameters['用语-锁定的选项内容'] || "");
	DrillUp.g_SSpH_locked_context = DrillUp.g_SSpH_locked_context.substring(1,DrillUp.g_SSpH_locked_context.length-1);
	DrillUp.g_SSpH_locked_context = DrillUp.g_SSpH_locked_context.replace(/\\\\/g,"\\");
...
// * 全局 - 读取
//==============================
	var global_fileId = DrillUp.g_SSpH_title_data_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SSpH" );
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">信息面板H" ){
		...
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				DrillUp.global_SSpH_enableTank[ Number(temp1) ] = true;			//全局存储
				$gameSystem._drill_SSpH_enableTank[ Number(temp1) ] = true;		//正常存储
				StorageManager.drill_SSpH_saveData();
			}
			if( type == "隐藏选项" ){
				DrillUp.global_SSpH_enableTank[ Number(temp1) ] = false;		//全局存储
				$gameSystem._drill_SSpH_enableTank[ Number(temp1) ] = false;	//正常存储
				StorageManager.drill_SSpH_saveData();
			}
			if( type == "锁定选项" ){
				DrillUp.global_SSpH_lockTank[ Number(temp1) ] = true;			//全局存储
				$gameSystem._drill_SSpH_lockTank[ Number(temp1) ] = true;		//正常存储
				StorageManager.drill_SSpH_saveData();
			}
			if( type == "解锁选项" ){
				DrillUp.global_SSpH_lockTank[ Number(temp1) ] = false;			//全局存储
				$gameSystem._drill_SSpH_lockTank[ Number(temp1) ] = false;		//正常存储
				StorageManager.drill_SSpH_saveData();
			}
			...
			if( type == "设置末尾选项" || type == "末尾页" ){
				if( temp1 == "最后" || temp1 == "最后一页" ){					//只正常存储
					...
					$gameSystem._drill_SSpH_end_page = Number(temp1);
				}
			}
		}
	}
...
Scene_Drill_SSpH.prototype.drill_updateIndex = function() {
	if( $gameSystem._drill_SSpH_context_index != undefined ){
		var temp = $gameSystem._drill_SSpH_context_index;
	...
	if( this._window_select._index == null || 
		this._window_select._index > $gameTemp._drill_SSpH_visibleList.length -1 ||
		this._window_select._index < 0){ this._window_select.select(0);}
	if( $gameTemp._drill_SSpH_visibleList.length == 0 ){ return };	//如果选项全部为空，强制选择第一个
...
Scene_Drill_SSpH.prototype.drill_refreshArrow = function( index ){
	...
	if( index % DrillUp.g_SSpH_selWin_col == DrillUp.g_SSpH_selWin_col-1 ||
		index == $gameTemp._drill_SSpH_visibleList.length -1 ){
		r_visible = false;
	}
	if( index <= DrillUp.g_SSpH_selWin_col-1 ){
		u_visible = false;
	}
	if( index > $gameTemp._drill_SSpH_visibleList.length -1 - DrillUp.g_SSpH_selWin_col ){
		d_visible = false;
	}
...
Drill_SSpH_SelectWindow.lastTopRow = 0;
Drill_SSpH_SelectWindow.lastIndex  = 0;
//==============================
// * 选项窗口 - 初始化
//==============================
...
// * 选项窗口 - 设置选项
//==============================
Drill_SSpH_SelectWindow.prototype.drill_initSelect = function() {
	if( Drill_SSpH_SelectWindow.lastIndex >= this._list.length ){
		Drill_SSpH_SelectWindow.lastIndex = this._list.length-1;
	}
	this.setTopRow(Drill_SSpH_SelectWindow.lastTopRow);
	this.select(Drill_SSpH_SelectWindow.lastIndex);
}
//==============================
// * 选项窗口 - 绘制选项
//==============================
Drill_SSpH_SelectWindow.prototype.drawItem = function(index) {
	...
	// > 绘制内容
	var op = {
		"align":DrillUp.g_SSpH_selWin_align,
		"autoLineheight":true,
		"x": rect.x,
		"y": rect.y,
		"width": rect.width,
...
Drill_SSpH_SelectWindow.prototype.processCancel = function() {
	...
	Drill_SSpH_SelectWindow.lastTopRow = this.topRow();
	Drill_SSpH_SelectWindow.lastIndex = this.index();
};
//==============================
// * 选项窗口 - 兼容 - mog菜单指针插件
//==============================
...
// * 流程锁定 - 退出键
//==============================
var _drill_SSpH_processCancel = Drill_SSpH_SelectWindow.prototype.processCancel;
Drill_SSpH_SelectWindow.prototype.processCancel = function() {
