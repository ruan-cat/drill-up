...
//=============================================================================
	...
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
	DrillUp.drill_GFTP_getPluginTip_LowVersion = function(){
...
// * 预加载 - 初始化
//==============================
var _drill_GFTP_preload_initialize = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
    _drill_GFTP_preload_initialize.call(this);
	this.drill_GFTP_preloadInit();
}
//==============================
// * 预加载 - 版本校验
//==============================
if( Utils.generateRuntimeId == undefined ){
	alert( DrillUp.drill_GFTP_getPluginTip_LowVersion() );
}
//==============================
// * 预加载 - 执行资源预加载
//
...
//==============================
Game_Temp.prototype.drill_GFTP_preloadInit = function() {
	this._drill_GFTP_cacheId = Utils.generateRuntimeId();	//资源缓存id
    this._drill_GFTP_preloadTank = [];						//bitmap容器
	for( var i = 0; i < DrillUp.g_GFTP_style.length; i++ ){
		var temp_data = DrillUp.g_GFTP_style[i];
		if( temp_data == undefined ){ continue; }
		
		// > 『窗口皮肤的预加载』
		if( temp_data['window_type'] == "自定义窗口皮肤" ){
			this._drill_GFTP_preloadTank.push( 
				ImageManager.reserveBitmap( "img/system/", temp_data['window_sys_src'], 0, true, this._drill_GFTP_cacheId ) 
			);
		}
		if( temp_data['window_type'] == "自定义背景图片" ){
			this._drill_GFTP_preloadTank.push( 
				ImageManager.reserveBitmap( "img/Map__ui/", temp_data['window_pic_src'], 0, true, this._drill_GFTP_cacheId ) 
			);
		}
	}
}
