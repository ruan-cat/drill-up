 ...
 * @Drill_LE_var "DrillUp.g_SGaC_context_list_length"
 * 
 *
 * @help
 * =============================================================================
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
//		插件简称		SGaC（Scene_Gallery_A）
//		临时全局变量	DrillUp.g_SGaC_xxx
//		临时局部变量	this._drill_SGaC_xxx
//		存储数据变量	$gameSystem._drill_SGaC_context_list
//		全局存储变量	DrillUp.global_SGaC_enableTank
//		覆盖重写方法	无
//
//<<<<<<<<性能记录<<<<<<<<
//
...
//		★时间复杂度		o(n^3)*o(场景元素) 每帧
//		★性能测试因素	直接进入画廊进行测试。
//		★性能测试消耗	22.23ms（Scene_Drill_SGaC.prototype.update函数）
//						18.17ms（Scene_Drill_SGaC.prototype.drill_updateThumbnail函数）
//						16.96ms（Scene_Drill_SGaC.prototype.drill_updateArrow函数）
//		★最坏情况		无
//		★备注			无
//		
//		★优化记录		暂无
...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_SGaC_PluginTip_curName = "Drill_SceneGalleryC.js 面板-全自定义画廊C";
	...
	DrillUp.drill_SGaC_getPluginTip_NoBasePlugin = function(){
		...
		var message = "【" + DrillUp.g_SGaC_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_SGaC_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_SGaC_PluginTip_baseList[i];
		}
		return message;
	};
...
　　var DrillUp = DrillUp || {}; 
	...
	//				（~struct~DrillCursor）
	//==============================
	DrillUp.drill_SGaC_initMenuCursor = function( dataFrom ) {
		var data = {};
	...
	//				（~struct~DrillWindowMoving）
	//==============================
	DrillUp.drill_SGaC_initWindowMoving = function( dataFrom ) {
		var data = {};
	...
	//				（~struct~DrillWindowLayout）
	//==============================
	DrillUp.drill_SGaC_initWindowLayout = function( dataFrom ) {
		var data = {};
	...
	//				（~struct~DrillSelectWindow）
	//==============================
	DrillUp.drill_SGaC_initCommandWindow = function( dataFrom ) {
		var data = {};
		...
		if( dataFrom["选项窗口移动动画"] != "" &&
			...
			var slideAnim_data = DrillUp.drill_SGaC_initWindowMoving( slideAnim );
		...
		if( dataFrom["选项窗口布局"] != "" &&
			...
			var layout_data = DrillUp.drill_SGaC_initWindowLayout( layout );
		...
		if( dataFrom["选项窗口指针与边框"] != "" &&
			...
			data['cursor'] = DrillUp.drill_SGaC_initMenuCursor( cursor );
		}else{
			data['cursor'] = DrillUp.drill_SGaC_initMenuCursor( {} );
		}
	...
	//				（~struct~DrillSGaC）
	//==============================
	DrillUp.drill_SGaC_initContext = function( dataFrom ) {
		var data = {};
	...
	//				（~struct~SGaCSound）
	//==============================
	DrillUp.drill_SGaC_initSound = function( dataFrom ){
		var data = {};
	...
	/*-----------------杂项------------------*/
    DrillUp.g_SGaC_layout = String(DrillUp.parameters["资源-整体布局"] || "");
	DrillUp.g_SGaC_add_to_menu = String(DrillUp.parameters["是否添加到主菜单"] || "true") === "true";	
    DrillUp.g_SGaC_menu_name = String(DrillUp.parameters["主菜单显示名"] || "");
	DrillUp.g_SGaC_add_to_title = String(DrillUp.parameters["是否在标题窗口中显示"] || "false") === "true";	
    DrillUp.g_SGaC_title_name = String(DrillUp.parameters["标题窗口显示名"] || "");
	DrillUp.g_SGaC_title_data_global = String(DrillUp.parameters["数据是否全局存储"] || "false") === "true";	
    DrillUp.g_SGaC_title_data_fileId = Number(DrillUp.parameters["全局存储的文件路径"] || 1);	
	
	
	/*-----------------原图查看器------------------*/
	DrillUp.g_SGaC_imgExpandPic_slideTime = Number(DrillUp.parameters["原图显现时长"] || 20);
	DrillUp.g_SGaC_imgExpandHelp_src = String(DrillUp.parameters["资源-帮助图"] || "");
	DrillUp.g_SGaC_imgExpandHelp_x = Number(DrillUp.parameters["偏移-帮助图 X"] || 0);
	DrillUp.g_SGaC_imgExpandHelp_y = Number(DrillUp.parameters["偏移-帮助图 Y"] || 0);
	DrillUp.g_SGaC_imgExpandHelp_slideTime = Number(DrillUp.parameters["帮助图移动时长"] || 40);
	DrillUp.g_SGaC_imgExpandHelp_sustain = Number(DrillUp.parameters["帮助图持续时间"] || 180);
	DrillUp.g_SGaC_singlePicture_hide = String(DrillUp.parameters["单图模式时是否隐藏整体布局"] || "true") === "true";	
	...
	if( DrillUp.parameters["选项窗口"] != "" &&
		...
		DrillUp.g_SGaC_command_window = DrillUp.drill_SGaC_initCommandWindow( data );
	}else{
		DrillUp.g_SGaC_command_window = DrillUp.drill_SGaC_initCommandWindow( {} );
	}

	/*-----------------描述窗口------------------*/
	DrillUp.g_SGaC_descWin_x = Number(DrillUp.parameters["描述窗口 X"] || 285);
	DrillUp.g_SGaC_descWin_y = Number(DrillUp.parameters["描述窗口 Y"] || 420);
	DrillUp.g_SGaC_descWin_width = Number(DrillUp.parameters["描述窗口宽度"] || 450);
	DrillUp.g_SGaC_descWin_height = Number(DrillUp.parameters["描述窗口高度"] || 180);
	DrillUp.g_SGaC_descWin_fontsize = Number(DrillUp.parameters["描述窗口字体大小"] || 22);
	DrillUp.g_SGaC_descWin_replay = String(DrillUp.parameters["是否重播描述窗口移动动画"] || "true") === "true";	
	if( DrillUp.parameters["描述窗口移动动画"] != "" &&
		DrillUp.parameters["描述窗口移动动画"] != undefined ){
		var data = JSON.parse( DrillUp.parameters["描述窗口移动动画"] );
		DrillUp.g_SGaC_descWin_slideAnim = DrillUp.drill_SGaC_initWindowMoving( data );
	}else{
		DrillUp.g_SGaC_descWin_slideAnim = DrillUp.drill_SGaC_initWindowMoving( {} );
	}
	if( DrillUp.parameters["描述窗口布局"] != "" &&
		DrillUp.parameters["描述窗口布局"] != undefined ){
		var data = JSON.parse( DrillUp.parameters["描述窗口布局"] );
		DrillUp.g_SGaC_descWin_layout = DrillUp.drill_SGaC_initWindowLayout( data );
	}else{
		DrillUp.g_SGaC_descWin_layout = DrillUp.drill_SGaC_initWindowLayout( {} );
	}

	/*-----------------缩略图------------------*/
	DrillUp.g_SGaC_Thumbnail_x = Number(DrillUp.parameters["缩略图 X"] || 285);
	DrillUp.g_SGaC_Thumbnail_y = Number(DrillUp.parameters["缩略图 Y"] || 60);
	DrillUp.g_SGaC_Thumbnail_width = Number(DrillUp.parameters["缩略图宽度"] || 450);
	DrillUp.g_SGaC_Thumbnail_height = Number(DrillUp.parameters["缩略图高度"] || 340);
	DrillUp.g_SGaC_Thumbnail_mode = String(DrillUp.parameters["缩略图缩放模式"] || "等比缩放");
	DrillUp.g_SGaC_Thumbnail_replay = String(DrillUp.parameters["是否重播缩略图移动动画"] || "true") === "true";	
	DrillUp.g_SGaC_Thumbnail_showInstant = String(DrillUp.parameters["是否瞬间显示缩略图"] || "false") === "true";	
	if( DrillUp.parameters["缩略图移动动画"] != "" &&
		DrillUp.parameters["缩略图移动动画"] != undefined ){
		var data = JSON.parse( DrillUp.parameters["缩略图移动动画"] );
		DrillUp.g_SGaC_Thumbnail_slideAnim = DrillUp.drill_SGaC_initWindowMoving( data );
	}else{
		DrillUp.g_SGaC_Thumbnail_slideAnim = DrillUp.drill_SGaC_initWindowMoving( {} );
	}
	
	/*-----------------完成度窗口------------------*/
	DrillUp.g_SGaC_compWin_x = Number(DrillUp.parameters["完成度窗口 X"] || 50);
	DrillUp.g_SGaC_compWin_y = Number(DrillUp.parameters["完成度窗口 Y"] || 530);
	DrillUp.g_SGaC_compWin_width = Number(DrillUp.parameters["完成度窗口宽度"] || 210);
	DrillUp.g_SGaC_compWin_height = Number(DrillUp.parameters["完成度窗口高度"] || 70);
	DrillUp.g_SGaC_compWin_fontsize = Number(DrillUp.parameters["完成度窗口字体大小"] || 22);
	DrillUp.g_SGaC_compWin_word = String(DrillUp.parameters["完成度用语"] || "完成度：");
	if( DrillUp.parameters["完成度窗口移动动画"] != "" &&
		DrillUp.parameters["完成度窗口移动动画"] != undefined ){
		var data = JSON.parse( DrillUp.parameters["完成度窗口移动动画"] );
		DrillUp.g_SGaC_compWin_slideAnim = DrillUp.drill_SGaC_initWindowMoving( data );
	}else{
		DrillUp.g_SGaC_compWin_slideAnim = DrillUp.drill_SGaC_initWindowMoving( {} );
	}
	if( DrillUp.parameters["完成度窗口布局"] != "" &&
		DrillUp.parameters["完成度窗口布局"] != undefined ){
		var data = JSON.parse( DrillUp.parameters["完成度窗口布局"] );
		DrillUp.g_SGaC_compWin_layout = DrillUp.drill_SGaC_initWindowLayout( data );
	}else{
		DrillUp.g_SGaC_compWin_layout = DrillUp.drill_SGaC_initWindowLayout( {} );
	}
	
	/*-----------------箭头------------------*/
	DrillUp.g_SGaC_arrowLeft = String(DrillUp.parameters['资源-左箭头'] || "");
	DrillUp.g_SGaC_arrowRight = String(DrillUp.parameters['资源-右箭头'] || "");
	DrillUp.g_SGaC_arrowUp = String(DrillUp.parameters['资源-上箭头'] || "");
	DrillUp.g_SGaC_arrowDown = String(DrillUp.parameters['资源-下箭头'] || "");
	DrillUp.g_SGaC_arrowLeft_X = Number(DrillUp.parameters['平移-左箭头 X'] || 60);
	DrillUp.g_SGaC_arrowLeft_Y = Number(DrillUp.parameters['平移-左箭头 Y'] || 300);
	DrillUp.g_SGaC_arrowRight_X = Number(DrillUp.parameters['平移-右箭头 X'] || 750);
	DrillUp.g_SGaC_arrowRight_Y = Number(DrillUp.parameters['平移-右箭头 Y'] || 300);
	DrillUp.g_SGaC_arrowUp_X = Number(DrillUp.parameters['平移-上箭头 X'] || 408);
	DrillUp.g_SGaC_arrowUp_Y = Number(DrillUp.parameters['平移-上箭头 Y'] || 35);
	DrillUp.g_SGaC_arrowDown_X = Number(DrillUp.parameters['平移-下箭头 X'] || 408);
	DrillUp.g_SGaC_arrowDown_Y = Number(DrillUp.parameters['平移-下箭头 Y'] || 560);
	DrillUp.g_SGaC_arrow_zoom = String(DrillUp.parameters['是否使用缩放效果'] || "false") === "true";	
	DrillUp.g_SGaC_arrow_flicker = String(DrillUp.parameters['是否使用闪烁效果'] || "false") === "true";	
	DrillUp.g_SGaC_arrow_float_val = Number(DrillUp.parameters['浮动偏移量'] || 10);
	DrillUp.g_SGaC_arrow_float_lr = String(DrillUp.parameters['是否使用左右浮动'] || "true") === "true";
	DrillUp.g_SGaC_arrow_float_ud = String(DrillUp.parameters['是否使用上下浮动'] || "true") === "true";
	DrillUp.g_SGaC_arrowClickDefaultEnabled = String(DrillUp.parameters['箭头是否使用默认音效'] || "true") === "true";
	if( DrillUp.parameters['箭头悬停音效'] != "" && 
		DrillUp.parameters['箭头悬停音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头悬停音效']);
		DrillUp.g_SGaC_arrowHoverSound = DrillUp.drill_SGaC_initSound( temp );
	}else{
		DrillUp.g_SGaC_arrowHoverSound = null;
	}
	if( DrillUp.parameters['箭头点击音效'] != "" && 
		DrillUp.parameters['箭头点击音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头点击音效']);
		DrillUp.g_SGaC_arrowClickSound = DrillUp.drill_SGaC_initSound( temp );
	}else{
...
// * 全局 - 读取
//==============================
	var global_fileId = DrillUp.g_SGaC_title_data_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SGaC" );
	
	// > 显示情况
	if( DrillUp.global_SGaC_enableTank == null ){			//（游戏没关时，不会为null)
		var data = global_data["global_enableTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SGaC_enableTank = data;
		DrillUp.drill_SGaC_gCheckData_enable();				//（检查时自动赋新值）
	}
	// > 锁定情况
	if( DrillUp.global_SGaC_lockTank == null ){	
...
// * 全局 - 存储
//==============================
StorageManager.drill_SGaC_saveData = function(){
	var file_id = DrillUp.g_SGaC_title_data_fileId;
	var data = {};
...
//##############################
DrillUp.g_SGaC_saveEnabled = true;
//##############################
// * 存储数据 - 初始化
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SGaC_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_SGaC_sys_initialize.call(this);
	this.drill_SGaC_initSysData();
};
//##############################
// * 存储数据 - 载入存档
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SGaC_sys_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function( contents ){
	_drill_SGaC_sys_extractSaveContents.call( this, contents );
	
	// > 参数存储 启用时（检查数据）
	if( DrillUp.g_SGaC_saveEnabled == true ){	
		$gameSystem.drill_SGaC_checkSysData();
		
	// > 参数存储 关闭时（直接覆盖）
	}else{
		$gameSystem.drill_SGaC_initSysData();
	}
};
//##############################
// * 存储数据 - 初始化数据【标准函数】
...
//##############################
Game_System.prototype.drill_SGaC_initSysData = function() {
	this.drill_SGaC_initSysData_Private();
};
//##############################
// * 存储数据 - 载入存档时检查数据【标准函数】
//			
...
//##############################
Game_System.prototype.drill_SGaC_checkSysData = function() {
	this.drill_SGaC_checkSysData_Private();
};
//=============================================================================
// ** 存储数据（接口实现）
//=============================================================================
...
// * 存储数据 - 初始化数据（私有）
//==============================
Game_System.prototype.drill_SGaC_initSysData_Private = function() {
...
// * 存储数据 - 载入存档时检查数据（私有）
//==============================
Game_System.prototype.drill_SGaC_checkSysData_Private = function() {
	
	// > 旧存档数据自动补充
	if( this._drill_SGaC_enableTank == undefined ){
		this.drill_SGaC_initSysData();
	}
	...
	for( var i = 0; i < DrillUp.g_SGaC_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SGaC_enableTank[i] == undefined ){
				this._drill_SGaC_enableTank[i] = temp_data['enabled'];
			...
			// > 未存储的，重新初始化
			if( this._drill_SGaC_lockTank[i] == undefined ){
				this._drill_SGaC_lockTank[i] = temp_data['locked'];
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if( command === ">画廊C" ){
		...
		if(args.length == 4){
			...
			if( type == "选中页" ){
				$gameSystem._drill_SGaC_context_index = Number(temp1) -1;
			}
			if( type == "打开面板(单图查看模式)" ){
...
// ** ☆主菜单选项
//
//			说明：	> 此模块专门关联主菜单选项，选项进入后跳转到 画廊C 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================
var _drill_SGaC_createCommandWindow = Scene_Menu.prototype.createCommandWindow;
Scene_Menu.prototype.createCommandWindow = function() {
	_drill_SGaC_createCommandWindow.call(this);
    this._commandWindow.setHandler('Drill_SGaC',   this.drill_SGaC_menuCommand.bind(this));
};
Scene_Menu.prototype.drill_SGaC_menuCommand = function() {
    SceneManager.push(Scene_Drill_SGaC);
};
var _drill_SGaC_addOriginalCommands = Window_MenuCommand.prototype.addOriginalCommands;
Window_MenuCommand.prototype.addOriginalCommands = function() {
	_drill_SGaC_addOriginalCommands.call(this);
	if( DrillUp.g_SGaC_add_to_menu ){
		this.addCommand(DrillUp.g_SGaC_menu_name, 'Drill_SGaC', this.areMainCommandsEnabled());
	}
};
...
// ** ☆标题选项
//
//			说明：	> 此模块专门关联标题选项，选项进入后跳转到 画廊C 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================	
var _drill_SGaC_title_createCommandWindow = Scene_Title.prototype.createCommandWindow;
Scene_Title.prototype.createCommandWindow = function() {
    _drill_SGaC_title_createCommandWindow.call(this);
	this._commandWindow.setHandler('Drill_SGaC',  this.drill_SGaC_titleCommand.bind(this));
};
Scene_Title.prototype.drill_SGaC_titleCommand = function() {
    this._commandWindow.close();
    SceneManager.push(Scene_Drill_SGaC);
};
var _drill_SGaC_title_makeCommandList = Window_TitleCommand.prototype.makeCommandList;
Window_TitleCommand.prototype.makeCommandList = function() {
    _drill_SGaC_title_makeCommandList.call(this);
	if( DrillUp.g_SGaC_add_to_title ){
		this.addCommand( DrillUp.g_SGaC_title_name ,'Drill_SGaC');
	}
};	
...
// * 面板控制 - 初始化
//==============================
var _drill_SGaC_temp_initialize = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {	
	_drill_SGaC_temp_initialize.call(this);
...
// * 面板控制 - 判断 锁定情况
//==============================
Game_Temp.prototype.drill_SGaC_isLocked = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SGaC_title_data_global == true ){
		if( DrillUp.global_SGaC_lockTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SGaC_lockTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * 面板控制 - 判断 显示情况
//==============================
Game_Temp.prototype.drill_SGaC_isEnabled = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SGaC_title_data_global == true ){
		if( DrillUp.global_SGaC_enableTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SGaC_enableTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
Scene_Drill_SGaC.prototype.terminate = function() { 
	...
	$gameTemp._drill_SGaC_isSinglePictureMode = false;	//（关闭单图模式）
}
...
Scene_Drill_SGaC.prototype.drill_isAllBitmapLoaded = function() {
	for(var i=0; i < this._drill_SGaC_bitmaps.length; i++){
		if( this._drill_SGaC_bitmaps[i].isReady() != true ){
			return false;
		}
	}
	return true;
...
Scene_Drill_SGaC.prototype.drill_createDesc = function() {
	var data = {
		"x": DrillUp.g_SGaC_descWin_x,
		"y": DrillUp.g_SGaC_descWin_y,
		"width": DrillUp.g_SGaC_descWin_width,
		"height": DrillUp.g_SGaC_descWin_height,
		"fontsize": DrillUp.g_SGaC_descWin_fontsize,
		
		"slideMoveType": DrillUp.g_SGaC_descWin_slideAnim['slideMoveType'],
		"slideTime": DrillUp.g_SGaC_descWin_slideAnim['slideTime'],
		"slideDelay": DrillUp.g_SGaC_descWin_slideAnim['slideDelay'],
		"slidePosType": DrillUp.g_SGaC_descWin_slideAnim['slidePosType'],
		"slideX": DrillUp.g_SGaC_descWin_slideAnim['slideX'],
		"slideY": DrillUp.g_SGaC_descWin_slideAnim['slideY'],
		"slideAbsoluteX": DrillUp.g_SGaC_descWin_slideAnim['slideAbsoluteX'],
		"slideAbsoluteY": DrillUp.g_SGaC_descWin_slideAnim['slideAbsoluteY'],
		
		"layoutType": DrillUp.g_SGaC_descWin_layout['layoutType'],
		"layoutX": DrillUp.g_SGaC_descWin_layout['layoutX'],
		"layoutY": DrillUp.g_SGaC_descWin_layout['layoutY'],
		"layoutSrc": DrillUp.g_SGaC_descWin_layout['layoutSrc'],
...
Scene_Drill_SGaC.prototype.drill_createThumbnail = function() {
	var data = {
		"x": DrillUp.g_SGaC_Thumbnail_x,
		"y": DrillUp.g_SGaC_Thumbnail_y,
		
		"slideMoveType": DrillUp.g_SGaC_Thumbnail_slideAnim['slideMoveType'],
		"slideTime": DrillUp.g_SGaC_Thumbnail_slideAnim['slideTime'],
		"slideDelay": DrillUp.g_SGaC_Thumbnail_slideAnim['slideDelay'],
		"slidePosType": DrillUp.g_SGaC_Thumbnail_slideAnim['slidePosType'],
		"slideX": DrillUp.g_SGaC_Thumbnail_slideAnim['slideX'],
		"slideY": DrillUp.g_SGaC_Thumbnail_slideAnim['slideY'],
		"slideAbsoluteX": DrillUp.g_SGaC_Thumbnail_slideAnim['slideAbsoluteX'],
...
Scene_Drill_SGaC.prototype.drill_updateThumbnail = function() {
	...
	var rxx = 0;
	var ryy = 0;
	if( DrillUp.g_SGaC_Thumbnail_mode == "等比缩放" ){
		var pw = DrillUp.g_SGaC_Thumbnail_width / ww;
		var ph = DrillUp.g_SGaC_Thumbnail_height / hh;
		if( pw > ph ){ pw = ph; }			
		this._sprite_thumbnail.scale.x = pw;
		this._sprite_thumbnail.scale.y = pw;
		rxx = (DrillUp.g_SGaC_Thumbnail_width - ww * pw)*0.5;
		ryy = (DrillUp.g_SGaC_Thumbnail_height - hh * pw)*0.5;
	}else{	//（拉伸缩放）
		var pw = DrillUp.g_SGaC_Thumbnail_width / ww;
		var ph = DrillUp.g_SGaC_Thumbnail_height / hh;
		this._sprite_thumbnail.scale.x = pw;
	...
	// > 位置控制
	this._sprite_thumbnail.x = DrillUp.g_SGaC_Thumbnail_x + rxx;
	this._sprite_thumbnail.y = DrillUp.g_SGaC_Thumbnail_y + ryy;
...
Scene_Drill_SGaC.prototype.drill_updateImgExpandSprite = function() {
	...
	this._sprite_imgExpand_pic['cur_time'] = this._sprite_imgExpand_pic['cur_time'].clamp( 0, DrillUp.g_SGaC_imgExpandHelp_slideTime );
	
	// > 透明度控制
	this._sprite_imgExpand_pic.opacity = 255 * this._sprite_imgExpand_pic['cur_time'] / DrillUp.g_SGaC_imgExpandPic_slideTime;
...
Scene_Drill_SGaC.prototype.drill_updateImgExpandHelp = function() {
	...
	// > 移动时间
	if( this._window_imgExpand.active == true &&
		this._sprite_imgExpand_help['sustain_time'] < DrillUp.g_SGaC_imgExpandHelp_sustain ){
		this._sprite_imgExpand_help['cur_time'] += 1;
	}else{
		this._sprite_imgExpand_help['cur_time'] -= 1;
	}
	this._sprite_imgExpand_help['cur_time'] = this._sprite_imgExpand_help['cur_time'].clamp( 0, DrillUp.g_SGaC_imgExpandHelp_slideTime );
	
	// > 持续时间
	if( this._sprite_imgExpand_help['cur_time'] == DrillUp.g_SGaC_imgExpandHelp_slideTime ){
	...
	// > 透明度控制
	this._sprite_imgExpand_help.opacity = 255 * this._sprite_imgExpand_help['cur_time'] / DrillUp.g_SGaC_imgExpandHelp_slideTime;
	
	// > 位置控制
	var xx = Graphics.boxWidth * 0.5;
	var yy = 0;
	var hh = this._sprite_imgExpand_help.bitmap.height;
	xx += DrillUp.g_SGaC_imgExpandHelp_x;
	yy += DrillUp.g_SGaC_imgExpandHelp_y;
	yy -= hh;		//（从上方下滑动画）
	yy += hh * this._sprite_imgExpand_help['cur_time'] / DrillUp.g_SGaC_imgExpandHelp_slideTime;
...
// * 画廊C（场景基类） - 开始运行
//==============================
Scene_Drill_SGaC.prototype.start = function() {
    Scene_MenuBase.prototype.start.call(this);
};
//==============================
// * 画廊C（场景基类） - 结束运行
//==============================
Scene_Drill_SGaC.prototype.stop = function() {
...
// * 画廊C（场景基类） - 判断加载完成
//==============================
Scene_Drill_SGaC.prototype.isReady = function() {
...
// * 画廊C（菜单界面基类） - 创建 - 菜单背景
//==============================
Scene_Drill_SGaC.prototype.createBackground = function() {
	Scene_MenuBase.prototype.createBackground.call(this);
};
//==============================
// * 画廊C（菜单界面基类） - 创建 - 帮助窗口
//==============================
Scene_Drill_SGaC.prototype.createHelpWindow = function() {
...
Scene_Drill_SGaC.prototype.drill_createArrow = function() {
	...
	this._arrow_left = new Sprite(ImageManager.load_MenuGallery(DrillUp.g_SGaC_arrowLeft));
	this._arrow_right = new Sprite(ImageManager.load_MenuGallery(DrillUp.g_SGaC_arrowRight));
	this._arrow_up = new Sprite(ImageManager.load_MenuGallery(DrillUp.g_SGaC_arrowUp));
	this._arrow_down = new Sprite(ImageManager.load_MenuGallery(DrillUp.g_SGaC_arrowDown));
	this._arrow_left._org_x = DrillUp.g_SGaC_arrowLeft_X ; 
	this._arrow_left._org_y = DrillUp.g_SGaC_arrowLeft_Y ;
	this._arrow_right._org_x = DrillUp.g_SGaC_arrowRight_X ;
	this._arrow_right._org_y = DrillUp.g_SGaC_arrowRight_Y ;
	this._arrow_up._org_x = DrillUp.g_SGaC_arrowUp_X ; 
	this._arrow_up._org_y = DrillUp.g_SGaC_arrowUp_Y ; 
	this._arrow_down._org_x = DrillUp.g_SGaC_arrowDown_X ;
	this._arrow_down._org_y = DrillUp.g_SGaC_arrowDown_Y ;
...
Scene_Drill_SGaC.prototype.drill_updateArrowSprite = function() {
	...
	// > 左右浮动
	if( DrillUp.g_SGaC_arrow_float_lr ){
		this._arrow_left.x = this._arrow_left._org_x + this._arrow_changeValue* DrillUp.g_SGaC_arrow_float_val ; 	
		this._arrow_left.y = this._arrow_left._org_y ; 
		this._arrow_right.x = this._arrow_right._org_x - this._arrow_changeValue* DrillUp.g_SGaC_arrow_float_val ; 
		this._arrow_right.y = this._arrow_right._org_y ;
	}
	// > 上下浮动
	if( DrillUp.g_SGaC_arrow_float_ud ){
		this._arrow_up.x = this._arrow_up._org_x;
		this._arrow_up.y = this._arrow_up._org_y + this._arrow_changeValue* DrillUp.g_SGaC_arrow_float_val ; 	
		this._arrow_down.x = this._arrow_down._org_x;
		this._arrow_down.y = this._arrow_down._org_y - this._arrow_changeValue* DrillUp.g_SGaC_arrow_float_val ; 	
	}
	// > 缩放
	if( DrillUp.g_SGaC_arrow_zoom ){
	...
	// > 闪烁
	if( DrillUp.g_SGaC_arrow_flicker ){
...
Scene_Drill_SGaC.prototype.drill_updateArrowTouch = function() {
    if( TouchInput.isTriggered() ){
		...
		if( this._arrow_hoverType == 8 ){ this._window_select.cursorUp();    SoundManager.drill_SGaC_playClick_Arrow(); }
		if( this._arrow_hoverType == 2 ){ this._window_select.cursorDown();  SoundManager.drill_SGaC_playClick_Arrow(); }
	}
}
//==============================
// * 箭头 - 帧刷新 高亮
...
Scene_Drill_SGaC.prototype.drill_refreshArrow = function( index ){
	...
	var u_visible = true;
	var d_visible = true;
	if( index % DrillUp.g_SGaC_command_window['col'] == 0 ){
		l_visible = false;
	}
...
// * 选项窗口 - 定义
//==============================
function Drill_SGaC_SelectWindow() {
	this.initialize.apply(this, arguments);
}
...
if( Imported.MOG_MenuCursor == true ){
	...
	Drill_SGaC_SelectWindow.prototype.need_set_mcursor_data = function() {
		if( DrillUp.g_SGaC_command_window['cursor']['mog_enabled'] == false ){
			return false;
		}
		return _drill_SGaC_mog_set_mcursor_data.call(this);
	}
}
//==============================
// * 选项窗口 - 兼容 - mog菜单边框插件
...
if( Imported.MOG_CursorBorder == true ){
	...
	Drill_SGaC_SelectWindow.prototype.createSprSelMenu = function() {
		if( DrillUp.g_SGaC_command_window['cursor']['mog_borderEnabled'] == false ){
			return ;
		}
		_drill_SGaC_mog_createSprSelMenu.call(this);
	}
}
//==============================
...
if( Imported.Drill_MenuCursor == true ){
	Drill_SGaC_SelectWindow.prototype.drill_MCu_cursorEnabled = function() {
		return DrillUp.g_SGaC_command_window['cursor']['MCu_enabled'];
	}
	Drill_SGaC_SelectWindow.prototype.drill_MCu_cursorStyleId = function() {
		if( DrillUp.g_SGaC_command_window['cursor']['MCu_lock'] == true ){
			return DrillUp.g_SGaC_command_window['cursor']['MCu_style'];
		}else{
...
if( Imported.Drill_MenuCursorBorder == true ){
	Drill_SGaC_SelectWindow.prototype.drill_MCB_glimmerRectVisible = function() {
		return DrillUp.g_SGaC_command_window['cursor']['MCB_rectEnabled'];
	}
	Drill_SGaC_SelectWindow.prototype.drill_MCB_borderEnabled = function() {
		return DrillUp.g_SGaC_command_window['cursor']['MCB_enabled'];
	}
	Drill_SGaC_SelectWindow.prototype.drill_MCB_borderStyleId = function() {
		if( DrillUp.g_SGaC_command_window['cursor']['MCB_lock'] == true ){
			return DrillUp.g_SGaC_command_window['cursor']['MCB_style'];
		}else{
...
if( Imported.Drill_MenuScrollBar == true ){
	Drill_SGaC_SelectWindow.prototype.drill_MSB_scrollBarEnabled = function() {
		return DrillUp.g_SGaC_command_window['cursor']['MSB_enabled'];
	}
	Drill_SGaC_SelectWindow.prototype.drill_MSB_scrollBarStyleId = function() {
		if( DrillUp.g_SGaC_command_window['cursor']['MSB_lock'] == true ){
			return DrillUp.g_SGaC_command_window['cursor']['MSB_style'];
		}else{
...
// * 显示窗口 - 定义
//==============================
function Drill_SGaC_DescWindow() {
    this.initialize.apply(this, arguments);
}
Drill_SGaC_DescWindow.prototype = Object.create(Window_Base.prototype);
Drill_SGaC_DescWindow.prototype.constructor = Drill_SGaC_DescWindow;
//==============================
// * 显示窗口 - 初始化
//==============================
...
// * 完成度窗口 - 定义
//==============================
function Drill_SGaC_CompletionWindow() {
    this.initialize.apply(this, arguments);
}
Drill_SGaC_CompletionWindow.prototype = Object.create(Window_Base.prototype);
Drill_SGaC_CompletionWindow.prototype.constructor = Drill_SGaC_CompletionWindow;
//==============================
// * 完成度窗口 - 初始化
//==============================
...
// * 原图查看器窗口 - 定义
//==============================
function Drill_SGaC_ImgExpandWindow() {
	this.initialize.apply(this, arguments);
}
Drill_SGaC_ImgExpandWindow.prototype = Object.create(Window_Selectable.prototype);
Drill_SGaC_ImgExpandWindow.prototype.constructor = Drill_SGaC_ImgExpandWindow;
//==============================
// * 原图查看器窗口 - 初始化
//==============================
...
// * 原图查看器窗口 - 窗口数据
//==============================
Drill_SGaC_ImgExpandWindow.prototype.maxCols = function(){ return 1; };
Drill_SGaC_ImgExpandWindow.prototype.maxItems = function(){ return 1; };
//==============================
// * 原图查看器窗口 - 帧刷新
//==============================
...
// * 原图查看器窗口 - 阻塞
//==============================
Drill_SGaC_ImgExpandWindow.prototype.cursorPagedown = function(){}
Drill_SGaC_ImgExpandWindow.prototype.cursorPageup = function(){}
Drill_SGaC_ImgExpandWindow.prototype.scrollDown = function(){}
Drill_SGaC_ImgExpandWindow.prototype.scrollUp = function(){}
...
// * 箭头 - 监听高亮声音
//==============================
var _drill_SGaC_drill_updateArrowHover = Scene_Drill_SGaC.prototype.drill_updateArrowHover;
Scene_Drill_SGaC.prototype.drill_updateArrowHover = function() {
	_drill_SGaC_drill_updateArrowHover.call(this);
	...
	if( this._arrow_hoverType != this._arrow_lastHoverType ){
		...
		// > 播放声音
		SoundManager.drill_SGaC_playHover_Arrow();
	}
}
//==============================
// * 箭头 - 高亮声音
//==============================
SoundManager.drill_SGaC_playHover_Arrow = function() {
	if( DrillUp.g_SGaC_arrowClickDefaultEnabled == true ){
		//（不操作）
	}else{
		if( DrillUp.g_SGaC_arrowHoverSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SGaC_arrowHoverSound );
		}
	}
}
//==============================
// * 箭头 - 点击声音
//==============================
SoundManager.drill_SGaC_playClick_Arrow = function() {
	if( DrillUp.g_SGaC_arrowClickDefaultEnabled == true ){
		SoundManager.playCursor();
	}else{
		if( DrillUp.g_SGaC_arrowClickSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SGaC_arrowClickSound );
		}
	}
}
...
//=============================================================================
		...
		Imported.Drill_SceneGalleryC = false;
		var pluginTip = DrillUp.drill_SGaC_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
