 ...
 * @on 锁定
 * @off 不锁定
 * @desc 默认不锁定，当你拉伸游戏窗口时，游戏窗口会根据固定比值来缩放。
 * @default false
 *
 * @param 是否自动释放缓存
 * @parent ---屏幕---
...
// * 字体设置 - 参数
//==============================
Yanfly.Param.ChineseFont = "GameFont";	//（全部由 窗口字符-字体管理器 控制）
Yanfly.Param.KoreanFont = "GameFont";
...
ScreenSprite.prototype.initialize = function() {
  ...
  this.scale.x = Graphics.boxWidth * 10;	//（不太理解为什么要把旧版本贴图放大那么大，但好在程序不会进入到这里）
...
Yanfly.openConsole = function() {
  ...
  if (Utils.isNwjs() && Utils.isOptionValid('test')) {
    ...
    var _debugWindow = win.showDevTools();
...
Yanfly.Util.displayError = function(e, code, message) {
  ...
  if (Utils.isNwjs() && Utils.isOptionValid('test')) {
	var gui = require('nw.gui'); 
	var win = gui.Window.get(); 
    if( win.isDevToolsOpen != undefined &&
