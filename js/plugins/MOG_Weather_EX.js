 ...
 * 参数E：混合模式
 *        pixi的渲染混合模式。0-普通,1-变亮。其他更详细相关介绍，去看看"0.基本定义 > 混合模式.docx"。
 * 参数F：天气粒子资源文件
 ...
 *
 *
 * $gameSystem.getWeatherOpacity(1)
 * $gameSystem.changeWeatherOpacity(id,po,time);
 * $gameSystem.setWeatherOpacity(id,opacity);
 * $gameSystem.getWeatherPow(1)
 * $gameSystem.getWeatherSpeed(1)
 * $gameSystem.ifWeather(id);
 * $gameSystem.changeWeatherPow(id,power,time);
 * $gameSystem.changeWeatherSpeed(id,speed,time);
 * -----------------------------------------------------------------------------
 * ----高级设置
 * 你还可以加更多参数控制天气。
 * （注意：冒号两边都有一个空格。）
 ...
 * @author ZPM
 */
Game_System.prototype.ifWeather = function(id) {
	id = parseInt(id);
	if (isNaN(id) || id >= Moghunter.weatherEX_max) {
		return false;
	};
	return (this._weatherEX_Data[id].mode > -1);
};
 ...
 * @author ZPM
 */
Game_System.prototype.getWeatherSpeed = function (id) {
	if (this.ifWeather(id)) {
		return this._weatherEX_Data[id].speed;
	};
	return 0;
};
 ...
 * @author ZPM
 */
Game_System.prototype.getWeatherPow = function (id) {
	if (this.ifWeather(id)) {
		return this._weatherEX_Data[id].power;
	};
	return 0;
};
 ...
 * @author ZPM
 */
Game_System.prototype.getWeatherOpacity = function (id) {
	if (this.ifWeather(id)) {
		return this._weatherEX_Data[id].opacity || 0;
	};
	return 0;
};
 ...
 * @author ZPM
 */
Game_System.prototype.changeWeatherSpeed = function (id, ps, t) {
	if (this.ifWeather(id)) {
		ps = parseInt(ps) || 100;
		t = parseInt(t) || 1;
		this._weatherEX_Data[id].speedChangePerTime = ps;
		this._weatherEX_Data[id].speedTime = t;
		return true;
	};
	return false;
};
 ...
 * @author ZPM
 */
Game_System.prototype.changeWeatherPow = function (id, pp, t) {
	if (this.ifWeather(id)) {
		pp = parseInt(pp) || 1;
		t = parseInt(t) || 1;
		this._weatherEX_Data[id].powerChangePerTime = pp;
		this._weatherEX_Data[id].powerTime = t;
		return true;
	};
	return false;
};
 ...
 * @author ZPM
 */
Game_System.prototype.changeWeatherOpacity = function (id, po, t) {
	if (this.ifWeather(id)) {
		po = parseInt(po) || 1;
		t = parseInt(t) || 1;
		this._weatherEX_Data[id].opacityChangePerTime = po;
		this._weatherEX_Data[id].opacityTime = t;
		return true;
	};
	return false;
};
 ...
 * @author ZPM
 */
Game_System.prototype.setWeatherOpacity = function (id, opacity) {
	id = parseInt(id);
	if (isNaN(id) || id >= Moghunter.weatherEX_max) {
		return false;
	};
	opacity = Math.abs(parseInt(opacity) || 0);
	this._weatherEX_Data[id].opacity = opacity;
	return opacity;
};
...
Game_System.prototype.clearWeatherEX = function(id,needrefresh) {
	...
	/** @author ZPM */
	this._weatherEX_Data[id].opacity = 255;
	/** -------------------------------------------------- */
};
...
Game_Map.prototype.setWeatherEX = function (nt) {
	...
	notetags.forEach(function (note) {
		...
		if (note_data[0].toLowerCase() == "weather") {
...
Spriteset_Base.prototype.reloadWeatherEX = function() {
	/**@author ZPM @index 1 */
	if( !$gameSystem._weatherEX_Data ){
		$gameSystem.weatherEX_initialize();
		return;
	};
	/** -------------------------------------------------- */
...
Spriteset_Base.prototype.createWeatherField_3 = function(mode) {
	...
	this._drill_mapPicArea.addChild(this._weatherField_3);
};
 ...
 * @anthor ZPM
 */
SpriteWeatherEX.prototype.changePowerWith = function (pow) {
	if (pow > 0) {
		this.powerUp(pow)
	};
	if (pow < 0) {
		this.powerDown(Math.min(Math.abs(pow), (this._sprites.length - 1)));
	};
	this.data().power = this._sprites.length;
};
 ...
 * @anthor ZPM
 */
SpriteWeatherEX.prototype.powerUp = function (pow) {
	for (var i = this.data().power; i < this.data().power + pow; i++) {
		if (this.isTilingWeather()) {
			this._sprites[i] = new TilingSprite(this._image);
			this._sprites[i].tiling = true;
			this._sprites[i].needRefreshMove = false;
 ...
 * @anthor ZPM
 */
SpriteWeatherEX.prototype.powerDown = function (pow) {
	for (var i = 0; i < pow; i++) {
 ...
 * @author ZPM
 */
SpriteWeatherEX.prototype.updateChanges = function () {
	var _wea = this.data();
	if (_wea.speedTime > 0) {
		_wea.speedTime--;
		_wea.speed += _wea.speedChangePerTime;
	};
	if (_wea.powerTime > 0) {
		_wea.powerTime--;
		this.changePowerWith(_wea.powerChangePerTime);
	};
	if (_wea.opacityTime > 0) {
		_wea.opacityTime--;
		this.opacity+= _wea.opacityChangePerTime;
		_wea.opacity = this.opacity;
	};
};
