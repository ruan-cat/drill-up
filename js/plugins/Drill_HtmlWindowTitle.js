...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_HWT_PluginTip_curName = "Drill_HtmlWindowTitle.js 系统-游戏窗体标题修改";
	DrillUp.g_HWT_PluginTip_baseList = ["Drill_CoreOfGlobalSave.js 管理器-全局存储核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	...
	DrillUp.drill_HWT_getPluginTip_NoBasePlugin = function(){
		...
		var message = "【" + DrillUp.g_HWT_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_HWT_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_HWT_PluginTip_baseList[i];
		}
		return message;
	};
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_HWT_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
