...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_SSpG_PluginTip_curName = "Drill_SceneSelfplateG.js 面板-全自定义信息面板G";
	DrillUp.g_SSpG_PluginTip_baseList = [
	...
	//==============================
	DrillUp.drill_SSpG_getPluginTip_NoBasePlugin = function(){
		if( DrillUp.g_SSpG_PluginTip_baseList.length == 0 ){ return ""; }
		var message = "【" + DrillUp.g_SSpG_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_SSpG_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_SSpG_PluginTip_baseList[i];
		}
		return message;
	};
...
　　var DrillUp = DrillUp || {}; 
	...
	//				（~struct~SSpGSound）
	//==============================
	DrillUp.drill_SSpG_initSound = function( dataFrom ){
		var data = {};
	...
	DrillUp.g_SSpG_arrowClickDefaultEnabled = String(DrillUp.parameters['箭头是否使用默认音效'] || "true") === "true";
	if( DrillUp.parameters['箭头悬停音效'] != "" && 
		DrillUp.parameters['箭头悬停音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头悬停音效']);
		DrillUp.g_SSpG_arrowHoverSound = DrillUp.drill_SSpG_initSound( temp );
	}else{
		DrillUp.g_SSpG_arrowHoverSound = null;
	}
	if( DrillUp.parameters['箭头点击音效'] != "" && 
		DrillUp.parameters['箭头点击音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头点击音效']);
		DrillUp.g_SSpG_arrowClickSound = DrillUp.drill_SSpG_initSound( temp );
	}else{
		DrillUp.g_SSpG_arrowClickSound = null;
	}
	...
	DrillUp.g_SSpG_endClickDefaultEnabled = String(DrillUp.parameters['结束按钮是否使用默认音效'] || "true") === "true";
	if( DrillUp.parameters['结束按钮悬停音效'] != "" && 
		DrillUp.parameters['结束按钮悬停音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['结束按钮悬停音效']);
		DrillUp.g_SSpG_endHoverSound = DrillUp.drill_SSpG_initSound( temp );
	}else{
		DrillUp.g_SSpG_endHoverSound = null;
	}
	if( DrillUp.parameters['结束按钮点击音效'] != "" && 
		DrillUp.parameters['结束按钮点击音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['结束按钮点击音效']);
		DrillUp.g_SSpG_endClickSound = DrillUp.drill_SSpG_initSound( temp );
	}else{
		DrillUp.g_SSpG_endClickSound = null;
	}
...
// * 信息面板G（场景基类） - 开始运行
//==============================
Scene_Drill_SSpG.prototype.start = function() {
    Scene_MenuBase.prototype.start.call(this);
};
//==============================
// * 信息面板G（场景基类） - 结束运行
//==============================
Scene_Drill_SSpG.prototype.stop = function() {
    Scene_MenuBase.prototype.stop.call(this);
};
//==============================
// * 信息面板G（场景基类） - 判断是否激活/启动
//==============================
Scene_Drill_SSpG.prototype.isActive = function() {
	return Scene_MenuBase.prototype.isActive.call(this);
};
//==============================
// * 信息面板G（场景基类） - 析构函数
//==============================
Scene_Drill_SSpG.prototype.terminate = function() {
...
// * 信息面板G（场景基类） - 判断加载完成
//==============================
Scene_Drill_SSpG.prototype.isReady = function() {
	return Scene_MenuBase.prototype.isReady.call(this);
};
//==============================
// * 信息面板G（场景基类） - 忙碌状态
//==============================
Scene_Drill_SSpG.prototype.isBusy = function() {
	return Scene_MenuBase.prototype.isBusy.call(this);
};

//==============================
// * 信息面板G（菜单界面基类） - 创建 - 菜单背景
//==============================
Scene_Drill_SSpG.prototype.createBackground = function() {
	Scene_MenuBase.prototype.createBackground.call(this);
};
//==============================
// * 信息面板G（菜单界面基类） - 创建 - 帮助窗口
//==============================
Scene_Drill_SSpG.prototype.createHelpWindow = function() {
...
// * 箭头 - 帧刷新 贴图
//==============================
Scene_Drill_SSpG.prototype.drill_updateArrowSprite = function() {
	...
	if( DrillUp.g_SSpG_arrow_float_lr ){
		this._arrow_left.x = this._arrow_left._org_x + this._arrow_changeValue* DrillUp.g_SSpG_arrow_float_val ; 	
		this._arrow_left.y = this._arrow_left._org_y ; 
		this._arrow_right.x = this._arrow_right._org_x - this._arrow_changeValue* DrillUp.g_SSpG_arrow_float_val ; 
		this._arrow_right.y = this._arrow_right._org_y ;
	}
	// > 上下浮动
	if( DrillUp.g_SSpG_arrow_float_ud ){
		this._arrow_up.x = this._arrow_up._org_x;
		this._arrow_up.y = this._arrow_up._org_y + this._arrow_changeValue* DrillUp.g_SSpG_arrow_float_val ; 	
		this._arrow_down.x = this._arrow_down._org_x;
		this._arrow_down.y = this._arrow_down._org_y - this._arrow_changeValue* DrillUp.g_SSpG_arrow_float_val ; 	
	}
	// > 缩放
...
// * 箭头 - 帧刷新 点击
//==============================
Scene_Drill_SSpG.prototype.drill_updateArrowTouch = function() {
    if( TouchInput.isTriggered() ){
		if( this._arrow_hoverType == 4 ){ this._window_select.cursorLeft();  SoundManager.drill_SSpG_playClick_Arrow(); }
		if( this._arrow_hoverType == 6 ){ this._window_select.cursorRight(); SoundManager.drill_SSpG_playClick_Arrow(); }
		if( this._arrow_hoverType == 8 ){ this._window_select.cursorUp();    SoundManager.drill_SSpG_playClick_Arrow(); }
		if( this._arrow_hoverType == 2 ){ this._window_select.cursorDown();  SoundManager.drill_SSpG_playClick_Arrow(); }
	}
}
//==============================
// * 箭头 - 帧刷新 高亮
//==============================
Scene_Drill_SSpG.prototype.drill_updateArrowHover = function() {
...
// * 箭头 - 帧刷新 上一次高亮
//==============================
Scene_Drill_SSpG.prototype.drill_updateArrowLastHover = function() {
	this._arrow_lastHoverType = this._arrow_hoverType;
}
//==============================
// * 箭头 - 是否鼠标悬停
//==============================
Scene_Drill_SSpG.prototype.drill_isOnHover = function( x, y, temp_sprite ){
...
Drill_SSpG_SelectWindow.prototype.processCancel = function() {
	if( !this.drill_isInEndPage() ){		//非末尾选项，只翻页
		this.cursorRight();
		SoundManager.drill_SSpG_playClick_Arrow();
		return ;
	}
...
// * 流程锁定 - 结束按钮 - 帧刷新 贴图
//==============================
Scene_Drill_SSpG.prototype.drill_updateEndBtnSprite = function() {
...
// * 流程锁定 - 结束按钮 - 帧刷新 点击
//==============================
Scene_Drill_SSpG.prototype.drill_updateEndBtnTouch = function() {
    if( TouchInput.isTriggered() ){
		if( this._end_isHover == true ){
			SoundManager.drill_SSpG_playClick_EndBtn();
			this.popScene();
		}
	}
}
...
// * 流程锁定 - 结束按钮 - 帧刷新 高亮
//==============================
Scene_Drill_SSpG.prototype.drill_updateEndBtnHover = function() {
	var xx = _drill_mouse_x;
	var yy = _drill_mouse_y;
...
// * 流程锁定 - 结束按钮 - 帧刷新 上一次高亮
//==============================
Scene_Drill_SSpG.prototype.drill_updateEndBtnLastHover = function() {
...
// * 箭头 - 监听高亮声音
//==============================
var _drill_SSpG_drill_updateArrowHover = Scene_Drill_SSpG.prototype.drill_updateArrowHover;
Scene_Drill_SSpG.prototype.drill_updateArrowHover = function() {
	_drill_SSpG_drill_updateArrowHover.call(this);
	...
	if( this._arrow_hoverType != this._arrow_lastHoverType ){
		...
		// > 播放声音
		SoundManager.drill_SSpG_playHover_Arrow();
	}
}
//==============================
// * 箭头 - 高亮声音
//==============================
SoundManager.drill_SSpG_playHover_Arrow = function() {
	if( DrillUp.g_SSpG_arrowClickDefaultEnabled == true ){
		//（不操作）
	}else{
		if( DrillUp.g_SSpG_arrowHoverSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpG_arrowHoverSound );
		}
	}
}
//==============================
// * 箭头 - 点击声音
//==============================
SoundManager.drill_SSpG_playClick_Arrow = function() {
	if( DrillUp.g_SSpG_arrowClickDefaultEnabled == true ){
		SoundManager.playCursor();
	}else{
		if( DrillUp.g_SSpG_arrowClickSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpG_arrowClickSound );
		}
	}
}
...
// * 结束按钮 - 监听高亮声音
//==============================
var _drill_SSpG_drill_updateEndBtnHover = Scene_Drill_SSpG.prototype.drill_updateEndBtnHover;
Scene_Drill_SSpG.prototype.drill_updateEndBtnHover = function() {
	_drill_SSpG_drill_updateEndBtnHover.call(this);
	...
	if( this._end_isHover != this._end_isLastHover ){
		...
		// > 播放声音
		SoundManager.drill_SSpG_playHover_EndBtn();
	}
}
//==============================
// * 结束按钮 - 高亮声音
//==============================
SoundManager.drill_SSpG_playHover_EndBtn = function() {
	if( DrillUp.g_SSpG_endClickDefaultEnabled == true ){
		SoundManager.playCursor();
	}else{
		if( DrillUp.g_SSpG_endHoverSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpG_endHoverSound );
		}
	}
}
//==============================
// * 结束按钮 - 点击声音
//==============================
SoundManager.drill_SSpG_playClick_EndBtn = function() {
	if( DrillUp.g_SSpG_endClickDefaultEnabled == true ){
		SoundManager.playOk();
	}else{
		if( DrillUp.g_SSpG_endClickSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpG_endClickSound );
		}
	}
}
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_SSpG_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
