...
//=============================================================================
	...
	// * 提示信息 - 报错 - 底层版本过低
	//==============================
	DrillUp.drill_MPFC_getPluginTip_LowVersion = function(){
		return "【" + DrillUp.g_MPFC_PluginTip_curName + "】\n游戏底层版本过低，插件基本功能无法执行。\n你可以去看\"rmmv软件版本（必看）.docx\"中的 \"旧工程升级至1.6版本\" 章节，来升级你的游戏底层版本。";
	};
...
// * 预加载 - 初始化
//==============================
var _drill_MPFC_preload_initialize = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
    _drill_MPFC_preload_initialize.call(this);
	this.drill_MPFC_preloadInit();
}
//==============================
// * 预加载 - 版本校验
//==============================
if( Utils.generateRuntimeId == undefined ){
	alert( DrillUp.drill_MPFC_getPluginTip_LowVersion() );
}
//==============================
// * 预加载 - 执行资源预加载
//
...
//==============================
Game_Temp.prototype.drill_MPFC_preloadInit = function() {
	this._drill_MPFC_cacheId = Utils.generateRuntimeId();	//资源缓存id
    this._drill_MPFC_preloadTank = [];						//bitmap容器
	for( var i = 0; i < DrillUp.g_MPFC_style_list.length; i++ ){
		var temp_data = DrillUp.g_MPFC_style_list[i];
		if( temp_data == undefined ){ continue; }
		
		// > 『窗口皮肤的预加载』
		if( temp_data['window_type'] == "自定义窗口皮肤" ){
			this._drill_MPFC_preloadTank.push( 
				ImageManager.reserveBitmap( "img/system/", temp_data['window_sys_src'], 0, true, this._drill_MPFC_cacheId ) 
			);
		}
		if( temp_data['window_type'] == "自定义背景图片" ){
			this._drill_MPFC_preloadTank.push( 
				ImageManager.reserveBitmap( "img/system/", temp_data['window_pic_src'], 0, true, this._drill_MPFC_cacheId ) 
			);
		}
	}
}
...
// * C位置跟随 - 重设数据
//==============================
Drill_MPFC_Window.prototype.drill_resetData_Position = function( data ) {
...
//==============================
Drill_MPFC_Window.prototype.drill_resetData_Skin = function( data ){
...
// * E窗口内容 - 重设数据
//==============================
Drill_MPFC_Window.prototype.drill_resetData_Message = function( data ){
