...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_EDMB_PluginTip_curName = "Drill_EventDynamicMaskB.js 地图-行走图动态遮罩板B";
	DrillUp.g_EDMB_PluginTip_baseList = ["Drill_CoreOfDynamicMask.js 系统-动态遮罩核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	...
	DrillUp.drill_EDMB_getPluginTip_NoBasePlugin = function(){
		...
		var message = "【" + DrillUp.g_EDMB_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_EDMB_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_EDMB_PluginTip_baseList[i];
		}
		return message;
	};
	//==============================
	// * 提示信息 - 报错 - 找不到事件
	//==============================
	DrillUp.drill_EDMB_getPluginTip_EventNotFind = function( e_id ){
		return "【" + DrillUp.g_EDMB_PluginTip_curName + "】\n插件指令错误，当前地图并不存在id为"+e_id+"的事件。";
	};
	//==============================
	// * 提示信息 - 报错 - 找不到图片
	//==============================
	DrillUp.drill_EDMB_getPluginTip_PictureNotFind = function( pic_id ){
		return "【" + DrillUp.g_EDMB_PluginTip_curName + "】\n插件指令错误，id为"+pic_id+"的图片还没被创建。\n你可能需要将指令放在'显示图片'事件指令之后。";
	};
	//==============================
	// * 提示信息 - 报错 - 高级透视镜未创建
	//==============================
	DrillUp.drill_EDMB_getPluginTip_DataNotCreate = function( data_id ){
		return "【" + DrillUp.g_EDMB_PluginTip_curName + "】\n插件指令错误，id为"+data_id+"的高级透视镜未创建，需要创建再使用。";
	};
	//==============================
	// * 提示信息 - 报错 - 找不到样式配置
	//==============================
	DrillUp.drill_EDMB_getPluginTip_DataNotFind = function( data_id ){
	...
	if( command === ">行走图动态遮罩板B" ){
		...
		if( args.length >= 2 ){
			...
			if( temp1.indexOf("高级透视镜[") != -1 ){
				...
				if( marker == undefined ){
					alert( DrillUp.drill_EDMB_getPluginTip_DataNotCreate( temp1 ) );
				}
...
Game_Map.prototype.drill_EDMB_isEventExist = function( e_id ){
	...
	if( e == undefined ){
		alert( DrillUp.drill_EDMB_getPluginTip_EventNotFind( e_id ) );
		return false;
	}
	return true;
};
...
Game_Screen.prototype.drill_EDMB_isPictureExist = function( pic_id ){
	...
	if( pic == undefined ){
		alert( DrillUp.drill_EDMB_getPluginTip_PictureNotFind( pic_id ) );
		return false;
	}
	return true;
};
...
Game_Map.prototype.drill_EDMB_addSimplePerspect_characterId = function( character_id, style_id ){	
	...
	if( data == undefined ){
		alert( DrillUp.drill_EDMB_getPluginTip_DataNotFind(style_id+1) );
		return;
	}
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_EDMB_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
