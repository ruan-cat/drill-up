 ...
 * 插件指令：>信息面板D : 打开面板
 *
 * 插件指令：>信息面板D : 显示选项 : 选项[1]
 * 插件指令：>信息面板D : 显示选项 : 选项变量[21]
 * 插件指令：>信息面板D : 隐藏选项 : 选项[1]
 * 插件指令：>信息面板D : 隐藏选项 : 选项变量[21]
 * 插件指令：>信息面板D : 显示全部选项
 * 插件指令：>信息面板D : 隐藏全部选项
 * 
 * 插件指令：>信息面板D : 锁定选项 : 选项[1]
 * 插件指令：>信息面板D : 锁定选项 : 选项变量[21]
 * 插件指令：>信息面板D : 解锁选项 : 选项[1]
 * 插件指令：>信息面板D : 解锁选项 : 选项变量[21]
 * 插件指令：>信息面板D : 锁定全部选项
 * 插件指令：>信息面板D : 解锁全部选项
 *
 ...
 * 
 * 插件指令：>信息面板D : 选中选项 : 选项[1]
 * 插件指令：>信息面板D : 选中选项 : 选项变量[21]
 * 
 ...
 * @desc 控制窗口框架与窗口背景。
 * @default {"布局类型":"单张背景贴图","---单张背景贴图---":"","资源-贴图":"信息面板D-选项窗口","贴图位置修正 X":"0","贴图位置修正 Y":"0"}
 * 
 * @param 选项窗口指针与边框
 * @parent ----选项窗口----
...
//		临时局部变量	this._drill_SSpD_xxx
//		存储数据变量	$gameSystem._drill_SSpD_context_list
//		全局存储变量	DrillUp.global_SSpD_enableTank
//		覆盖重写方法	无
//
//<<<<<<<<性能记录<<<<<<<<
//
...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_SSpD_PluginTip_curName = "Drill_SceneSelfplateD.js 面板-全自定义信息面板D";
	DrillUp.g_SSpD_PluginTip_baseList = [
	...
	//==============================
	DrillUp.drill_SSpD_getPluginTip_NoBasePlugin = function(){
		if( DrillUp.g_SSpD_PluginTip_baseList.length == 0 ){ return ""; }
		var message = "【" + DrillUp.g_SSpD_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_SSpD_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_SSpD_PluginTip_baseList[i];
		}
		return message;
	};
...
　　var DrillUp = DrillUp || {}; 
	...
	//				（~struct~DrillCursor）
	//==============================
	DrillUp.drill_SSpD_initMenuCursor = function( dataFrom ) {
		var data = {};
	...
	//				（~struct~SSpDSound）
	//==============================
	DrillUp.drill_SSpD_initSound = function( dataFrom ){
		var data = {};
    ...
    DrillUp.g_SSpD_title_data_fileId = Number(DrillUp.parameters['全局存储的文件路径'] || 1);	
	...
	DrillUp.g_SSpD_arrow_flicker = String(DrillUp.parameters['是否使用闪烁效果'] || "false") === "true";	
	...
	DrillUp.g_SSpD_arrowClickDefaultEnabled = String(DrillUp.parameters['箭头是否使用默认音效'] || "true") === "true";
	if( DrillUp.parameters['箭头悬停音效'] != "" && 
		DrillUp.parameters['箭头悬停音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头悬停音效']);
		DrillUp.g_SSpD_arrowHoverSound = DrillUp.drill_SSpD_initSound( temp );
	}else{
		DrillUp.g_SSpD_arrowHoverSound = null;
	}
	if( DrillUp.parameters['箭头点击音效'] != "" && 
		DrillUp.parameters['箭头点击音效'] != undefined ){
		var temp = JSON.parse(DrillUp.parameters['箭头点击音效']);
		DrillUp.g_SSpD_arrowClickSound = DrillUp.drill_SSpD_initSound( temp );
	}else{
		DrillUp.g_SSpD_arrowClickSound = null;
	}
	...
	if( DrillUp.parameters['选项窗口指针与边框'] != "" &&
		...
		DrillUp.g_SSpD_selWin_cursor = DrillUp.drill_SSpD_initMenuCursor( cursor );
	}else{
		DrillUp.g_SSpD_selWin_cursor = DrillUp.drill_SSpD_initMenuCursor( {} );
	}
	...
	for( var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ){
		if( DrillUp.parameters["内容-" + String(i) ] != "" &&
			...
			DrillUp.g_SSpD_context_list[i] = null;
		}
	};
	...
	/*-----------------全局存储对象------------------*/
	DrillUp.global_SSpD_enableTank = null;
	DrillUp.global_SSpD_lockTank = null;
...
// * 全局 - 检查数据 - 显示情况
//==============================
DrillUp.drill_SSpD_gCheckData_enable = function(){
	for( var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ){
		var temp_c = DrillUp.g_SSpD_context_list[i];
		
		// > 指定数据为空时
		if( DrillUp.global_SSpD_enableTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				DrillUp.global_SSpD_enableTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				DrillUp.global_SSpD_enableTank[i] = temp_c['enabled'];
			}
...
// * 全局 - 检查数据 - 锁定情况
//==============================
DrillUp.drill_SSpD_gCheckData_lock = function(){
	for( var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ){
		var temp_c = DrillUp.g_SSpD_context_list[i];
		
		// > 指定数据为空时
		if( DrillUp.global_SSpD_lockTank[i] == null ){
			if( temp_c == null ){		//（无内容配置，跳过）
				DrillUp.global_SSpD_lockTank[i] = null;
			}else{						//（有内容配置，初始化默认）
				DrillUp.global_SSpD_lockTank[i] = temp_c['locked'];
			}
...
// * 全局 - 读取
//==============================
	var global_fileId = DrillUp.g_SSpD_title_data_fileId;
	var global_data = StorageManager.drill_COGS_loadData( global_fileId, "SSpD" );
	
	// > 显示情况
	if( DrillUp.global_SSpD_enableTank == null ){			//（游戏没关时，不会为null)
		var data = global_data["global_enableTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SSpD_enableTank = data;
		DrillUp.drill_SSpD_gCheckData_enable();				//（检查时自动赋新值）
	}
	// > 锁定情况
	if( DrillUp.global_SSpD_lockTank == null ){	
		var data = global_data["global_lockTank"];
		if( data == undefined ){ data = [] };
		DrillUp.global_SSpD_lockTank = data;
		DrillUp.drill_SSpD_gCheckData_lock();
	}
	
//==============================
// * 全局 - 存储
//==============================
StorageManager.drill_SSpD_saveData = function(){
	var file_id = DrillUp.g_SSpD_title_data_fileId;
	var data = {};
	data["global_enableTank"] = DrillUp.global_SSpD_enableTank;
	data["global_lockTank"] = DrillUp.global_SSpD_lockTank;
	this.drill_COGS_saveData( file_id, "SSpD", data );
};
...
//##############################
DrillUp.g_SSpD_saveEnabled = true;
//##############################
// * 存储数据 - 初始化
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SSpD_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_SSpD_sys_initialize.call(this);
	this.drill_SSpD_initSysData();
};
//##############################
// * 存储数据 - 载入存档
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SSpD_sys_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function( contents ){
	_drill_SSpD_sys_extractSaveContents.call( this, contents );
	
	// > 参数存储 启用时（检查数据）
	if( DrillUp.g_SSpD_saveEnabled == true ){	
		$gameSystem.drill_SSpD_checkSysData();
		
	// > 参数存储 关闭时（直接覆盖）
	}else{
		$gameSystem.drill_SSpD_initSysData();
	}
};
//##############################
// * 存储数据 - 初始化数据【标准函数】
...
//##############################
Game_System.prototype.drill_SSpD_initSysData = function() {
	this.drill_SSpD_initSysData_Private();
};
//##############################
// * 存储数据 - 载入存档时检查数据【标准函数】
//			
...
//##############################
Game_System.prototype.drill_SSpD_checkSysData = function() {
	this.drill_SSpD_checkSysData_Private();
};
//=============================================================================
// ** 存储数据（接口实现）
//=============================================================================
...
// * 存储数据 - 初始化数据（私有）
//==============================
Game_System.prototype.drill_SSpD_initSysData_Private = function() {
	
	this._drill_SSpD_enableTank = [];				//显示情况
	this._drill_SSpD_lockTank = [];					//锁定情况
	for(var i = 0; i < DrillUp.g_SSpD_context_list.length; i++){
		var temp_data = DrillUp.g_SSpD_context_list[i];
		if( temp_data == undefined ){ continue; }
		this._drill_SSpD_enableTank[i] = temp_data['enabled'];
		this._drill_SSpD_lockTank[i] = temp_data['locked'];
	}
};
//==============================
// * 存储数据 - 载入存档时检查数据（私有）
//==============================
Game_System.prototype.drill_SSpD_checkSysData_Private = function() {
	
	// > 旧存档数据自动补充
	if( this._drill_SSpD_enableTank == undefined ){
		this.drill_SSpD_initSysData();
	}
	...
	for( var i = 0; i < DrillUp.g_SSpD_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpD_enableTank[i] == undefined ){
				this._drill_SSpD_enableTank[i] = temp_data['enabled'];
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpD_lockTank[i] == undefined ){
				this._drill_SSpD_lockTank[i] = temp_data['locked'];
	...
	if( command === ">信息面板D" ){
		...
		if(args.length == 2){
			...
			if( type == "显示全部选项" || type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.global_SSpD_enableTank[i] = true;			//全局存储
					$gameSystem._drill_SSpD_enableTank[i] = true;		//正常存储
				}
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "隐藏全部选项" || type == "隐藏全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.global_SSpD_enableTank[i] = false;			//全局存储
					$gameSystem._drill_SSpD_enableTank[i] = false;		//正常存储
				}
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "锁定全部选项" || type == "锁定全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.global_SSpD_lockTank[i] = true;				//全局存储
					$gameSystem._drill_SSpD_lockTank[i] = true;			//正常存储
				}
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "解锁全部选项" || type == "解锁全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					DrillUp.global_SSpD_lockTank[i] = false;			//全局存储
					$gameSystem._drill_SSpD_lockTank[i] = false;		//正常存储
				}
				StorageManager.drill_SSpD_saveData();
			}
		}
		...
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				DrillUp.global_SSpD_enableTank[ Number(temp1) ] = true;			//全局存储
				$gameSystem._drill_SSpD_enableTank[ Number(temp1) ] = true;		//正常存储
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "隐藏选项" ){
				DrillUp.global_SSpD_enableTank[ Number(temp1) ] = false;		//全局存储
				$gameSystem._drill_SSpD_enableTank[ Number(temp1) ] = false;	//正常存储
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "锁定选项" ){
				DrillUp.global_SSpD_lockTank[ Number(temp1) ] = true;			//全局存储
				$gameSystem._drill_SSpD_lockTank[ Number(temp1) ] = true;		//正常存储
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "解锁选项" ){
				DrillUp.global_SSpD_lockTank[ Number(temp1) ] = false;			//全局存储
				$gameSystem._drill_SSpD_lockTank[ Number(temp1) ] = false;		//正常存储
				StorageManager.drill_SSpD_saveData();
			}
			if( type == "选中选项" || type == "选中页" ){
				$gameSystem._drill_SSpD_context_index = Number(temp1) -1;
			}
		}
	}
...
// ** ☆主菜单选项
//
//			说明：	> 此模块专门关联主菜单选项，选项进入后跳转到 信息面板D 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================
...
// ** ☆标题选项
//
//			说明：	> 此模块专门关联标题选项，选项进入后跳转到 信息面板D 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================	
...
// * 面板控制 - 判断 锁定情况
//==============================
Game_Temp.prototype.drill_SSpD_isLocked = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SSpD_title_data_global == true ){
		if( DrillUp.global_SSpD_lockTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SSpD_lockTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * 面板控制 - 判断 显示情况
//==============================
Game_Temp.prototype.drill_SSpD_isEnabled = function( context_realIndex ){
	
	// > 全局存储控制
	if( DrillUp.g_SSpD_title_data_global == true ){
		if( DrillUp.global_SSpD_enableTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
	...
	// > 正常存储控制
		...
		if( $gameSystem._drill_SSpD_enableTank[ context_realIndex ] == true ){
			return true;
		}else{
			return false;
		}
...
// * 创建 - 整体布局
//==============================
Scene_Drill_SSpD.prototype.drill_createLayout = function() {
...
// * 创建 - 选项窗口
//==============================
Scene_Drill_SSpD.prototype.drill_createSelect = function() {
	var data = {
...
// * 创建 - 描述窗口
//==============================
Scene_Drill_SSpD.prototype.drill_createDesc = function() {
	var data = {
...
// * 创建 - 描述图片
//==============================
Scene_Drill_SSpD.prototype.drill_createDescPic = function() {
	var data = {
...
// * 信息面板D - 重设窗口起点（切换选项时）
//==============================
Scene_Drill_SSpD.prototype.drill_resetPosition = function() {
...
Scene_Drill_SSpD.prototype.drill_refreshDescPic = function( cur_index ) {
	...
	if( src_tank.length == 0 ){
		...
		for( var i=0; i < temp_list.length; i++ ){
			...
			src_tank[ i+1 ] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpD_context_list[ context_realIndex ]["pic"]);	
		}
	...
	if( $gameTemp.drill_SSpD_isLocked( context_realIndex ) == true && 
...
// * 帧刷新 - 描述图片
//==============================
Scene_Drill_SSpD.prototype.drill_updateDescPic = function() {
	if( DrillUp.g_SSpD_descPic_showInstant == false ){
		this._sprite_descPic.opacity += 255/DrillUp.g_SSpD_descPic_slideAnim['slideTime'];
	}
}
//==============================
// * 帧刷新 - 窗口选项刷新
//==============================
Scene_Drill_SSpD.prototype.drill_updateIndex = function() {
	if( $gameSystem._drill_SSpD_context_index != undefined ){
		var temp = $gameSystem._drill_SSpD_context_index;
		$gameSystem._drill_SSpD_context_index = null;	//（激活后清空）
		if( temp < 0 ){ temp = 0; };
...
// * 箭头 - 创建
//==============================
Scene_Drill_SSpD.prototype.drill_createArrow = function() {
...
// * 箭头 - 帧刷新 贴图
//==============================
Scene_Drill_SSpD.prototype.drill_updateArrowSprite = function() {
	...
	if( DrillUp.g_SSpD_arrow_float_lr ){
		this._arrow_left.x = this._arrow_left._org_x + this._arrow_changeValue* DrillUp.g_SSpD_arrow_float_val ; 	
		this._arrow_left.y = this._arrow_left._org_y ; 
		this._arrow_right.x = this._arrow_right._org_x - this._arrow_changeValue* DrillUp.g_SSpD_arrow_float_val ; 
		this._arrow_right.y = this._arrow_right._org_y ;
	}
	// > 上下浮动
	if( DrillUp.g_SSpD_arrow_float_ud ){
		this._arrow_up.x = this._arrow_up._org_x;
		this._arrow_up.y = this._arrow_up._org_y + this._arrow_changeValue* DrillUp.g_SSpD_arrow_float_val ; 	
		this._arrow_down.x = this._arrow_down._org_x;
		this._arrow_down.y = this._arrow_down._org_y - this._arrow_changeValue* DrillUp.g_SSpD_arrow_float_val ; 	
	}
	// > 缩放
	...
	// > 闪烁
	if( DrillUp.g_SSpD_arrow_flicker ){
...
// * 箭头 - 帧刷新 点击
//==============================
Scene_Drill_SSpD.prototype.drill_updateArrowTouch = function() {
    if( TouchInput.isTriggered() ){
		if( this._arrow_hoverType == 4 ){ this._window_select.cursorLeft();  SoundManager.drill_SSpD_playClick_Arrow(); }
		if( this._arrow_hoverType == 6 ){ this._window_select.cursorRight(); SoundManager.drill_SSpD_playClick_Arrow(); }
		if( this._arrow_hoverType == 8 ){ this._window_select.cursorUp();    SoundManager.drill_SSpD_playClick_Arrow(); }
		if( this._arrow_hoverType == 2 ){ this._window_select.cursorDown();  SoundManager.drill_SSpD_playClick_Arrow(); }
	}
}
//==============================
// * 箭头 - 帧刷新 高亮
//==============================
Scene_Drill_SSpD.prototype.drill_updateArrowHover = function() {
...
// * 箭头 - 帧刷新 上一次高亮
//==============================
Scene_Drill_SSpD.prototype.drill_updateArrowLastHover = function() {
	this._arrow_lastHoverType = this._arrow_hoverType;
}
//==============================
// * 箭头 - 是否鼠标悬停
//==============================
Scene_Drill_SSpD.prototype.drill_isOnHover = function( x, y, temp_sprite ){
...
// * 箭头 - 窗口选项刷新 箭头显示
//==============================
Scene_Drill_SSpD.prototype.drill_refreshArrow = function( index ){
	var l_visible = true;
	var r_visible = true;
	var u_visible = true;
	var d_visible = true;
...
Drill_SSpD_SelectWindow.prototype.refresh = function() {
	...
	for(var i=1; i<= DrillUp.g_SSpD_context_list_length ;i++){
		...
		if( $gameTemp.drill_SSpD_isEnabled( i ) == true ){
			$gameTemp._drill_SSpD_visibleList.push( temp_c );
		}
	}
	...
	for( var j=0; j< $gameTemp._drill_SSpD_visibleList.length ;j++ ){
		var temp_c = $gameTemp._drill_SSpD_visibleList[j];
		var context_realIndex = temp_c['index'];
...
if( Imported.MOG_MenuCursor == true ){
	...
	Drill_SSpD_SelectWindow.prototype.need_set_mcursor_data = function() {
		if( DrillUp.g_SSpD_selWin_cursor['mog_enabled'] == false ){
			return false;
		}
...
if( Imported.MOG_CursorBorder == true ){
	...
	Drill_SSpD_SelectWindow.prototype.createSprSelMenu = function() {
		if( DrillUp.g_SSpD_selWin_cursor['mog_borderEnabled'] == false ){
			return ;
		}
...
if( Imported.Drill_MenuCursor == true ){
	Drill_SSpD_SelectWindow.prototype.drill_MCu_cursorEnabled = function() {
		return DrillUp.g_SSpD_selWin_cursor['MCu_enabled'];
	}
	Drill_SSpD_SelectWindow.prototype.drill_MCu_cursorStyleId = function() {
		if( DrillUp.g_SSpD_selWin_cursor['MCu_lock'] == true ){
			return DrillUp.g_SSpD_selWin_cursor['MCu_style'];
		}else{
...
if( Imported.Drill_MenuCursorBorder == true ){
	Drill_SSpD_SelectWindow.prototype.drill_MCB_glimmerRectVisible = function() {
		return DrillUp.g_SSpD_selWin_cursor['MCB_rectEnabled'];
	}
	Drill_SSpD_SelectWindow.prototype.drill_MCB_borderEnabled = function() {
		return DrillUp.g_SSpD_selWin_cursor['MCB_enabled'];
	}
	Drill_SSpD_SelectWindow.prototype.drill_MCB_borderStyleId = function() {
		if( DrillUp.g_SSpD_selWin_cursor['MCB_lock'] == true ){
			return DrillUp.g_SSpD_selWin_cursor['MCB_style'];
		}else{
...
if( Imported.Drill_MenuScrollBar == true ){
	Drill_SSpD_SelectWindow.prototype.drill_MSB_scrollBarEnabled = function() {
		return DrillUp.g_SSpD_selWin_cursor['MSB_enabled'];
	}
	Drill_SSpD_SelectWindow.prototype.drill_MSB_scrollBarStyleId = function() {
		if( DrillUp.g_SSpD_selWin_cursor['MSB_lock'] == true ){
			return DrillUp.g_SSpD_selWin_cursor['MSB_style'];
		}else{
...
// * 箭头 - 监听高亮声音
//==============================
var _drill_SSpD_drill_updateArrowHover = Scene_Drill_SSpD.prototype.drill_updateArrowHover;
Scene_Drill_SSpD.prototype.drill_updateArrowHover = function() {
	_drill_SSpD_drill_updateArrowHover.call(this);
	...
	if( this._arrow_hoverType != this._arrow_lastHoverType ){
		...
		// > 播放声音
		SoundManager.drill_SSpD_playHover_Arrow();
	}
}
//==============================
// * 箭头 - 高亮声音
//==============================
SoundManager.drill_SSpD_playHover_Arrow = function() {
	if( DrillUp.g_SSpD_arrowClickDefaultEnabled == true ){
		//（不操作）
	}else{
		if( DrillUp.g_SSpD_arrowHoverSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpD_arrowHoverSound );
		}
	}
}
//==============================
// * 箭头 - 点击声音
//==============================
SoundManager.drill_SSpD_playClick_Arrow = function() {
	if( DrillUp.g_SSpD_arrowClickDefaultEnabled == true ){
		SoundManager.playCursor();
	}else{
		if( DrillUp.g_SSpD_arrowClickSound != null ){
			AudioManager.playStaticSe( DrillUp.g_SSpD_arrowClickSound );
		}
	}
}
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_SSpD_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
