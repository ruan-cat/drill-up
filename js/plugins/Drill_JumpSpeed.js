...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_JSp_PluginTip_curName = "Drill_JumpSpeed.js 物体-跳跃速度";
	DrillUp.g_JSp_PluginTip_baseList = [];
	//==============================
	// * 提示信息 - 报错 - 找不到事件
	//==============================
	DrillUp.drill_JSp_getPluginTip_EventNotFind = function( e_id ){
		return "【" + DrillUp.g_JSp_PluginTip_curName + "】\n插件指令错误，当前地图并不存在id为"+e_id+"的事件。";
	};
...
Game_Map.prototype.drill_JSp_isEventExist = function( e_id ){
	...
	if( e == undefined ){
		alert( DrillUp.drill_JSp_getPluginTip_EventNotFind( e_id ) );
		return false;
	}
	return true;
};
