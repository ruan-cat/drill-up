...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_GOBTN_PluginTip_curName = "Drill_GaugeOfBufferTimeNum.js UI-缓冲时间数字";
	DrillUp.g_GOBTN_PluginTip_baseList = ["Drill_CoreOfGaugeNumber.js 系统-参数数字核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	...
	DrillUp.drill_GOBTN_getPluginTip_NoBasePlugin = function(){
		...
		var message = "【" + DrillUp.g_GOBTN_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_GOBTN_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_GOBTN_PluginTip_baseList[i];
		}
		return message;
	};
	//==============================
	// * 提示信息 - 报错 - 找不到事件
	//==============================
	DrillUp.drill_GOBTN_getPluginTip_EventNotFind = function( e_id ){
		return "【" + DrillUp.g_GOBTN_PluginTip_curName + "】\n插件指令错误，当前地图并不存在id为"+e_id+"的事件。";
	};
...
Game_Map.prototype.drill_GOBTN_isEventExist = function( e_id ){
	...
	if( e == undefined ){
		alert( DrillUp.drill_GOBTN_getPluginTip_EventNotFind( e_id ) );
		return false;
	}
	return true;
};
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_GOBTN_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
