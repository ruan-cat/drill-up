 ...
 * 插件指令：>信息面板I : 打开面板
 *
 * 插件指令：>信息面板I : 显示选项 : 选项[1]
 * 插件指令：>信息面板I : 显示选项 : 选项变量[21]
 * 插件指令：>信息面板I : 隐藏选项 : 选项[1]
 * 插件指令：>信息面板I : 隐藏选项 : 选项变量[21]
 * 插件指令：>信息面板I : 显示全部选项
 * 插件指令：>信息面板I : 隐藏全部选项
 * 
 * 插件指令：>信息面板I : 锁定选项 : 选项[1]
 * 插件指令：>信息面板I : 锁定选项 : 选项变量[21]
 * 插件指令：>信息面板I : 解锁选项 : 选项[1]
 * 插件指令：>信息面板I : 解锁选项 : 选项变量[21]
 * 插件指令：>信息面板I : 锁定全部选项
 * 插件指令：>信息面板I : 解锁全部选项
 *
 ...
 * 
 * 插件指令：>信息面板I : 选中选项 : 选项[1]
 * 插件指令：>信息面板I : 选中选项 : 选项变量[21]
 * 
...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_SSpI_PluginTip_curName = "Drill_SceneSelfplateI.js 面板-全自定义信息面板I";
	DrillUp.g_SSpI_PluginTip_baseList = [
	...
	//==============================
	DrillUp.drill_SSpI_getPluginTip_NoBasePlugin = function(){
		if( DrillUp.g_SSpI_PluginTip_baseList.length == 0 ){ return ""; }
		var message = "【" + DrillUp.g_SSpI_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_SSpI_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_SSpI_PluginTip_baseList[i];
		}
		return message;
	};
...
//##############################
DrillUp.g_SSpI_saveEnabled = true;
//##############################
// * 存储数据 - 初始化
//          
//			说明：	> 下方为固定写法，不要动。
...
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_SSpI_sys_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function( contents ){
	_drill_SSpI_sys_extractSaveContents.call( this, contents );
	
	// > 参数存储 启用时（检查数据）
	if( DrillUp.g_SSpI_saveEnabled == true ){	
		$gameSystem.drill_SSpI_checkSysData();
		
	// > 参数存储 关闭时（直接覆盖）
	}else{
		$gameSystem.drill_SSpI_initSysData();
	}
};
//##############################
// * 存储数据 - 初始化数据【标准函数】
...
//##############################
Game_System.prototype.drill_SSpI_initSysData = function() {
	this.drill_SSpI_initSysData_Private();
};
//##############################
// * 存储数据 - 载入存档时检查数据【标准函数】
//			
...
//##############################
Game_System.prototype.drill_SSpI_checkSysData = function() {
	this.drill_SSpI_checkSysData_Private();
};
//=============================================================================
// ** 存储数据（接口实现）
//=============================================================================
...
// * 存储数据 - 初始化数据（私有）
//==============================
Game_System.prototype.drill_SSpI_initSysData_Private = function() {
	
	this._drill_SSpI_enableTank = [];				//显示情况
	this._drill_SSpI_lockTank = [];					//锁定情况
	for(var i = 0; i < DrillUp.g_SSpI_context_list.length; i++){
		var temp_data = DrillUp.g_SSpI_context_list[i];
		if( temp_data == undefined ){ continue; }
		this._drill_SSpI_enableTank[i] = temp_data['enabled'];
		this._drill_SSpI_lockTank[i] = temp_data['locked'];
	}
};
//==============================
// * 存储数据 - 载入存档时检查数据（私有）
//==============================
Game_System.prototype.drill_SSpI_checkSysData_Private = function() {
	
	// > 旧存档数据自动补充
	if( this._drill_SSpI_enableTank == undefined ){
		this.drill_SSpI_initSysData();
	}
	...
	for( var i = 0; i < DrillUp.g_SSpI_context_list.length; i++ ){
		...
		if( temp_data != undefined ){
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpI_enableTank[i] == undefined ){
				this._drill_SSpI_enableTank[i] = temp_data['enabled'];
			...
			// > 未存储的，重新初始化
			if( this._drill_SSpI_lockTank[i] == undefined ){
				this._drill_SSpI_lockTank[i] = temp_data['locked'];
...
// ** ☆主菜单选项
//
//			说明：	> 此模块专门关联主菜单选项，选项进入后跳转到 信息面板I 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================
...
// ** ☆标题选项
//
//			说明：	> 此模块专门关联标题选项，选项进入后跳转到 信息面板I 界面。
//					（插件完整的功能目录去看看：功能结构树）
//=============================================================================	
...
// * 信息面板I（场景基类） - 开始运行
//==============================
Scene_Drill_SSpI.prototype.start = function() {
    Scene_MenuBase.prototype.start.call(this);
};
//==============================
// * 信息面板I（场景基类） - 结束运行
//==============================
Scene_Drill_SSpI.prototype.stop = function() {
    Scene_MenuBase.prototype.stop.call(this);
};
//==============================
// * 信息面板I（场景基类） - 判断是否激活/启动
//==============================
Scene_Drill_SSpI.prototype.isActive = function() {
	return Scene_MenuBase.prototype.isActive.call(this);
};
//==============================
// * 信息面板I（场景基类） - 析构函数
//==============================
Scene_Drill_SSpI.prototype.terminate = function() {
...
// * 信息面板I（场景基类） - 判断加载完成
//==============================
Scene_Drill_SSpI.prototype.isReady = function() {
	return Scene_MenuBase.prototype.isReady.call(this);
};
//==============================
// * 信息面板I（场景基类） - 忙碌状态
//==============================
Scene_Drill_SSpI.prototype.isBusy = function() {
	return Scene_MenuBase.prototype.isBusy.call(this);
};

//==============================
// * 信息面板I（菜单界面基类） - 创建 - 菜单背景
//==============================
Scene_Drill_SSpI.prototype.createBackground = function() {
	Scene_MenuBase.prototype.createBackground.call(this);
};
//==============================
// * 信息面板I（菜单界面基类） - 创建 - 帮助窗口
//==============================
Scene_Drill_SSpI.prototype.createHelpWindow = function() {
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_SSpI_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
