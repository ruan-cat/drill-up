 ...
 *      如果你想设计碎片粉碎的轨迹，可以去看看"32.数学模型 > 关于弹道.docx"。
...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_LSE_PluginTip_curName = "Drill_LayerShatterEffect.js 地图-方块粉碎效果";
	DrillUp.g_LSE_PluginTip_baseList = ["Drill_CoreOfShatterEffect.js 系统-方块粉碎核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	//			说明：	此函数只提供提示信息，不校验真实的插件关系。
	//==============================
	DrillUp.drill_LSE_getPluginTip_NoBasePlugin = function(){
		if( DrillUp.g_LSE_PluginTip_baseList.length == 0 ){ return ""; }
		var message = "【" + DrillUp.g_LSE_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_LSE_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_LSE_PluginTip_baseList[i];
		}
		return message;
	};
...
//##############################
DrillUp.g_LSE_saveEnabled = true;
//##############################
// * 存储数据 - 初始化
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_LSE_sys_initialize = Game_System.prototype.initialize;
Game_System.prototype.initialize = function() {
    _drill_LSE_sys_initialize.call(this);
	this.drill_LSE_initSysData();
};
//##############################
// * 存储数据 - 载入存档
//          
//			说明：	> 下方为固定写法，不要动。
//##############################
var _drill_LSE_sys_extractSaveContents = DataManager.extractSaveContents;
DataManager.extractSaveContents = function( contents ){
	_drill_LSE_sys_extractSaveContents.call( this, contents );
	
	// > 参数存储 启用时（检查数据）
	if( DrillUp.g_LSE_saveEnabled == true ){	
		$gameSystem.drill_LSE_checkSysData();
		
	// > 参数存储 关闭时（直接覆盖）
	}else{
		$gameSystem.drill_LSE_initSysData();
	}
};
//##############################
// * 存储数据 - 初始化数据【标准函数】
...
//##############################
Game_System.prototype.drill_LSE_initSysData = function() {
	this.drill_LSE_initSysData_Private();
};
//##############################
// * 存储数据 - 载入存档时检查数据【标准函数】
//			
...
//##############################
Game_System.prototype.drill_LSE_checkSysData = function() {
	this.drill_LSE_checkSysData_Private();
};
//=============================================================================
// ** 存储数据（接口实现）
//=============================================================================
...
// * 存储数据 - 初始化数据（私有）
//==============================
Game_System.prototype.drill_LSE_initSysData_Private = function() {
...
// * 存储数据 - 载入存档时检查数据（私有）
//==============================
Game_System.prototype.drill_LSE_checkSysData_Private = function() {
	
	// > 旧存档数据自动补充
	if( this._drill_LSE_opacityType == undefined ){
		this.drill_LSE_initSysData();
	}
...
// * 粉碎贴图 - 初始化
//==============================
var _drill_LSE_temp_initialize = Game_Temp.prototype.initialize;
Game_Temp.prototype.initialize = function() {
	_drill_LSE_temp_initialize.call(this);
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_LSE_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
