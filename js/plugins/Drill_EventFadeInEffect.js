...
//=============================================================================
	...
	//==============================
	var DrillUp = DrillUp || {}; 
	DrillUp.g_EFIE_PluginTip_curName = "Drill_EventFadeInEffect.js 行走图-显现动作效果";
	DrillUp.g_EFIE_PluginTip_baseList = ["Drill_CoreOfEventFrame.js 行走图-行走图优化核心"];
	//==============================
	// * 提示信息 - 报错 - 缺少基础插件
	//			
	//			说明：	此函数只提供提示信息，不校验真实的插件关系。
	//==============================
	DrillUp.drill_EFIE_getPluginTip_NoBasePlugin = function(){
		if( DrillUp.g_EFIE_PluginTip_baseList.length == 0 ){ return ""; }
		var message = "【" + DrillUp.g_EFIE_PluginTip_curName + "】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对：";
		for(var i=0; i < DrillUp.g_EFIE_PluginTip_baseList.length; i++){
			message += "\n- ";
			message += DrillUp.g_EFIE_PluginTip_baseList[i];
		}
		return message;
	};
	//==============================
	// * 提示信息 - 报错 - 找不到事件
	//==============================
	DrillUp.drill_EFIE_getPluginTip_EventNotFind = function( e_id ){
		return "【" + DrillUp.g_EFIE_PluginTip_curName + "】\n插件指令错误，当前地图并不存在id为"+e_id+"的事件。";
	};
	//==============================
	// * 提示信息 - 报错 - NaN校验值
	//==============================
	DrillUp.drill_EFIE_getPluginTip_ParamIsNaN = function( param_name ){
...
Game_Map.prototype.drill_EFIE_isEventExist = function( e_id ){
	...
	if( e == undefined ){
		alert( DrillUp.drill_EFIE_getPluginTip_EventNotFind( e_id ) );
		return false;
	}
	return true;
};
...
//					  旋转+缩放+斜切 可为负数。
//=============================================================================
Game_Temp.prototype.drill_EFIE_Math2D_getFixPointInAnchor = function( 
...
//=============================================================================
Game_Temp.prototype.drill_EFIE_Math2D_getParabolicThree = function( x1,y1,x2,y2,x3,y3 ){
...
Game_Character.prototype.drill_EFIE_setOpacity = function( opacity ){
	if( isNaN(opacity) ){
		alert( DrillUp.drill_EFIE_getPluginTip_ParamIsNaN( "opacity" ) );
	}
...
Game_Character.prototype.drill_EFIE_updateShowingFall = function() {
	...
	if( ef.fA_time < ef.fA_dtime ){
		...
		// > 锚点(0.5,1.0)锁定
		var fix_point = $gameTemp.drill_EFIE_Math2D_getFixPointInAnchor( ef.anchor_x,ef.anchor_y, 0.5,1.0, ef.real_width,ef.real_height, ef.rotation, ef.scale_x+1, ef.scale_y+1 );
		ef.x = fix_point.x;
		ef.y = fix_point.y;
...
Game_Character.prototype.drill_EFIE_playShowingEnlargeSpring = function( dtime, overflow_scale, anchor_x, anchor_y, opacity_off ){
	...
	ef.f_abc = $gameTemp.drill_EFIE_Math2D_getParabolicThree( 0,-1, dtime*0.8,overflow_scale, dtime,0 );
...
Game_Character.prototype.drill_EFIE_updateShowingEnlargeSpring = function() {
	...
	if( ef.f_time < ef.f_dTime ){
		...
		// > 锚点锁定
		var fix_point = $gameTemp.drill_EFIE_Math2D_getFixPointInAnchor( ef.anchor_x,ef.anchor_y, ef.f_anchor_x, ef.f_anchor_y, ef.real_width,ef.real_height, ef.rotation, ef.scale_x+1, ef.scale_y+1 );
		ef.x = fix_point.x;
		ef.y = fix_point.y;
...
//=============================================================================
		...
		var pluginTip = DrillUp.drill_EFIE_getPluginTip_NoBasePlugin();
		alert( pluginTip );
}
