...
//			来源：	继承于Window
//			实例：	无（父类，一般被继承使用）
//			应用：	作用于该脚本文件的全部子类。其他插件也可继承此类或直接实例化。
//			
//			作用域：	地图界面、战斗界面、菜单界面
//			主功能：	> 定义窗口的默认属性
//						> 定义一个文本域
...
//							> 透明
//			
//			说明：	> 窗口被迫硬性嵌入了字符串绘制、转义功能。
//					  如果你想绘制的可转义的字符串，那么你必须建一个Window_Base，而不是Sprite。
//			禁忌：	> 窗口宽度高度不能随意修改，尤其是在绘制了文本的窗口中。（见createContents函数）
//			
//=============================================================================
//==============================
// * 窗口基类 - 定义
...
Window_Base.prototype.initialize = function( x, y, width, height ){
	...
	// > 初始执行函数
    this.loadWindowskin();				//D窗口皮肤 - 初始化（必须放最前面）
    this.move(x, y, width, height);		//移动到指定位置
    this.updatePadding();				//A默认属性 - 设置内边距（非帧刷新）
    this.updateBackOpacity();			//A默认属性 - 设置背景透明度（非帧刷新）
    this.updateTone();					//A默认属性 - 初始确定窗口色调
	this.createContents();				//B文本域 - 建立画布
										//C开关动画（无）
										//E绘制（无）
										//F背景类型（无）
	...
	// > 私有属性初始化
										//A默认属性（无）
										//B文本域（无）
    this._opening = false;				//C开关动画 - 打开中 标记
    this._closing = false;				//C开关动画 - 关闭中 标记
										//D窗口皮肤（无）
										//E绘制（无）
    this._dimmerSprite = null;			//F背景类型 - 暗淡背景
};
//==============================
// * 窗口基类 - 帧刷新
//==============================
Window_Base.prototype.update = function(){
    ...
    this.updateTone();					//帧刷新 - A默认属性 - 色调
										//帧刷新 - B文本域（无）
    this.updateOpen();					//帧刷新 - C开关动画 - 执行打开动画
    this.updateClose();					//帧刷新 - C开关动画 - 执行关闭动画
										//帧刷新 - D窗口皮肤（无）
										//帧刷新 - E绘制（无）
    this.updateBackgroundDimmer();		//帧刷新 - F背景类型 - 暗淡背景
};
...
// * A默认属性 - 属性（可继承）
//==============================
Window_Base.prototype.lineHeight = function(){ return 36; };			//行高
Window_Base.prototype.standardFontSize = function(){ return 28; };		//字体大小
Window_Base.prototype.standardPadding = function(){ return 18; };		//内边距 - 窗口外框
Window_Base.prototype.textPadding = function(){ return 6; };			//内边距 - 文字外框
Window_Base.prototype.standardBackOpacity = function(){ return 192; };	//背景透明度
//==============================
// * A默认属性 - 默认字体类型
//
//			来源：	来自于object中"Game_System.prototype.isJapanese"的判断函数。
//			应用：	无
//			功能：	> 该函数来自于数据库中的语言配置，实际上这些函数并没有明显的实际效果。
//					> 功能比较鸡肋，可有可无。
//==============================
...
// * A默认属性 - 设置背景透明度（非帧刷新）
//==============================
Window_Base.prototype.updateBackOpacity = function(){ this.backOpacity = this.standardBackOpacity(); };
//==============================
// * A默认属性 - 色调帧刷新
//==============================
...
// * B文本域 - 建立画布
//
//			说明：	> 只要【窗口宽度】发生变化，那么bitmap就需要同步重建。
//				  	> 重建之后，之前在bitmap中绘制的文本会被清空。
//					> 也有很多插件使用 this.contents.clear() 来清理画布，但继承时不要过多考虑这一项特殊情况。
//					  因为调用此方法的函数，都是要静默清空画布，而非重建画布。
//==============================
...
// * B文本域 - 画布的绝对坐标
//
//			说明：	> 注意，因为受到内边距的影响，所以contents画布所在的绝对坐标如下：
//						this.x + this.standardPadding()
//						this.y + this.standardPadding()
//					  如果你要获取子类的 Window_Selectable 的矩形的绝对坐标，要注意。（可见插件Drill_MenuCursor）
//==============================
//（无）
//==============================
// * B文本域 - 获取绝对坐标X（依次累加父类位置）
//==============================
...
// * C开关动画 - 访问器
//==============================
//Window.prototype.isOpen = function(){ return this._openness >= 255; };	//是否已打开（来自父类）
//Window.prototype.isClosed = function(){ return this._openness <= 0; };	//是否已关闭（来自父类）
Window_Base.prototype.isOpening = function(){ return this._opening; };		//是否正在打开
Window_Base.prototype.isClosing = function(){ return this._closing; };		//是否正在关闭
//==============================
// * C开关动画 - 帧刷新 执行打开动画
//==============================
...
// * D窗口皮肤 - 初始化
//
//			说明：	> 此处只有颜色的处理函数，其它对窗口皮肤的读取，见父类Window。
//==============================
...
//			
//			说明：	> 宽度计算基于h5默认提供的 measureText 方法，字符宽度受 字体大小/字体类型 影响。
//					> 注意是基本文本的宽度，特殊字符（比如图标\i[1]）的宽度无法识别，会被作为普通文本进行计算。
//==============================
...
// * E绘制『窗口辅助核心』 - 绘制【基本文本】
//			
//			说明：	> 扩展文本经过多次转义，最终还是会调用到基本文本的函数。
//					> 该函数提供一串字符串的绘制，所以具有 左对齐/居中/右对齐 的选项。
//					> 扩展文本是一个字一个字的绘制的（见processCharacter函数），所以无法设置 左对齐/居中/右对齐。
//					  （要实现Ex文本对齐方式，可见插件 窗口字符核心Drill_CoreOfWindowCharacter 的内容。）
//==============================
...
// * E绘制『窗口辅助核心』 - 绘制【扩展文本】
//			
//			说明：	> 该方法是一套固定流程。一个字一个字地进行识别/转义/绘制。
//					> textState 相当于文本域的光标。
//					  指定xy坐标后，开始塞入并绘制每个具体字符。
//					> 该函数返回绘制后的长度。也就是说，如果要获得Ex文本长度，必须先绘制一遍才能得到。
//==============================
...
// * E绘制 - 绘制脸图 - 执行绘制
//				
//			说明：	> 在contents画板中的指定xy位置，绘制脸图。可控制脸图大小。
//					> 越界绘制也不会报错，只是没有效果。
//==============================
...
// * E绘制 - 绘制参数条 - 执行绘制
//				
//			说明：	> 在contents画板中的指定xy位置，绘制rmmv简单长方形参数条。
//					> 越界绘制也不会报错，只是没有效果。
//==============================
...
Window_Base.prototype.showBackgroundDimmer = function(){
    ...
    if( bitmap.width !== this.width || bitmap.height !== this.height ){
        this.refreshDimmerBitmap();		//（窗口大小变化时，暗淡背景变化）
    }
...
//							->执行 下一页
//			
//			说明：	> 该类中，H滚动位置 和 I选择 是两个模块，
//					  选择一定会改变滚动位置，但滚动位置不一定会改变选择。
//=============================================================================
//==============================
// * 选项窗口基类 - 定义
//==============================
...
Window_Selectable.prototype.initialize = function( x, y, width, height ){
    ...
    this._index = -1;					//I选择 - 当前选择
	
    this._cursorFixed = false;			//J指针 - 闪烁白矩形 - 固定指针
    this._cursorAll = false;			//J指针 - 闪烁白矩形 - 选择全部
	
    this._helpWindow = null;			//K子窗口 - 帮助窗口
	
    this._handlers = {};				//N输入触发 - 函数集
	
    this._stayCount = 0;				//L鼠标与触屏 - 反应延迟
    this._touching = false;				//L鼠标与触屏 - 反应延迟
	
    this._scrollX = 0;					//H滚动位置 - 滚动位置X（实际没用到）
    this._scrollY = 0;					//H滚动位置 - 滚动位置Y
...
Window_Selectable.prototype.update = function(){
    ...
    this.updateArrows();				//帧刷新 - H滚动位置 - 上下箭头 可见
	
    this.processCursorMove();			//帧刷新 - M键盘与手柄 - 选项指针
    this.processHandling();				//帧刷新 - M键盘与手柄 - 输入触发
	
    this.processWheel();				//帧刷新 - L鼠标与触屏 - 滚轮移动
    this.processTouch();				//帧刷新 - L鼠标与触屏 - 输入触发
    this._stayCount++;					//帧刷新 - L鼠标与触屏 - 反应延迟
};
//==============================
// * 选项窗口基类 - 激活
//==============================
Window_Selectable.prototype.activate = function(){
    ...
    this.reselect();		//（重新选择，确保窗口激活改变后看到当前选项，如果index为-1，则 滚动位置置顶。）
};
//==============================
// * 选项窗口基类 - 取消激活
//==============================
...
// * G子项 - 属性（可继承）
//==============================
Window_Selectable.prototype.maxCols = function(){ return 1; };		//列数
Window_Selectable.prototype.maxItems = function(){ return 0; };		//子项数量
Window_Selectable.prototype.spacing = function(){ return 12; };		//子项间距
//==============================
// * G子项 - 计算后的属性 - 子项宽度（与列数相关）
//==============================
...
// * H滚动位置 - 属性
//
//			说明：	> 滚动位置包含 页 的定义，能够影响当前 子项 矩形位置的绘制。
//					> 注意，I选择 基于 H滚动位置 的定义。
//					  选择一项后会 滚动到选择位置，而滚动不会改变当前选择。
//==============================
//	见 this._scrollY 
//==============================
// * H滚动位置 - 计算后的属性 - 当前页的首行
...
// * I选择 - 属性
//
//			说明：	> 注意，I选择 基于 H滚动位置 的定义。
//					  选择一项后会 滚动到选择位置，而滚动不会改变当前选择。
//==============================
Window_Selectable.prototype.index = function(){ return this._index; };		//当前选择
//==============================
// * I选择 - 计算后的属性 - 当前行数
//
//			说明：	> 如果index为-1，则返回-1。
...
// * I选择 - 选择一项
//
//			说明：	> 注意，选择 与 滚动位置 是两个不同的属性。
//==============================
Window_Selectable.prototype.select = function( index ){
    this._index = index;
	
    this._stayCount = 0;			//L鼠标与触屏 - 反应延迟置零
	
    this.ensureCursorVisible();		//H滚动位置 - 滚动到选择位置
	
    this.updateCursor();			//J指针 - 闪烁白矩形 - 刷新选项
	
    this.callUpdateHelp();			//K子窗口 - 帮助窗口 - 刷新触发
};
//==============================
// * I选择 - 取消选择
//==============================
Window_Selectable.prototype.deselect = function(){ this.select(-1); };
//==============================
// * I选择 - 重新选择
//==============================
...
// * J指针 - 闪烁白矩形 - 固定指针
//==============================
Window_Selectable.prototype.cursorFixed = function(){ return this._cursorFixed; };
Window_Selectable.prototype.setCursorFixed = function( cursorFixed ){ this._cursorFixed = cursorFixed; };
//==============================
// * J指针 - 闪烁白矩形 - 选择全部
//==============================
Window_Selectable.prototype.cursorAll = function(){ return this._cursorAll; };
Window_Selectable.prototype.setCursorAll = function( cursorAll ){ this._cursorAll = cursorAll; };
//==============================
// * J指针 - 闪烁白矩形 - 刷新选项（非帧刷新）
//==============================
...
// * J指针 - 判断 - 指针是否可移动
//
//			说明：	> 此设置只对 L鼠标与触屏 和 M键盘与手柄 有效，对脚本控制的 下移/上移/右移/左移 无效。
//==============================
...
// * N输入触发 - 执行 确定键 - 音效
//==============================
Window_Selectable.prototype.playOkSound = function(){ SoundManager.playOk(); };
Window_Selectable.prototype.playBuzzerSound = function(){ SoundManager.playBuzzer(); };
//==============================
// * N输入触发 - 执行 取消键
//==============================
...
Window_Selectable.prototype.updateInputData = function(){
    Input.update();					//（键盘/手柄 帧刷新）
    TouchInput.update();    		//（鼠标/触屏 帧刷新）
};
