...
Game_Interpreter.prototype.character = function (param) {
	...
	if ($gameParty.inBattle()) {
		...
		//返回 游戏地图 事件( 如果 参数>0 返回 参数 否则 返回 事件id )
...
Game_Interpreter.prototype.operateValue = function (operation, operandType, operand) {
	//值  = 如果 操作种类 === 0 返回 操作数 否则 返回 游戏变量组 值(操作数)
	var value = operandType === 0 ? operand : $gameVariables.value(operand);
	//返回 如果 运算 === 0 返回 值 否则 返回 -值
...
Game_Interpreter.prototype.command201 = function () {
	...
	if (!$gameParty.inBattle() && !$gameMessage.isBusy()) {
		...
		if (this._params[0] === 0) {
			...
			// Designation with variables 变量指定
			//地图id = 游戏变量组 值( 参数组[1])
...
Game_Interpreter.prototype.command203 = function () {
	...
	if (character) {
		...
		if (this._params[1] === 0) {
			...
			// Exchange with another event 和其他事件交换
			//人物2  = 人物( 参数组[2] )
...
Game_Interpreter.prototype.command282 = function () {
	...
	//循环 (开始时 i = 0 ; i < 图块组 图块组名称组 长度 ;每一次 i++)
	for (var i = 0; i < tileset.tilesetNames.length; i++) {
		//图像管理器 读取图块组(图块组 图块组名称组[i])
...
Game_Interpreter.prototype.command301 = function () {
	...
	if (!$gameParty.inBattle()) {
		...
		if (this._params[0] === 0) {
			...
			// Designation with a variable 变量指定
			//敌群id = 游戏变量组 值(参数组[1])
			troopId = $gameVariables.value(this._params[1]);
			//否则
		} else {
			// Same as Random Encounter 随机遭遇
			//敌群id = 游戏游戏者 制作遭遇敌群id()
