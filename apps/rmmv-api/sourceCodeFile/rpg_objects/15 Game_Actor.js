...
Game_Actor.prototype.isEquipChangeOk = function (slotId) {
  //返回 ( 不是 是装备种类锁定( 装备槽 槽id )  并且 不是 是装备种类封印 ( 装备槽 槽id )     )
  return (
...
Game_Actor.prototype.changeEquip = function (slotId, item) {
  //如果 交物品和队伍 物品 装备组[槽id]) 并且 (不是 物品 或者 装备槽[槽id] === 物品 装备种类id)
  if (
...
Game_Actor.prototype.attackAnimationId1 = function () {
  ...
  if (this.hasNoWeapons()) {
    ...
    //返回 如果 武器组[0] 返回 武器组[0] 动画id 否则 返回 0
...
Game_Actor.prototype.attackAnimationId2 = function () {
  //武器组 = 武器组()
  var weapons = this.weapons();
  //返回 如果 武器组[1] 返回 武器组[1] 动画id 否则 返回 0
...
Game_Actor.prototype.finalExpRate = function () {
  //返回 经验值比例 * ( 如果 是战斗成员() 返回 1 否则 返回 保留成员的经验值比例() )
  return this.exr * (this.isBattleMember() ? 1 : this.benchMembersExpRate());
};
//保留成员的经验值比例
Game_Actor.prototype.benchMembersExpRate = function () {
  //返回 如果 数据系统 选择额外经验值 返回 1 否则 返回 0
...
Game_Actor.prototype.performAttack = function () {
  //武器组 = 武器组()
  var weapons = this.weapons();
  //武器种类id = 如果 武器组[0] 返回 武器组[0] 武器种类id 否则 返回 0
...
Game_Actor.prototype.maxFloorDamage = function () {
  //返回  如果 数据系统 选择地面死亡 返回 hp 否则 返回 数学 最大值(hp-1,0)
