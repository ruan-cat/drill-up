 ...
 * @class SceneManager
 * @classdesc 场景管理类
 * @tutorial use-jsdoc-to-plugin-js
 *
 * @mermaid
 *  graph TD;
 *    A-->B;
...
/**
 * @description 堆 = []。_stack变量的本质是一个数组。
 */
SceneManager._stack = [];
//停止的
...
SceneManager.changeScene = function () {
	...
	if (this.isSceneChanging() && !this.isCurrentSceneBusy()) {
		//如果 场景 (场景 存在)
		if (this._scene) {
			//场景 结束 //猜想：真正使得场景退出到其他场景的是 terminate 方法。感觉不对。
		...
		if (this._scene) {
			//场景 创建//换句话说，想要调取当前的场景，恐怕要用SceneManager._scene方法来调用。
			this._scene.create();
			//下一个场景 = null
...
/**
 * @param {*} sceneClass 场景类类名。
 * @description
 * 每个场景类必须写清楚其constructor，原因在于此方法调用此写法。
 *
 * push方法会默认调用goto方法，而且一定会创建一个具体的场景类对象。
 */
SceneManager.push = function (sceneClass) {
	//堆 添加 (场景 创造者) //含义是调用其 构造方法。
