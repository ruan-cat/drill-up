 ...
 * 初始化一个位图对象
 * @param {*} width 设置位图的宽度
 * @param {*} height 设置位图的高度
 * @description
 * 任何一个bitmap，其本质都是一个独立的canvas标签。
 *
 * 位图形参实际设置的是canvas画布的大小。
 */
Bitmap.prototype.initialize = function (width, height) {
	//只要是位图，就一定是一个单独的canvas标签。
	this._canvas = document.createElement("canvas");
	//_context就是可以执行canvas方法的对象。
 ...
 * @description
 * 发现一个特点。fillRect方法可以重复地，反复地对同一个bitmap对象进行内容的修改，
 * 所产生的图像是覆盖式的。
 */
//填充矩形
...
Bitmap.prototype._makeFontNameText = function () {
	return (this.fontBold ? "bold " : "") + (this.fontItalic ? "Italic " : "") + this.fontSize + "px " + this.fontFace;
};
