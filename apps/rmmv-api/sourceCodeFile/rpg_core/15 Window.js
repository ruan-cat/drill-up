 ...
 * @type Number
 * @description
 * opacity变量的本质是 _windowSpriteContainer.alpha ， 窗口精灵容器的透明度。
 */
//定义属性 
 ...
 * @type Number
 * @description
 * 这个变量的本质是 this._windowContentsSprite.alpha ，是一个窗口内容精灵的透明度。
 * 
 * 这个变量在mv中，只被Window_MapName类所使用。
 */
 ...
 * 
 * @description
 * 猜想、设置一个窗口色调的本质是调用 _refreshBack 方法修改位图色调。
 * 
 * 这个方法仅仅是用来装载数据。真正更改窗口的颜色 还是要取决于 _refreshBack 方法。
 */
 ...
 * @private
 * @description
 * 窗口精灵容器_windowSpriteContainer包括了：_windowBackSprite和_windowFrameSprite，
 * 即窗口背景精灵和窗口框精灵。
 * 
 * 每一个窗口的this.children数组，就是本方法中按次序执行的六个addChild方法。
 */
 ...
 * @private
 * @description 
 * 这个方法会重新移动背景精灵的位置。换句话说，背景精灵的位置永远是错位的。
 */
Window.prototype._refreshBack = function() {
    //这个精灵与其内容稍微设置了一点边缘。
    var m = this._margin; //m = 4
    var w = this._width - m * 2; 
    var h = this._height - m * 2;
    var bitmap = new Bitmap(w, h);

    this._windowBackSprite.bitmap = bitmap;
    //为什么要写这个代码？设置这个方形的大小，位置。
