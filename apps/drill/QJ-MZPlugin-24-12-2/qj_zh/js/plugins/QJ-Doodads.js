...
(($) => {
	...
	DataManager.checkDoodadsDataQJ = function () {
		//translate VS to QJ
		if (typeof $dataDoodadsQJ.length == "number") {
			//isArray
			...
			for (let i = 1, detail, il = $dataDoodadsQJ.length, mapDoodadsData, mapTilesetData; i < il; i++) {
				...
				mapTilesetData = [];
				for (
					let tilesetsName = tilesetsData[DataManager.loadMapDataSynDoodadsQJ(0, i).tilesetId].tilesetNames,
						_i1 = tilesetsName.length - 1;
					_i1 >= tilesetsName.length - 4;
					_i1--
				) {
		...
		if (!DataManager.DoodadsQJPreLoad) {
			...
			for (let i in $dataDoodadsQJ) {
				for (let j = 0, jData = $dataDoodadsQJ[i], jl = jData.length, detail; j < jl; j++) {
					...
					if (detail.bit) {
						if (detail.bit[0] == 0) {
							preLoadData[detail.bit[1] + detail.bit[2]] = ImageManager.loadBitmap(
								folderName + detail.bit[1],
								detail.bit[2],
							);
	...
	Sprite_DoodadQJ.prototype.setAllDoodadFilter = function () {
		...
		if (
			...
			if (
				this.blendColor &&
				(this.blendColor[0] > 0 || this.blendColor[1] > 0 || this.blendColor[2] > 0 || this.blendColor[3] > 0)
			) {
		...
		if (this.data.contrast > 0 || this.data.sepia || this.data.bright) {
			...
			if (this.data.contrast > 0) {
				colorFilter.contrast(0.01 * this.data.contrast, firstColorMax);
				firstColorMax = true;
			}
			if (this.data.sepia) {
				colorFilter.sepia(firstColorMax);
				firstColorMax = true;
			}
			if (this.data.bright) {
				colorFilter._loadMatrix([2, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0, 0, 2, 0, 0, 0, 0, 0, 1, 0], firstColorMax);
				firstColorMax = true;
			}
			filtersList.push(colorFilter);
		}
	...
	Sprite_DoodadQJ.prototype.drawOnOtherBitmap = function (bitmap, x, y, w, h) {
		...
		bitmap.bltDoodadsQJ(
			...
			frame.width,
			frame.height,
			(w - frame.width * scaleSize) / 2 + x,
			(h - frame.height * scaleSize) / 2 + y,
			frame.width * scaleSize,
			frame.height * scaleSize,
		);
	};
	...
	if (Imported.Blizzard_UltraMode7) {
		...
		Sprite_DoodadQJ.prototype.updateVisible = function () {
			...
			if (blendColor) {
				if (
					!this.blendColor ||
					this.blendColor[0] != blendColor[0] ||
					this.blendColor[1] != blendColor[1] ||
					this.blendColor[2] != blendColor[2] ||
					this.blendColor[3] != blendColor[3]
				) {
		...
		Sprite_DoodadQJ.prototype.screenBlendColor = function () {
			...
			const result = [
				Math.round(fadeColor[0] * 255),
				Math.round(fadeColor[1] * 255),
				Math.round(fadeColor[2] * 255),
				0,
			];
			if (z >= fadeEnd) {
				result[3] = 255;
				...
				result[3] = Math.round(
					((((z - fadeBegin) / (fadeEnd - fadeBegin)) * (z - fadeBegin)) / (fadeEnd - fadeBegin)) * 255,
				);
			}
			return result;
		};
	...
	if (editorMode) {
		...
		QJ.DD.box = function (x, y, w, h) {
			let body = new SATPolygon(new SATVector(x, y), [
		...
		Scene_Map.prototype.updateScene = function () {
			...
			if ($gameSystem.doodadsQJ.editing) {
				...
				} else if (this._doodadsWindowIndex == 3) {
					//put
		...
		Scene_Map.prototype.deleteSelectedDoodadsQJ = function () {
			...
			for (
				let j = 0, jData = $gameSystem.doodadsQJ.selecting, jl = jData.length, i, idata = QJ.DD.doodads();
				j < jl;
				j++
			) {
		...
		Scene_Map.prototype.updateSelectDoodads = function () {
			...
			if ($gameSystem.doodadsQJ.rectSelect[4]) {
				...
				if (!TouchInput.isPressed()) {
					...
					data[4] = false;
					let box =
						Math.abs(data[0] - data[2]) <= 2 && Math.abs(data[1] - data[3]) <= 2
							? null
							: QJ.DD.box(
									(data[0] + data[2]) / 2,
									(data[1] + data[3]) / 2,
									Math.abs(data[0] - data[2]),
									Math.abs(data[1] - data[3]),
								);
					let sprites = this._spriteset._doodadsQJ,
						clickList = [];
				...
				if (TouchInput.isPressed()) {
					...
					if ($gameSystem.doodadsQJ.selecting && $gameSystem.doodadsQJ.selecting.length > 0) {
						if (
							(this._editDoodadsWindowQJ.animType == 1 &&
								mouseX <= this._editDoodadsWindowQJ.screenWidth - this._editDoodadsWindowQJ.width) ||
		...
		Scene_Map.prototype.updatePutNewDoodadsQJ = function () {
			...
			if (Input.isPressed("shift")) {
				...
				if (TouchInput.isClicked()) {
					if (
						(this._editDoodadsWindowQJ.visible && mouseX < Graphics.width - this._editDoodadsWindowQJ.width) ||
		...
		Scene_Map.prototype.updateSelectDoodadsRect = function () {
			...
			if (data[4]) {
				...
				if (data[5]) y -= dy * 48;
				let w = Math.abs(data[0] - data[2]),
					h = Math.abs(data[1] - data[3]);
		...
		Scene_Map.prototype.drawHelpWindow = function (id) {
			...
			if (typeof id == "number") {
				if (id == 0) {
					...
					id = [
						"Click the doodads to select.  Press and release to select doodads in certain rectangle region.",
					...
					id = [
						"Select img and add new doodads.",
					...
					id = ["Set the grid snap.", "Press G to show/hide grid snap.", "Press alt to adjust the doodad to grid."];
				} else if (id == 5) {
					id = ["Find the doodad."];
				} else if (id == 6) {
					id = [
						"Do you want to close doodads editing window without saving?",
		...
		Scene_Map.prototype.createDoodadsMainWindow = function () {
			...
			this._mainDoodadsWindowQJ.select = function (index) {
				...
				if (this.active && SceneManager._scene && SceneManager._scene.refreshDoodadsMainWindowHelpQJ)
				...
				list = list.concat([
					{
						...
						ext: {
							...
							fixValue: (value) => {
								value = Number(value);
								return isNaN(Number(value)) || !value ? (itemData.x ? itemData.x : 0) : value;
							},
							left: (value) => {
			...
			list = list.concat([
				{
					...
					ext: {
						...
						fixValue: (value) => {
							value = Number(value);
							return value >= 0 ? Math.floor(value * 1000) / 1000 : 3;
						},
						left: (value) => {
							return Number(value) - 1;
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.rotation,
						inputType: 1,
						attribute: ["rotation"],
						fixValue: (value) => {
							value = Number(value);
							return value < 0 || value >= 360
								? Math.floor((value % 360) * 1000) / 1000
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.rotateSpeed,
						inputType: 1,
						attribute: ["rotateSpeed"],
						fixValue: (value) => {
							value = Number(value);
							return value < 0 || value >= 360
								? Math.floor((value % 360) * 1000) / 1000
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.anchorX,
						inputType: 1,
						attribute: ["anchorX"],
						fixValue: (value) => {
							value = Number(value);
							return isNaN(Number(value)) || !value ? 1 : Math.floor(value * 1000) / 1000;
						},
						left: (value) => {
							return Math.floor((Number(value) - 0.01) * 1000) / 1000;
						},
						right: (value) => {
							return Math.floor((Number(value) + 0.01) * 1000) / 1000;
						},
				...
				},
				{
					...
					ext: {
						...
						fixValue: (value) => {
							value = Number(value);
							return value > 255 ? 255 : value < -255 ? -255 : Math.floor(value);
						},
						left: (value) => {
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.frameUpdate,
						inputType: 1,
						attribute: ["frameUpdate"],
						fixValue: (value) => {
							value = Number(value);
				...
				},
				{
					...
					ext: {
						...
						help: [
							"The switch that control the visiblility of doodads.",
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.condition,
						inputType: 1,
						attribute: ["condition"],
						fixValue: (value) => {
							return value;
						},
						help: ["The js script.", "e.g: $gameVariables.value(5)>10 "],
					},
					handler: null,
				},
			]);
			list = list.concat([
				{
					...
					enable: true,
					ext: {
						content: itemData.blur,
						inputType: 1,
						attribute: ["blur"],
						fixValue: (value) => {
							value = Number(value);
							return value >= 0 && value <= 200 ? Math.floor(value) : 0;
						},
						left: (value) => {
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.contrast,
						inputType: 1,
						attribute: ["contrast"],
						fixValue: (value) => {
							value = Number(value);
							return value >= 0 && value <= 100 ? Math.floor(value) : 0;
						},
						left: (value) => {
							return Number(value) - 10;
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.sepia ? "true" : "false",
						inputType: 3,
						attribute: ["sepia"],
						fixValue: (value) => {
							return !eval(value);
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.hue,
						inputType: 1,
						attribute: ["hue"],
						fixValue: (value) => {
							value = Number(value);
							return value >= 0 && value <= 360 ? Math.floor(value) : 0;
						},
						left: (value) => {
							return Number(value) - 15;
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: itemData.bright ? "true" : "false",
						inputType: 3,
						attribute: ["bright"],
						fixValue: (value) => {
							return !eval(value);
		...
		Scene_Map.prototype.refreshSelectDoodadsImgWindowQJ = function (name) {
			...
			} else if (win.folderData.length - 1 > win.folderLayer) {
				//subtract
		...
		Scene_Map.prototype.refreshDoodadsGridSnap = function () {
			...
			let regionId,
				color,
				getRegion = $gameMap.regionId.bind($gameMap);
		...
		Window_DoodadsGridSnapQJ.prototype.cursorRight = function () {
			...
			if (index == 0) $gameSystem.doodadsQJ.grid = !$gameSystem.doodadsQJ.grid;
				...
				$gameSystem.doodadsQJ.gridX = Math.min(Math.floor(Graphics.width / 2), $gameSystem.doodadsQJ.gridX + 1);
			else if (index == 2)
				$gameSystem.doodadsQJ.gridY = Math.min(Math.floor(Graphics.height / 2), $gameSystem.doodadsQJ.gridY + 1);
		...
		Window_DoodadsTilesetSelectQJ.prototype.initialize = function () {
			...
			Window_Command.prototype.initialize.call(
				this,
				new Rectangle(Math.floor((Graphics.boxWidth - width) / 2), 0, width, Graphics.height - 8),
			);
			this.hide();
		...
		Window_DoodadsTilesetSelectQJ.prototype.processOk = function () {
			...
			if (!this.rangeSelect) {
				...
				this.rangeSelect = [
					extraData.tilesetName,
					(extraData.index % 16) * 48,
					Math.floor(extraData.index / 16) * 48,
					48,
					48,
				];
			}
		...
		Window_DoodadsSelectQJ.prototype.initialize = function (data) {
			...
			Window_Selectable.prototype.initialize.call(
				this,
				new Rectangle(0, 0, data.width ? data.width : 280, this.calculateHeight(data.list ? data.list.length : 0)),
			);
			this._list = [];
			this.select(0);
			this.visible = false;
    ...
    `;
			...
			if (hide || !this.inputtingStatus || !this._list || this.index() < 0 || this.animStatus != 0) {
				...
				if (ext.inputType == 0) {
					...
					if (ext.inputType == 1) {
						this.setElement(
							this.inputElement,
							this.x + rect.x + rect.width + 22,
							this.y + rect.y + 22 + (-this.scrollY() % this.itemHeight()),
						);
						if (this.editingData) {
		...
		Window_DoodadsEditQJ.prototype.drawItem = function (index) {
			...
			this.changePaintOpacity(this.isCommandEnabled(index));
			if (typeof ext.content == "number" || (typeof ext.content == "string" && ext.content.length > 0)) {
		...
		Window_DoodadsImgSelectQJ.prototype.drawDoodadsImg = function (img, x, y, w, h) {
			...
			this.contents.blt(img, 0, 0, bw, bh, x + (w - bw * scale) / 2, y + (h - bh * scale) / 2, bw * scale, bh * scale);
		};
		//==========================================================
		//
		//==========================================================
