...
(($) => {
	...
	//
	//=============================================================================
	QJ.MPMZ.text1 = ["The projectile attribute`s value error."];
	QJ.MPMZ.text2 = ["子弹的属性值错误."];
	...
	//=============================================
	QJ.createTextureLineForTrailEffect = function (
		bit,
		name,
		container,
		posStartX,
	...
	Game_QJBulletMZ.prototype.calculateData = function () {
		...
		//==========================================
		this.needAccuracy =
			this.data.judgeAccuracyMove > 0 ||
			this.data.judgeAccuracyRotation > 0 ||
			this.data.judgeAccuracyScale > 0 ||
			this.data.judgeAccuracyAnchor > 0;
		//==========================================
		this.needRemData =
			(this.data.extra && this.data.extra.needRemData) ||
			this.data.afterImage ||
			(this.data.particles && this.data.particles.length > 0) ||
			this.needAccuracy ||
			(this.data.trailEffect && this.data.trailEffect.length > 0);
		this.dataRem = [];
		//==========================================
		this.needAdvancedData =
			this.data.moveJS.length > 0 ||
			this.data.moveCE.length > 0 ||
			this.data.moveF.length > 0 ||
			this.data.deadJS.length > 0 ||
			this.data.deadCE.length > 0 ||
			this.data.deadF.length > 0;
		//==========================================
...
Object.defineProperties(Game_QJBulletMZ.prototype, {
	...
	Game_QJBulletMZ.prototype.refreshBox = function (justMakeData) {
		...
		if (body[0] == "C") {
			...
			newSATBody = QJ.SAT.box(
				this.x,
				this.y,
				["R", body[1] * this.scaleX, body[2] * this.scaleY],
	...
	Game_QJBulletMZ.prototype.updateAdvancedExistData = function () {
		if (this.needAccuracy && this.dataRem.length > 1) {
			...
			if (stepNum > 1) {
				...
				for (let m = 0, ml = stepNum - 1; m < ml; m++) {
					//从下一步开始到最后一步之前的一步
					this.x += dx;
					this.y += dy;
					this.rotationImg += dr;
		...
		switch (moveType[0]) {
			...
			case "TB": {
				...
				if (target) {
					...
					//=========================================================
					let standardTargetRo = math.angleStandard_MPMZ(
						QJ.calculateAngleByTwoPointAngle(this.x, this.y, targetX, targetY) +
					...
					if (judgeTargetRo - judgeLastRo >= 180) {
						//逆时针
			...
			case "B": {
				...
				if (moveType[10]) {
					this.rotationMove = QJ.MPMZ.model[0].initialRotationExtra(
						QJ.makeDeepCopy(data.initialRotationRem),
						character,
						[cx, cy],
					);
				}
			...
			case "C": {
				...
				if (moveType.length <= 5) {
					...
					moveType[2] = Array.isArray(moveType[2])
						? moveType[2].map((a) => new DEFrameQJ(null, a, 0))
						: [new DEFrameQJ(null, moveType[2], 0)];
					moveType[3] = new DEFrameQJ(null, moveType[3], 0);
				...
				if (character) {
					...
					if (moveType[4] && noOriginData) {
						let tarX =
							character._realX * tileSize + tileSize / 2 + moveType[5].radiusA * Math.sin(moveType[5].rotation);
						let tarY =
							character._realY * tileSize + tileSize / 2 - moveType[5].radiusB * Math.cos(moveType[5].rotation);
					...
					if (!target && dealData.t === undefined) {
						...
						this.rotationMove =
							((Math.atan2(
								...
								(2 * dealData.tx + 2 * dealData.x - 4 * dealData.cx) * t - 2 * dealData.x + 2 * dealData.cx,
							) +
								Math.PI / 2) *
								180) /
							Math.PI;
			...
			case "BL": {
				...
				while (speed > 0) {
					if (detail.lastLen <= 0) {
						...
						detail.lastLen = math.sqrt(
							(detail.tx - detail.ox) * (detail.tx - detail.ox) + (detail.ty - detail.oy) * (detail.ty - detail.oy),
						);
	...
	Game_QJBulletMZ.prototype.updateImgRotation = function () {
		...
		switch (data.imgRotation[0]) {
			case "F":
				this.rotationImg = this.rotationMove + data.imgRotation[1].get();
				break;
			case "R":
				this.rotationImg += data.imgRotation[1].get();
				break;
			case "S":
				this.rotationImg = data.imgRotation[1].get();
				break;
			...
			if (data.d) {
				if (data.d[1] == 0) {
					...
					if (data.d[0] == 0) {
						if (data.d.length == 2) {
							data.d.push(null);
							data.d.push({ t: 0, fadeFun: new DEFrameQJ(null, "0|" + this.opacity + "~" + data.d[1] + "/0", 0) });
						}
					} else if (data.d[0] == 1) {
						if (data.d.length == 3) {
							data.d.push({
	...
	Game_QJBulletMZ.prototype.playAnimation = function (anim) {
		if (typeof anim === "number" && anim > 0) {
			...
			QJ.MPMZ.Shoot({
				...
				initialRotation: 0,
				moveType: ["S", 0],
				position: [this.inheritX(), this.inheritY()],
				existData: [{ t: ["Time", anim.time] }],
				z: anim.z ? anim.z : this.data.z,
				opacity:
					anim.fadeIn || anim.fadeOut
						? "0|1~" + anim.fadeIn + "/1~" + (anim.time - anim.fadeIn - anim.fadeOut) + "|1~" + anim.fadeOut + "/0"
						: 1,
			});
	...
	Game_QJBulletMZ.prototype.dealAction = function (
		actionListData = [],
		target = null,
		bulletTarget = null,
		ifPierce = false,
			...
			if (target) {
				if (target == $gamePlayer) {
					if (actionData[0] == "C" || actionData[0] == "CommonEvent") {
						...
						let argu =
							actionData[2] === undefined ? [] : Array.isArray(actionData[2]) ? actionData[2].slice() : [actionData[2]];
						argu.push({
					...
					if (actionData[0] == "EP" || actionData[0] == "EventPage") {
						...
						$gameSelfSwitches.setValue(
							[$gameMap.mapId(), target.eventId(), actionData[1].toUpperCase()],
							!!actionData[2],
						);
	...
	Game_QJBulletMZ.prototype.updateImgDeadAnim = function () {
		if (this.imgDeadAnimData) {
			...
			if (data.t >= this.imgDeadAnimData[1]) {
				...
				data.t++;
				if (data.fadeFun) {
					this.opacity = data.fadeFun.get();
				}
				if (data.scaleXFun) {
					this.scaleX = data.scaleXFun.get();
				}
				if (data.scaleYFun) {
					this.scaleY = data.scaleYFun.get();
				}
	...
	Game_QJBulletMZ.prototype.updateTrailDelay = function () {
		if (this.data.trailEffect) {
			return this.data.trailEffect.every(
				(d) => !d.ifProjctileWait || (d.ifProjctileWait && d.dataRemLength <= d.dataRemStart),
			);
		} else return true;
	};
	...
	Game_QJBulletMZ.prototype.updateExistData = function (
		haveTile = true,
		haveRebound = true,
		haveTime = true,
		ifPierce = true,
		...
		for (let k = 1, kdata = this.data.existData, kl = kdata.length, detail; k < kl; k++) {
			...
			if (haveTile) {
				if (detail.t[0] == "NP") {
					...
					if (
						this.isBodyNice(judgeBody) &&
						this.dealCollisionAll("dealNoPassCollision", detail, judgeBody, sendDataArray)
					) {
						if (this.dead) return;
					}
					...
					if (
						this.isBodyNice(judgeBody) &&
						this.dealCollisionAll("dealRegionCollision", detail, judgeBody, sendDataArray)
					) {
						if (this.dead) return;
					}
					...
					if (
						this.isBodyNice(judgeBody) &&
						this.dealCollisionAll("dealTerrainCollision", detail, judgeBody, sendDataArray)
					) {
						if (this.dead) return;
					}
				}
				...
				if (detail.t[0] == "P") {
					...
					if (this.isBodyNice(judgeBody) && this.isBodyNice($gamePlayer._boxBodyQJ)) {
						...
						if (haveRebound) {
							if (
								result.result &&
								this.JudgeReBound(result, judgeBody, $gamePlayer._boxBodyQJ, detail, $gamePlayer, null)
							) {
								...
								for (let j of $gameMap._groupListQJ[i]) {
									...
									if (character && character._boxBodyQJ && this.isBodyNice(character._boxBodyQJ)) {
										...
										if (haveRebound) {
											if (
												result.result &&
												this.JudgeReBound(result, judgeBody, character._boxBodyQJ, detail, character, null)
											) {
		...
		this.data.existData.forEach((detail) => {
			if (detail.c) {
				if (detail.c[0] === "T" || detail.c[0] === "Time") {
					...
					if (detail.c[1] === 0 && detail.c[2] === 0 && detail.c[3]) {
						//重置穿透次数
	...
	Game_QJBulletMZ.prototype.dealRegionCollision = function (
		x,
		y,
		detail,
		judgeBody,
	...
	Game_QJBulletMZ.prototype.dealTerrainCollision = function (
		x,
		y,
		detail,
		judgeBody,
	...
	Game_QJBulletMZ.prototype.dealNoPassCollision = function (
		x,
		y,
		detail,
		judgeBody,
	...
	Game_QJBulletMZ.prototype.addTimelineEffect = function (type, value) {
		...
		switch (type) {
			...
			case "B": {
				//[0目标值,1计时,2目标计时,3目标缩放]
				data = [1, 0, Math.max(1, value[1]), value[0]];
				break;
			}
			case "S": {
				//[0目标值x,1目标值y,2计时,3目标计时,4目标方位,5目标距离]
	...
	Sprite_QJBulletMZ.prototype.drawBulletTextData = function (imgData) {
		...
		if (imgData.advanced && [0, 2].includes(imgData.arrangementMode)) {
			...
			win.contents.clear();
			win.resetFontSettings_MPMZ(
				imgData.textColor,
				imgData.fontSize,
				imgData.outlineColor,
		...
		let bitmap =
			imgData.update && this.bitmap && this.bitmap.width === bitmapWidth && this.bitmap.height === bitmapHeight
				? this.bitmap
		...
		switch (imgData.textAlign) {
			...
			case 1:
				drawX = 0;
				drawY = bitmapHeight - realHeight;
				break;
			case 2:
				drawX = (bitmapWidth - realWidth) / 2;
				drawY = bitmapHeight - realHeight;
				break;
			case 3:
				drawX = bitmapWidth - realWidth;
				drawY = bitmapHeight - realHeight;
				break;
			//==================================================================
			case 4:
				drawX = 0;
				drawY = (bitmapHeight - realHeight) / 2;
				break;
			case 5:
				drawX = (bitmapWidth - realWidth) / 2;
				drawY = (bitmapHeight - realHeight) / 2;
		...
		if (imgData.advanced && [0, 2].includes(imgData.arrangementMode)) {
			...
			//========================================================================================
			bitmap.textColor =
				typeof imgData.textColor === "object"
					? QJ.colorGrad(
							bitmap,
							imgData.textColor[0],
							drawX,
							...
							realWidth,
							realHeight,
							(imgData.textColor[1] * Math.PI) / 180,
						)
			...
			//========================================================================================
			bitmap.outlineColor =
				typeof imgData.outlineColor === "object"
					? QJ.colorGrad(
							bitmap,
							imgData.outlineColor[0],
							drawX,
							drawY,
							realWidth,
							realHeight,
							(imgData.outlineColor[1] * Math.PI) / 180,
						)
		...
		if (imgData.lineWidth > 0) {
			...
			let lineColor =
				imgData.lineColor === null
					? bitmap.textColor
					: typeof imgData.lineColor === "object"
	...
	Sprite_QJBulletMZ.prototype.updateColorToneMPMZ = function () {
		if (this.remColorTone !== this.data._colorTone) {
			//至于tone变化判定，写在game里即可。
	...
	Sprite_QJBulletMZ.prototype.updateTrailEffect = function () {
		...
		let effectList = this.data.data.trailEffect,
			img;
		for (let i = 0, il = Math.max(list.length, effectList.length); i < il; i++) {
			if (effectList[i]) {
						...
						if (Array.isArray(img)) {
							...
							if (lineTexture[tag]) {
								...
								if (this.bitmap && this.bitmap.isReady()) {
									QJ.createTextureLineForTrailEffect(
										this.bitmap,
										tag,
										lineTexture,
										img[1],
				...
				if (list[i]) {
					//注意，为了防止突然消失，这里不直接删除，Sprite_MPMZ_Rope自己进行相关判定。
					list[i] = null;
				}
			}
		}
	...
	Sprite_QJBulletMZ.prototype.addTrailRopeSprite = function (canvas, effectData) {
		...
		if (typeof this.z === "number") {
			...
			this.parent.addChild(
				(sprite = new Sprite_MPMZ_Rope(new PIXI.Texture(canvas), effectData, this.data.dataRem, this.z)),
			);
		} else {
	...
	Game_QJLaserMZ.prototype.updaeXYR = function (ifInit) {
		...
		if (ifInit || !data.rotationStatic) {
			let newRotation,
				getWell = true;
			try {
				newRotation = QJ.MPMZ.model[1].rotationExtra(
					QJ.makeDeepCopy(data.rotation),
					data.eventId > 0 ? QJ.getCharacter(data.eventId) : null,
					[this.x - dx48, this.y - dy48],
				);
			} catch (e) {
				getWell = false;
			}
	...
	Game_QJLaserMZ.prototype.claculateRotationAdd = function () {
		...
		switch (this.data.rotationAdd[0]) {
			case "F":
				this.rotationAdd = this.data.rotationAdd[1].get();
				break;
			case "R":
				this.rotationAdd += this.data.rotationAdd[1].get();
				break;
	...
	Game_QJLaserMZ.prototype.updaeLength = function (ifInit) {
		...
		if (data.length[0] == "S") {
			...
			if (data.length[1][0] === "L") {
				...
				let startPosition = xyGet(
					[data.length[1][1], data.length[1][2]],
					data.eventId > 0 ? QJ.getCharacter(data.eventId) : null,
				);
				let endPosition = xyGet(
					[data.length[1][3], data.length[1][4]],
					data.eventId > 0 ? QJ.getCharacter(data.eventId) : null,
				);
				maxLength = Math.sqrt(
					(startPosition[0] - endPosition[0]) * (startPosition[0] - endPosition[0]) +
			...
			for (let result1, result2, i = data.length[2]; i >= 0; i--) {
				...
				if (result1 && result2) {
					...
					this.lineList.push([
						Math.floor(x + maxLength * Math.sin(r)) - dx48,
						Math.floor(y - maxLength * Math.cos(r)) - dy48,
						this.lineList[this.lineList.length - 1][2],
					]);
					maxLength = 0;
				}
			...
			endPosition.push(du);
			this.lineList = [
				[this.x - dx48, this.y - dy48, du],
				[this.x + Math.sin(du) * this.distanceLength - dx48, this.y - Math.cos(du) * this.distanceLength - dy48, du],
			];
		}
		//==============================================
	};
		...
		if (body.type === 0) {
			...
			for (let point of judgeList) {
				...
				if ((b * x1 - a * y1) * (b * x2 - a * y2) > 0) {
					...
					// 是判断是否与矩形相交的办法，对于原点在矩形上或者内的射线来说无效。
					A = y2 - y1;
					B = x1 - x2;
					C = x2 * y1 - x1 * y2;
					D = A * a + B * b;
	...
	Game_QJLaserMZ.prototype.getTarGrid = function (il, condition, r, ox, oy) {
		...
		let x,
			...
			lastX = -1,
			lastY = -1;
		let w = Math.sin(r) / tileSize,
			h = -Math.cos(r) / tileSize,
			ifPixel = false;
		let jumpDistance = 16;
		//==============================================
		let getTT = $gameMap.terrainTag.bind($gameMap);
	...
	Game_QJLaserMZ.prototype.updateExistData = function () {
		//haveTile = true,haveRebound = true,haveTime = true,ifPierce = true,ifTarget = true
		let data = this.data;
		...
		if (data.length[0] == "S") {
			this.QJBodies = [];
			for (
				let idata = this.lineList, il = idata.length - 1, detail, detailNext, i = 0, timefirst = true, body;
				i < il;
				i++
			) {
				detail = idata[i];
				...
				let lll = Math.sqrt(
					(detail[0] - detailNext[0]) * (detail[0] - detailNext[0]) +
						(detail[1] - detailNext[1]) * (detail[1] - detailNext[1]),
				);
				body = QJ.SAT.box((detail[0] + detailNext[0]) / 2 + dx48, (detail[1] + detailNext[1]) / 2 + dy48, [
					"R",
					judgeWidth,
					lll,
				]);
			...
			this.QJBody = QJ.SAT.box(this.x + (lll * Math.sin(r)) / 2, this.y - (lll * Math.cos(r)) / 2, [
				"R",
				judgeWidth,
				lll,
			]);
		...
		if (data.judgeMode[0] === "W") {
			...
			if (data.length[0] === "S") {
				for (
					let idata = this.lineList, il = idata.length - 1, detail, detailNext, i = 0, timefirst = true;
					i < il;
					i++
				) {
					this.QJBody = this.QJBodies[i];
	...
	Sprite_QJLaserMZ.prototype.updateData = function () {
		...
		let lineData = this.data.lineList,
			il = lineData.length,
			sprite;
		//========================================
		if (this.data.data.length[0] == "S") {
			...
			if (this.lineList[0])
				this.updateLineSprite(this.lineList[0], lineData, 0, refreshDB1, drl, scaleX, alpha, tone, ifTile);
			if (this.pointList[0])
				this.updatePointSprite(this.pointList[0], lineData, 0, refreshDB2, drp, scaleX, alpha, tone, ifTile);
			if (this.pointList[1])
				this.updatePointSprite(this.pointList[1], lineData, 1, refreshDB2, drp, scaleX, alpha, tone, ifTile);
		}
		//========================================
	};
	...
	Sprite_QJLaserMZ.prototype.updateLineSprite = function (
		sprite,
		lineData,
		i,
		refreshDB,
	...
	Sprite_QJLaserMZ.prototype.updatePointSprite = function (
		sprite,
		lineData,
		i,
		refreshDB,
	...
	Sprite_ProjectileContainerQJMZ.prototype.drawAfterImage = function (child) {
		...
		for (
			let i = 0,
				iDelay = child.data.afterImageDelay ? child.data.afterImageDelay[1] : 0,
				il = data[2],
				detail,
				detailLast;
			i + iDelay < il;
			...
			if (detail && detailLast && remData[i + iDelay]) {
				...
				if (!detail.dis) {
					detail.dis = Math.sqrt(
						(detail.x - detailLast.x) * (detail.x - detailLast.x) +
	...
	Sprite_ProjectileContainerQJMZ.prototype.updateParticle = function (child) {
		...
		let sendData = [
			...
			0,
			child.rotation,
			math.sin(child.rotation),
			math.cos(child.rotation),
			dx48,
			dy48,
			null,
			0,
	...
	Sprite_ProjectileContainerQJMZ.prototype.calculateParticleData = function (i, math, sendData) {
		sendData[12] =
			i.offset[0] + math.floor((math.random() * (i.offsetMax[0] - i.offsetMin[0]) + i.offsetMin[0]) * 100) / 100;
		sendData[13] =
			i.offset[1] + math.floor((math.random() * (i.offsetMax[1] - i.offsetMin[1]) + i.offsetMin[1]) * 100) / 100;
		sendData[14] =
			((i.offset[2] + math.floor((math.random() * (i.offsetMax[2] - i.offsetMin[2]) + i.offsetMin[2]) * 100) / 100) *
				math.PI) /
			180;
	...
	Sprite_ProjectileContainerQJMZ.prototype.calculateTextureFrameDataMPMZ = function (name, texture) {
		...
		if (frameList && frameList[1]) {
			...
			if (frameList.length === 1) {
				...
				frameData = [
					2,
					texture.width / frameList[0],
					texture.height / frameList[1],
					frameList[1],
					frameList[0],
					frameList[2],
				];
	...
	Sprite_ParticleContainerQJMZ.prototype.initialize = function (blendMode = 0) {
		particleParentPointer.call(this, 163840, {
			//一batch是16384。
			rotation: true,
			scale: true,
			alpha: true,
	...
	Sprite_QJParticleMZ.prototype.initialize = function (sendData) {
		...
		this.fadeList = [
			...
			0,
			this.scale.x,
			this.scale.x * this.disappearScale,
			this.scale.y,
			this.scale.y * this.disappearScale,
		];
	...
	Sprite_QJParticleMZ.prototype.setupFrameData = function (sendData, frameData) {
		...
		switch (frameData[0]) {
			...
			case 1: {
				let verticalIndex =
					sendData[9] === -1
						? Math.randomInt(frameData[3])
						: sendData[9] === -2
							? sendData[8] % frameData[3]
							: sendData[9];
				this.dymaticBitmap = false;
	...
	if (isMV) {
		...
		Sprite_MPMZ_Rope.prototype.initialize = function (texture, effectData, dataRem, z) {
			...
			if (!effectData.hasInit) {
				//这里放的是需要储存的数据。需要保证这些数据能使Sprite_MPMZ_Rope在重创建时复原。
		...
		Sprite_MPMZ_Rope.prototype.update = function () {
			//写在这里，保证运算的帧率。故而记得不能写在_render中。
			...
			if (effectData.disappearPreTime > 0) {
				...
				//保证核心节点（每帧只加一次）时才停止。
				while (totalLength > effectData.dataRemStart && dataRem[effectData.dataRemStart].ic === false)
					effectData.dataRemStart++;
			}
			if (totalLength <= effectData.dataRemStart) {
				//未开始或暂时结束
				if (this.needDestroy) {
					this.destroy();
				}
			...
			let currentIndex = effectData.dataRemStart,
				batchIndex = 0;
    ...
    `;
		...
		Sprite_MPMZ_Rope_Renderer.prototype.render = function (mesh, geometryData) {
			...
			glData.shader.uniforms.uColor = PIXI.utils.premultiplyRgba(
				...
				texture.baseTexture.premultipliedAlpha,
			);
			//===========================================================================
		...
		Sprite_MPMZ_Rope.prototype.initialize = function (texture, effectData, dataRem, z) {
			...
			PIXI.Mesh.call(
				...
				new PIXI.MeshMaterial(texture, {
					program: Sprite_MPMZ_Rope.getProgramMPMZ(effectData.imgStretchMode, effectData.hOrV),
				}),
			);
			this.batchSize = batchSize;
			this.effectData = effectData;
...
*/
	...
	Game_Map.prototype.getGroupBulletListQJ = function (groupName) {
		return this._mapBulletsNameQJ
			? this._mapBulletsNameQJ[groupName]
				? Object.keys(this._mapBulletsNameQJ[groupName])
				: []
			: [];
	};
	...
	Game_Map.prototype.refreshMapBoxQJ = function () {
		...
		let nP = this._noPassBoxQJ,
			nPL = this._noPassBoxQJLaser,
			g2,
			g4,
			g6,
			g8;
	...
	Game_Map.prototype.addBulletQJ = function (bullet, type) {
		let bulletsTarget,
			index = this._mapBulletsQJLength; //this._mapBulletsQJ.length;
...
Game_Character.prototype.updateShadowCirccle = function() {
	...
	Game_Event.prototype.refreshScreenBoxDataQJ = function () {
		...
		//========================================
		let content = QJ.calculateAnnotation(this),
			detail;
		//========================================
...
Game_Follower.prototype.update = function(sceneActive) {
	...
	Spriteset_Map.prototype.findBulletContainerQJ = function (index) {
		...
		if (typeof data.data.z === "number") {
			...
			switch (data.data.z) {
				case "P":
					return this._1ContainerQJ;
				case "M":
					return this._2ContainerQJ;
				case "E":
					return this._3ContainerQJ;
				case "W":
					return this._4ContainerQJ;
				case "A":
					return this._5ContainerQJ;
				case "S":
					return this._6ContainerQJ;
				default:
					return null;
			}
		}
	...
	if (true) {
		...
		model1.initialRotationExtra = (value, event, pos) => {
			...
			if (typeof value !== "number") {
				...
				switch (value[0]) {
					case "PD": {
						value[0] = QJ.calculateAngleByDirectionAngle($gamePlayer.direction());
						addValue = value[1] ? value[1] : 0;
						break;
					}
					case "P": {
						value[0] = QJ.calculateAngleByTwoPointAngle(
							pos[0],
							pos[1],
							$gamePlayer.screenShootXQJ(),
							$gamePlayer.screenShootYQJ(),
						);
						addValue = value[1] ? value[1] : 0;
						break;
					}
					case "M": {
						if (isMV) {
							value[0] = QJ.calculateAngleByTwoPointAngle(pos[0], pos[1], mouseX, mouseY);
							addValue = value[1] ? value[1] : 0;
						} else {
							value[0] = QJ.calculateAngleByTwoPointAngle(pos[0], pos[1], TouchInput.x, TouchInput.y);
							addValue = value[1] ? value[1] : 0;
						}
						break;
					}
					case "E": {
						...
						if (eventData)
							value[0] = QJ.calculateAngleByTwoPointAngle(
								pos[0],
								pos[1],
								eventData.screenShootXQJ(),
								eventData.screenShootYQJ(),
							);
						else if (value[3]) {
							let subValue = model1.initialRotationExtra(value[3], event, pos);
							value[3] = null;
							return subValue;
						} else value[0] = 0;
					...
					case "G": {
						let id = QJ.MPMZ.getMinEventId(
							pos[0] + $gameMap.displayX() * tileSize,
							pos[1] + $gameMap.displayY() * tileSize,
							value[1],
						);
					...
					case "BT": {
						...
						if (bulletData)
							value[0] = QJ.calculateAngleByTwoPointAngle(
								pos[0],
								pos[1],
								bulletData.screenXShowQJ(),
								bulletData.screenYShowQJ(),
							);
						else if (value[3]) {
							let subValue = model1.initialRotationExtra(value[3], event, pos);
							value[3] = null;
			...
			switch (value[0]) {
				...
				case "E": {
					...
					if (charTemp) value = charTemp.screenShootXQJ();
						...
						let subValue = model1.x(value[3], event);
						value[3] = null;
						return subValue;
					} else value = 0;
					break;
			...
			switch (value[0]) {
				...
				case "E": {
					...
					if (charTemp) value = charTemp.screenShootYQJ();
						...
						let subValue = model1.y(value[3], event);
						value[3] = null;
						return subValue;
					} else value = 0;
					break;
		...
		model1.img = (value) => {
			if (typeof value === "object" && value[0] === "C") {
				...
				if (typeof value[1] === "object") {
					...
					//[0'T',1text,2arrangement mode,3text color,4text size,[5stroke color,stroke thickness,font face]老方法
				...
				for (
					let attributeNameList = [
						...
						i = 0,
						il = attributeNameList.length,
						defaultBitmap = Bitmap.blankBitmapForBulletQJ;
					i < il;
					i++
				) {
		...
		model1.dealExistData = (i, newList = null) => {
			...
			if (
				!i.rb ||
				!(i.t[0] == "R" || i.t[0] == "T" || i.t[0] == "NP" || i.t[0] == "G" || i.t[0] == "P" || i.t[0] == "B")
			)
				i.rb = [0, true, true, true];
	...
	QJ.MPMZ.Shoot = function (data) {
		...
		try {
			...
			//======================================
			if (data.judgeAccuracy !== undefined && data.judgeAccuracyMove === undefined)
				data.judgeAccuracyMove = data.judgeAccuracy;
			//======================================
	...
	//=================================================================
	//不能有offset value
	QJ.MPMZ.Shooter_ArcRange = function (
		initialRotation,
		data,
		minRotation,
		maxRotation,
	...
	QJ.MPMZ.itemGiverCharacter = function (type, itemId, charId, num = 1) {
		...
		let itemData;
		if (type == 0 || type == "item") {
			itemData = $dataItems[itemId];
			type = 0;
		} else if (type == 1 || type == "weapon") {
			itemData = $dataWeapons[itemId];
			type = 1;
		} else if (type == 2 || type == "armor") {
			itemData = $dataArmors[itemId];
			type = 2;
		} else if (type == 3 || type == "gold") {
			itemData = null;
			type = 3;
		} else return;
		if (type == 3 || type == "gold") {
			QJ.MPMZ.Shoot({
				...
				moveType: ["TP", 6, 10, 15],
				existData: [
					{ t: ["P"], c: ["T", 30, 0, true], a: ["F", QJ.gainItemEx, [3, 0, num]], d: [1, 15, 1.1] },
			...
			QJ.MPMZ.Shoot({
				...
				moveType: ["TP", 6, 10, 15],
				existData: [
					{ t: ["P"], c: ["T", 30, 0, true], a: ["F", QJ.gainItemEx, [type, itemId, num]], d: [1, 15, 1.1] },
					{ t: ["Time", 3600], d: [1, 15, 1.1] },
				],
			});
		}
	...
	Bitmap.measureTextSizeForBulletQJ = function (
		text,
		arrangement,
		fontSize,
		fontFace,
	...
	Window_ForImgText_MPMZ.prototype.getTextWidthEx_MPMZ = function (text) {
		return (
			...
			Math.max(
				this.contents.fontItalic || this.contents.fontBold ? this.contents.fontSize * 0.14 : 0,
	...
	Window_ForImgText_MPMZ.prototype.resetFontSettings_MPMZ = function (
		fontColor,
		fontSize,
		outlineColor,
		outlineWidth,
		...
		switch (code) {
			...
			case "FS":
				...
				textState.x += Math.max(
					this.contents.fontItalic || this.contents.fontBold
						? this.contents.fontSize * 0.14
						: this.contents.fontSize * 0.05,
					this.contents.outlineWidth + 2,
				);
				break;
			case "OC":
...
if (true) {
	...
	QJ.SAT.box = function (x, y, boxType, isCCW = false) {
		...
		if (boxType[0] == "C") {
			...
			body = isCCW
				? new SATPolygon(new SATVector(x, y), [
	...
	QJ.SAT.boxX = function (x, y, body, sx, sy, ax, ay, r) {
		...
		if (body[0] == "C") {
			...
			newSATBody.setOffset(new SATVector(body[1] * sx * (0.5 - ax), body[2] * sy * (0.5 - ay)));
	...
	if (QJ.MPMZ.isMV) {
		...
		AnimatedQJ.prototype.get = function (w, h) {
			return {
				x: (w / this.v) * (this.n % this.v),
				y: (h / this.h) * Math.floor(this.n / this.v),
				w: w / this.v,
				h: h / this.h,
			};
		};
		AnimatedQJ.prototype.set = function (objectData, w, h, force = false) {
			if (objectData.AnimatedQJIndex != this.n || force) {
				objectData.setFrame(
					(w / this.v) * (this.n % this.v),
					(h / this.h) * Math.floor(this.n / this.v),
					w / this.v,
					h / this.h,
				);
		...
		AnimatedQJ.prototype.getSaveWH = function () {
			return {
				x: (this.wi / this.v) * (this.n % this.v),
				y: (this.he / this.h) * Math.floor(this.n / this.v),
				w: this.wi / this.v,
				h: this.he / this.h,
			};
		};
		...
		QJ.colorGrad = function (bitmap, content, x, y, w, h, ro = 0, offset = 0) {
			if (content.constructor == Bitmap) {
				...
				const grad = bitmap._context.createLinearGradient(
					Math.floor(x + w / 2 + noSoildRadius * Math.sin(ro)),
					Math.floor(y + h / 2 - noSoildRadius * Math.cos(ro)),
					Math.floor(x + w / 2 - noSoildRadius * Math.sin(ro)),
					Math.floor(y + h / 2 + noSoildRadius * Math.cos(ro)),
				);
				if (offset == 0) {
					...
					for (let i = 0, detail, point; i < colorNum; i++) {
						...
						if (hasNotMoveUp && point > 1) {
							...
							let trueColor = QJ.gradientPointCalculate(
								lastColorDetail[1],
								detail[1],
								(1 - lastColorDetail[0] - offset) / (detail[0] - lastColorDetail[0]),
							);
