...
QJ.LL.isOnMapAndHaveLoaded = function () {
	return SceneManager._scene && SceneManager._scene._spriteset && SceneManager._scene.constructor.name == "Scene_Map";
};
//==========================================================
//
//==========================================================
...
(($ = {}) => {
	...
	let dx = 0,
		...
		gh = 0,
		gws = 0,
		ghs = 0; //map display x/y.graphhics width/height.
	//============================
	//============================
	//============================
	//============================
	...
	(() => {
		...
		for (let i of ps) {
			...
			if (miniLightsData[detail.lightId]) {
				...
				regionLightsData[Number(detail.id)].showConditionExtra =
					detail.showConditionExtra.length > 2
						? eval("(function(ifShow){" + eval(detail.showConditionExtra) + "})")
						: null;
			} else {
	...
	QJ.LL.getLLDataDetail = function (detail, annotation) {
		for (let i = 0, id = annotation.match(/<QJLL-[^:]*:[^>]*>/gi) || [], il = id.length, detailData; i < il; i++) {
			...
			else if (detailData[1] == "shadowCharacterMaxOpacity")
				detail.shadowCharacterMaxOpacity = Number(detail.shadowCharacterMaxOpacity);
			else if (detailData[1] == "shadowCharacterMaxDistance")
				detail.shadowCharacterMaxDistance = Number(detail.shadowCharacterMaxDistance);
	...
	QJ.LL.getCSData = function (detail, annotation) {
		...
		csData.imgName = "";
		for (let i = 0, id = annotation.match(/<QJCS-[^:]*:[^>]*>/gi) || [], il = id.length, detailData; i < il; i++) {
	...
	QJ.LL.tint = function (time, color, ifSystem) {
		if (time == 0) {
			$gameSystem.lightStaticChange = [0, null, color, ifSystem ? $gameSystem.lightStaticChange[3] : color];
		} else {
			$gameSystem.lightStaticChange = [
				time,
				new QJFrameLight("___", "0|" + $gameSystem.lightStaticChange[2] + "~" + time + "/" + color, 1),
				color,
	...
	QJ.LL.tempLight = function (lightId, during, x, y) {
		...
		let odata = new Game_QJLightLayerMini(
			{ type: 0, x: x + dx48 + standardExpand / 2, y: y + dy48 + standardExpand / 2 },
			initData,
			$gameSystem.miniLights.length,
		);
	...
	QJ.LL.tempLightMap = function (lightId, during, x, y) {
		//不加dx48
		let initData = QJ.LL.dealMiniLights(lightId);
		initData.during = during === -1 ? -1 : Math.max(0, during);
		let odata = new Game_QJLightLayerMini(
			{ type: 0, x: x + standardExpand / 2, y: y + standardExpand / 2 },
			initData,
			$gameSystem.miniLights.length,
		);
		$gameSystem.miniLights.push(odata);
...
*/
	...
	Game_Map.prototype.setup = function (mapId) {
		...
		for (let i = 0, il = this.width(), j, jl = this.height(), id, lsid, k, heightY; i < il; i++) {
			...
			for (j = 0; j < jl; j++) {
				...
				if (regionLightsData[id]) {
					//type 0固定位置 1地形 2对象
					$gameSystem.miniLights.push(
						new Game_QJLightLayerMini(
							{
								type: 1,
								regionId: id,
								x: i * standardTile + standardTile / 2,
								y: j * standardTile + standardTile / 2,
								mapX: i,
								mapY: j,
							},
							QJ.LL.dealRegionLights(id),
							$gameSystem.miniLights.length,
						),
					);
				}
	...
	Game_Player.prototype.refreshFollowersShadow = function () {
		for (
			let i = 0, il = this._followers._data.length, vis = this._followers._visible && $gamePlayer.QJSC.status;
			i < il;
			i++
		) {
...
Game_Event.prototype.update = function() {
	...
	Sprite_timeSystemSpriteQJ.prototype.refreshData = function () {
		...
		if (
			(this.model &&
				(!ImageManager.loadSystem(hub.timeSystemHubImg1).width ||
		...
		this.h = -1;
		this.working = true;
		let baseX = this.timeSystemHubX,
			baseY = this.timeSystemHubY;
		if (this.model) {
			//==================================================
	...
	Sprite_timeSystemSpriteQJ.prototype.updateData = function () {
		...
		if (this.model) {
			...
			this.bitmap5.clear();
			this.bitmap5.drawText(
				(this.h > 9 ? this.h : "0" + this.h) + ":" + (this.m > 9 ? this.m : "0" + this.m),
				0,
				0,
				this.sprite5.width,
				this.sprite5.height,
	...
	Sprite_QJLLParticleContainer.prototype.initialize = function () {
		PIXI.ParticleContainer.call(this, 10240, {
			//一batch是16384。
			rotation: true,
			scale: true,
			alpha: true,
		...
		for (let i = Math.floor(dx - 1), il = Math.ceil(dx + gw / 48 + 1); i < il; i++) {
			for (let j = Math.floor(dy - 1), jl = Math.ceil(dy + gh / 48 + 1); j < jl; j++) {
				...
				if (regionData[regionId] && regionData[regionId].rectShape.length > 0) {
					...
					for (
						let k = 0, mk = 0, kl = blockShape.length, initX = blockShape[0].x, initY = blockShape[0].y, x, y;
						k < kl;
						k++
					) {
						if (mk == 0) {
	...
	Game_QJLightLayer.prototype.initialize = function (characterId, initLightData) {
		...
		//=========================================
		this.dialogLength = Math.floor(
			Math.sqrt(
				(this.bimtapWidth * this.bimtapWidth) / this.lightSpriteFrame[2] / this.lightSpriteFrame[2] +
		...
		if (rfo.time <= 0) {
			if (rfo.frameTime >= rfo.realFadeTime) {
				rfo.time = Math.max(
					1,
					Math.floor(
						od.randomFlickerOpacity.minIntervalTime +
							Math.random() * (od.randomFlickerOpacity.maxIntervalTime - od.randomFlickerOpacity.minIntervalTime),
					),
				);
				rfo.frameTime = 0;
			} else {
	...
	Game_QJLightLayer.prototype.update = function () {
		...
		if (character._direction8dir != undefined) {
			if (
				character._direction8dir == 1 ||
				character._direction8dir == 3 ||
				character._direction8dir == 7 ||
				character._direction8dir == 9
			) {
		...
		if (this.dirRotationFrame[3] != od.dirRotation[d]) {
			if (od.dirRotationFrame > 0) {
				...
				if (Math.abs(changeDegree) > Math.PI) {
					this.dirRotationFrame[1] =
						(-Math.sign(changeDegree) * (Math.abs(changeDegree) - Math.PI)) / od.dirRotationFrame;
				} else {
	...
	Game_QJLightLayer.prototype.updatePosition = function (od, character, d) {
		//=========================================
		if (!$dataMap) return;
		this.x = $gameMap.adjustXWithoutDisplay(character._realX) * standardTile + od.offsetX.get() + od.dirOffsetX[d];
		this.y = $gameMap.adjustYWithoutDisplay(character._realY) * standardTile + od.offsetY.get() + od.dirOffsetY[d];
		//=========================================
	};
	Game_QJLightLayer.prototype.updateRotation = function (od, character, d) {
		...
		if (od.rotationMouse)
			this.rotation += QJ.LL.calculateAngleByTwoPoint(
				character.screenX(),
				character.screenY(),
				mouseX + standardExpand / 2,
				mouseY + standardExpand / 2,
			);
		if (this.attach) {
	...
	Sprite_QJLightLayer.prototype.initialize = function (_parent, odata) {
		...
		if (this.shadowWall) {
			...
			this._texture.baseTexture.sendRotationData = [
				0,
				0,
				0,
				0,
	...
	Sprite_QJLightLayer.prototype.update = function () {
		...
		//===================================
		let refreshShadow = false,
			refreshShadowUvs = false,
			newVisible = true;
		//===================================
		this.updatePosition();
		//===================================
	...
	Sprite_QJLightLayer.prototype.refreshShadowUvs = function () {
		...
		this._texture.baseTexture.sendRotationData[2] =
			0.5 - ((cos * this.oldScaleX - sin * this.oldScaleY) * 0.5) / this.oldScaleXRem;
		this._texture.baseTexture.sendRotationData[3] =
			0.5 - ((sin * this.oldScaleX + cos * this.oldScaleY) * 0.5) / this.oldScaleYRem;
	...
	Sprite_QJLightLayer.prototype.refreshShadowRegion = function () {
		...
		let sc = this.shadowSprite,
			dl = this.dialogLength / 2,
			sd = $gameMap.shadowDataQJLL;
		let absPX = mathPointer.floor(x / tileSize) * tileSize - x,
			absPY = mathPointer.floor(y / tileSize) * tileSize - y;
		let scaleX = this.oldScaleXRem,
			scaleY = this.oldScaleYRem;
		let iStart = mathPointer.max(mathPointer.floor((x - dl * scaleX) / tileSize), 0),
			iEnd = mathPointer.min(mathPointer.floor((x + dl * scaleX) / tileSize), $gameMap.width() - 1);
		let jStart = mathPointer.max(mathPointer.floor((y - dl * scaleY) / tileSize), 0),
			jEnd = mathPointer.min(mathPointer.floor((y + dl * scaleY) / tileSize), $gameMap.height() - 1);
		let id,
			x1,
			y1,
			x2,
			...
			sx,
			sy,
			expandTimes = mathPointer.max(jEnd - jStart, iEnd - iStart),
			addHeight = 0;
		//===================================
		...
		//===================================
		this.onWallMode =
			sd[mathPointer.min(mathPointer.max(mathPointer.round(x / tileSize), 0), $gameMap.width() - 1)][
			...
			for (let j = jStart; j <= jEnd; j++) {
				...
				if (regionData[id] && regionData[id].shadowShow) {
					...
					tempFillStyle1 = Object.assign(new pixiPointer.FillStyle(), {
						color: regionData[id].shadowTint,
						alpha: regionData[id].shadowOpacity,
						visible: true,
					});
					...
					if (this.onWallMode != -1) {
						...
						if (tarY != j && tarY == this.onWallMode) {
							//===========================
							sc.geometry.graphicsData.push(
								new pixiPointer.GraphicsData(
									new pixiPointer.Polygon(
										(sx + tileSize) / scaleX,
										sy / scaleY,
										(sx + tileSize) / scaleX,
										(sy + addHeight * tileSize + tileSize) / scaleY,
										sx / scaleX,
										(sy + addHeight * tileSize + tileSize) / scaleY,
										sx / scaleX,
										sy / scaleY,
							...
							if (j + addHeight - this.onWallMode == 1) {
								if (sx >= 0 && sy >= 0) {
									//3
									x1 = sx / scaleX;
									y1 = (sy + tileSize) / scaleY;
									x2 = (sx + tileSize) / scaleX;
									y2 = sy / scaleY;
									sc.geometry.graphicsData.unshift(
										new pixiPointer.GraphicsData(
								...
								} else if (sx <= -tileSize && sy >= 0) {
									//1
									x1 = sx / scaleX;
									y1 = sy / scaleY;
									x2 = (sx + tileSize) / scaleX;
									y2 = (sy + tileSize) / scaleY;
									sc.geometry.graphicsData.unshift(
										new pixiPointer.GraphicsData(
						...
						else if (sx <= -tileSize && sy < -tileSize) {
							x1 = sx + tileSize;
							y1 = sy;
							x2 = sx;
							y2 = sy + tileSize;
						...
						//===========================
						if (j + 1 < $gameMap.height() && regionIdForShadow(i, j + 1) != id) {
							//收集所有墙面数据
		...
		if (this.onWallMode != -1) {
			...
			sc.geometry.graphicsData.splice(
				0,
				0,
				new pixiPointer.GraphicsData(
					new pixiPointer.Polygon(dl, -dl, dl, sy, -dl, sy, -dl, -dl),
					Object.assign(new pixiPointer.FillStyle(), { color: 0x000000, alpha: 1, visible: true }),
					tempNullLineStyle,
					null,
				),
			);
			...
			for (let i = 0, il = whiteBlockList.length, detail, blackAfterAdd; i < il; i++) {
				...
				if (detail[3] >= 0) {
					...
					//分为两种，一种是渐变过渡色，一种是纯白，但不管是哪一个，只有完全可透时是自己的颜色，不然就是纯黑色
					...
					for (let j = 0, jl = blockBlockList.length, blockDetail, lineX1, lineX2; j < jl; j++) {
						...
						if (
							mathPointer.min(blockDetail[0], blockDetail[2]) >= detail[0] &&
						...
						if (blockDetail[0] == blockDetail[2] && blockDetail[0] * footSX > 0 && blockDetail[0] * footEX > 0) {
							//八个象限，这个方法中全左和全右需要特殊判定，也就是射点在同一竖列时
							...
							if (lineX1 * blockDetail[0] < 0) {
								//不要加上等于号
								continue;
							}
							lineX2 = (footY * blockDetail[2]) / blockDetail[3];
							if (lineX2 * blockDetail[2] < 0) {
								//不要加上等于号
								continue;
							}
						...
						lineX1 =
							footY *
							(blockDetail[0] < blockDetail[2] ? blockDetail[0] / blockDetail[1] : blockDetail[2] / blockDetail[3]);
						if (lineX1 > footSX && lineX1 < footEX) {
							//左白右黑分涂块
							sc.geometry.graphicsData.push(
								new pixiPointer.GraphicsData(
									new pixiPointer.Polygon(
					...
					for (let bAA of blackAfterAdd) {
						sc.geometry.graphicsData.push(
							new pixiPointer.GraphicsData(new pixiPointer.Polygon(bAA), tempFillStyle2, tempNullLineStyle, null),
						);
					}
					if (isTotalNoCave) {
						//完全没有被射到，是原色，或者说纯白块
	...
	if (Imported.Blizzard_UltraMode7) {
		...
		Sprite_QJLightLayer.prototype.updatePosition = function () {
			...
			//===================================
			let loopedPosition = $gameMap.adjustUltraMode7LoopedPosition(
				(this.odata.x + standardTile / 2) / standardTile,
				(this.odata.y + standardTile / 2) / standardTile,
			);
			let position = UltraMode7.mapToScreen(
				loopedPosition.x * $gameMap.tileWidth(),
				loopedPosition.y * $gameMap.tileHeight(),
			);
			let scale = (this.UM7scale = UltraMode7.mapToScreenScale(
				loopedPosition.x * $gameMap.tileWidth(),
				loopedPosition.y * $gameMap.tileHeight(),
			));
		...
		Sprite_QJLightLayerMini.prototype.updatePosition = function () {
			...
			//===================================
			let loopedPosition = $gameMap.adjustUltraMode7LoopedPosition(
				this.odata.x / standardTile,
				this.odata.y / standardTile,
			);
			let position = UltraMode7.mapToScreen(
				loopedPosition.x * $gameMap.tileWidth(),
				loopedPosition.y * $gameMap.tileHeight(),
			...
			let scale = UltraMode7.mapToScreenScale(
				loopedPosition.x * $gameMap.tileWidth(),
				loopedPosition.y * $gameMap.tileHeight(),
			);
			let scaleX = this.odata.scaleX * scale;
	...
	QJLightRender.prototype.buildDrawCalls = function (texArray, start, finish) {
		...
		for (var i = start; i < finish; ++i) {
			...
			var spriteBlendMode = PIXI.utils.premultiplyBlendMode[tex.alphaMode ? 1 : 0][sprite.blendMode];
			elements[i] = null;
	...
	QJLightRender.prototype.drawBatches = function () {
		...
		for (var i = 0; i < dcCount; i++) {
			...
			this._defaultSyncData.textureCount = 0;
			this.bindUniforms(
				this.renderer.shader.getglProgram().uniformData,
				this._shader.uniformGroup.uniforms,
				ref,
				this._defaultSyncData,
			);
			//=================================
	...
	Sprite_QJCharacterShadowLayer.prototype.update = function () {
		...
		for (let i = 0, il = children.length - 1; i < il; i++) {
			...
			for (let j in mcsl) {
				...
				angle = QJ.LL.calculateAngleByTwoPoint(
					lx,
					ly,
					cx - odata.shadowCharacterOffsetX,
					cy - odata.shadowCharacterOffsetY,
				);
				...
				distence =
					Math.sqrt(
						(cx - odata.shadowCharacterOffsetX - lx) * (cx - odata.shadowCharacterOffsetX - lx) +
				...
				csTarget.alpha =
					(Math.floor(
						csd.opacity *
							initData.shadowCharacterMaxOpacity *
							100 *
							Math.min(1, Math.max(1 - distence / initData.shadowCharacterMaxDistance)),
					) /
						100) *
	...
	if (Imported.QJDoodads) {
		...
		Scene_Map.prototype.addNewDoodadEditAttribute = function (list, doodadData, editWindow) {
			...
			list = list.concat([
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.status ? "true" : "false",
						inputType: 3,
						attribute: ["shadow", "status"],
						fixValue: (value) => {
							return !eval(value);
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.yCut,
						inputType: 1,
						attribute: ["shadow", "yCut"],
						fixValue: (value) => {
							value = Number(value);
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.offsetX,
						inputType: 1,
						attribute: ["shadow", "offsetX"],
						fixValue: (value) => {
							value = Number(value);
							return isNaN(value) ? 0 : Math.floor(value);
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.offsetY,
						inputType: 1,
						attribute: ["shadow", "offsetY"],
						fixValue: (value) => {
							value = Number(value);
							return isNaN(value) ? 0 : Math.floor(value);
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.opacity,
						inputType: 1,
						attribute: ["shadow", "opacity"],
						fixValue: (value) => {
							value = Number(value);
							return value >= 0 && value <= 1 ? Math.floor(value * 1000) / 1000 : 1;
						},
						left: (value) => {
						...
						help: [
							"The opacity of shadow. range 0-1.",
							"Press ← to subtract 0.05.Press → to add 0.05. ",
							"Need plugin QJ-Lighting.",
						],
					},
					handler: null,
				},
				{
					...
					ext: {
						content: doodadData.shadow.opacity,
						inputType: 1,
						attribute: ["shadow", "tint"],
						fixValue: (value) => {
							value = String(value);
							return value[0] == "#" && value.length == 7 && !isNaN(Number("0x" + value.substr(1))) ? value : "#000000";
						},
				...
				},
				{
					...
					enable: true,
					ext: {
						content: doodadData.shadow.model,
						inputType: 1,
						attribute: ["shadow", "model"],
						fixValue: (value) => {
							return value;
						},
						help: [
							"The model of shadow.",
							"Default value is B[].",
							"Need plugin QJ-Lighting. Please visit the help of QJ-Lighting for detail.",
						],
					},
					handler: null,
				},
		...
		//
		//==========================================================
		const LL_Sprite_QJCharacterShadowLayer_updateOtherShadow =
			Sprite_QJCharacterShadowLayer.prototype.updateOtherShadow;
		Sprite_QJCharacterShadowLayer.prototype.updateOtherShadow = function (container, lx, ly, initData, odata) {
		...
		Sprite_DoodadQJ.prototype.setBase = function () {
			...
			if (this.constructor.name == "Sprite_MouseDoodadsCursorQJ" || !this._spriteset.lightSystemSprite) {
