...
//=============================================================================
/*:
 * @plugindesc (v1.7)[v1.5]  地图UI - 时间系统固定框
 * @author Moghunter （砯九、Drill_up翻译+优化）
 *
 * @param 是否初始显示
 * @type boolean
 * @on 显示
 ...
 * @on 中心旋转
 * @off 默认
 * @desc true - 中心旋转，false - 默认，设置后，将按指针的中心点进行旋转，默认为正下方的锚点。
 * @default false
 *
 * @param 是否为中心旋转-时针
 * @parent 是否显示指针
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_TimeSystem_Hud');
	...
	Moghunter.timehud_point_centerAnchorB = String(Moghunter.parameters['是否为中心旋转-分针'] || "false") == "true";
	Moghunter.timehud_point_centerAnchorC = String(Moghunter.parameters['是否为中心旋转-时针'] || "false") == "true";
...
SpriteTimeEngine.prototype.create_point = function() {
	 ...
	 if( Moghunter.timehud_point_centerAnchorB == true ){
		this._point1.anchor.x = 0.5;
		this._point1.anchor.y = 0.5;
	 }else{
		this._point1.anchor.x = 0.5;
		this._point1.anchor.y = 0.0;
	 }
	 ...
	 if( Moghunter.timehud_point_centerAnchorC == true ){
		this._point2.anchor.x = 0.5;
		this._point2.anchor.y = 0.5;
	 }else{
		this._point2.anchor.x = 0.5;
		this._point2.anchor.y = 0.0;
	 }
