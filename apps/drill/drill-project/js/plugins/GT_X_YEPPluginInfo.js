...
if (Imported.GT_ObjectInfoWindow) {
	...
	DataManager.isDatabaseLoaded = function () {
		if (!GT.YEPInfo.DataManager_isDatabaseLoaded.call(this)) return false;
	...
	Window_ObjectInfo.prototype.getPassiveAuras = function (state) {
		...
		for (var i = 0; i < Yanfly.Aura.Types.length; i++) {
			...
			for (var j = 0; j < passiveAuras.length; j++) {
				...
				if (GT.Param.OIWShowIcon && auraIcon !== 0) auraName = "\\I[" + auraIcon + "]" + auraName;
	...
	Window_ObjectInfo.prototype.getStateTitle = function (state) {
		var name = state.name;
		if (GT.Param.OIWShowIcon && state.iconIndex !== 0) name = "\\I[" + state.iconIndex + "]" + name;
		...
		else var condition = this.addBasicColor(String(GT.Param.YEPPassiveStateText.AlwaysAct));
	...
	Window_ObjectInfo.prototype.getStateRequire = function (state) {
		if (!this.getObjTextShow("YEPPassiveStateText", "StateCondition")) return this._stateRequire.push(false);
	...
	Window_ObjectInfo.prototype.getSkillItemLifeSteal = function () {
		...
		for (var i = 0; i < 4; ++i) {
			...
			if (value) {
				text =
					this.getObjText(category, code) +
					this.textColon() +
					this.addBasicColor(i % 2 ? Math.floor(value) : Math.floor(value * 100) + "%");
				this._data.push(text);
			}
		}
		...
		for (var i = 0; i < 3; ++i) {
			...
			if (value) {
				text =
					this.addSystemColor(String(GT.Param.OIWMainText.TraitText)) +
					this.textColon() +
					this.addBasicColor(this.getObjText(category, code, 1));
				this._data.push(text);
			}
		}
	};
	...
	Window_ObjectInfo.prototype.getEquipLifeSteal = function (item) {
		...
		for (var i = 0; i < 12; ++i) {
			...
			if (value > 0) {
				text =
					this.getObjText(category, code) +
					this.addIncreaseColor("+" + (i % 6 < 3 ? Math.floor(value * 100) + "%" : Math.floor(value)));
				this.pushData(text, item);
			} else if (value < 0) {
				text =
					this.getObjText(category, code) +
					this.addDecreaseColor(i % 6 < 3 ? Math.floor(value * 100) + "%" : Math.floor(value));
				this.pushData(text, item);
			}
		}
	...
	Window_ObjectInfo.prototype.getEquipElement = function (item) {
		...
		if (this.getObjTextShow(category, "elementAbsorb")) {
			...
			if (elements.length) {
				for (var j = 0; j < elements.length; ++j) {
					...
					if (!id) continue;
					text =
						this.getObjText(category, "elementAbsorb") +
						this.textColon() +
						this.addBasicColor($dataSystem.elements[id]);
					this.pushData(text, item);
				}
			}
		}
		for (var i = 0; i < 3; ++i) {
			...
			if (this.getObjTextShow(category, code)) {
				...
				for (var j = 1; j <= length; ++j) {
					...
					var valueText =
						rate > 0
							? this.addIncreaseColor("+" + Math.floor(rate * 100) + "%")
							: this.addDecreaseColor(Math.floor(rate * 100) + "%");
		...
		if (item.elementNull && this.getObjTextShow(category, "elementNull")) {
			text =
				this.addSystemColor(String(GT.Param.OIWMainText.TraitText)) +
				this.textColon() +
				this.addBasicColor(this.getObjText(category, "elementNull", 1));
			this.pushData(text, item);
		}
		...
		for (var j = 1; j <= length; ++j) {
			...
			if (rate > 0 && this.getObjTextShow(category, "elementForcedRate1")) {
				...
				text =
					this.getObjText(category, "elementForcedRate1").format($dataSystem.elements[j]) +
					this.textColon() +
					valueText;
				this.pushData(text, item);
				...
				text =
					this.getObjText(category, "elementForcedRate2").format($dataSystem.elements[j]) +
					this.textColon() +
					valueText;
				this.pushData(text, item);
			}
	...
	Window_ObjectInfo.prototype.getSkillItemCritical = function () {
		...
		var item = this._item;
		var baseItem = item;
		if (item.baseItemId) baseItem = DataManager.getBaseItem(item);
		...
		if (eval(GT.Param.YEPCriCtrlText.ShowcritRate) && item.damage.type > 0 && item.damage.critical) {
			if (item.critRate.match(/rate[ ]=[ ](\d+).(\d+)/i)) {
				...
				eval(item.critRate);
				text =
					this.addSystemColor(this.getObjText("OIWXParamText", "CRI")) +
					this.textColon() +
					this.addBasicColor(Math.floor(rate * 100) + "%");
				this._data.push(text);
			}
		}
		if (this.getObjTextShow(category, "critMult") && item.damage.type > 0 && item.damage.critical) {
			if (item.critMult.match(/(?:value)[ ]\*\=[ ](\d+).(\d+)[ ]\+[ ]bonus/i)) {
				...
				text =
					this.getObjText(category, "critMult") + this.textColon() + this.addBasicColor(Math.floor(rate * 100) + "%");
				this._data.push(text);
			}
		}
		if (this.getObjTextShow(category, "flatCrit") && item.damage.type > 0 && item.damage.critical) {
			if (baseItem.note.match(/<(?:FLAT CRITICAL):[ ](\d+)([%％])[ ](.*)>/i)) {
				...
				text =
					this.getObjText(category, "flatCrit") +
					this.textColon() +
					this.addBasicColor(Math.floor(rate * 100) + "%" + " " + stat);
				this._data.push(text);
			}
		}
	};
	...
	Window_ObjectInfo.prototype.getEquipCritical = function (item) {
		...
		if (this.getObjTextShow(category, "flatCrit")) {
			...
			if (flat) {
				var valueText =
					flat > 0 ? this.addIncreaseColor("+" + Math.floor(flat)) : this.addDecreaseColor(Math.floor(flat));
	...
	Window_ObjectInfo.prototype.getSkillItemArmorScale = function () {
		...
		if (
			item.positiveArmorScale === "" &&
			item.negativeArmorScale === "" &&
			this.getObjTextShow(category, "armorPenetration")
		) {
	...
	Window_ObjectInfo.prototype.getSkillItemBarrier = function () {
			...
			if (barriers && this.getObjTextShow(category, code)) {
				for (var i = 0; i < barriers.length; ++i) {
					...
					if (value !== 0) {
						var valueText =
							value > 0
								? this.addIncreaseColor("+" + value + " " + (turn ? turn + turnText : ""))
								: this.addDecreaseColor(value + " " + (turn ? turn + turnText : ""));
		...
		if (this.getObjTextShow(category, "BarrierPenetration")) {
			...
			if (rate) {
				text =
					this.getObjText(category, "BarrierPenetration").format(barrierText) +
					this.textColon() +
					this.addBasicColor(Math.floor(rate * 100) + "%");
				this._data.push(text);
			}
			...
			if (flat) {
				text =
					this.getObjText(category, "BarrierPenetration").format(barrierText) +
					this.textColon() +
					this.addBasicColor(Math.floor(flat));
				this._data.push(text);
			}
		}
	};
	...
	Window_ObjectInfo.prototype.getEquipClassRequirements = function () {
		...
		for (var i = 0; i < classes.length; ++i) {
			...
			if (GT.Param.OIWShowIcon && Imported.YEP_ClassChangeCore) {
				...
				if (iconIndex !== 0) name = "\\I[" + iconIndex + "]" + name;
			}
	...
	Window_ObjectInfo.prototype.getEquipParamRequirements = function () {
		for (var i = 0; i < 8; ++i) {
			if (
				this._item.equipRequirements["atLeast"][i] > 0 &&
				this.getObjTextShow("YEPObjRequireText", "MinParamRequire")
			) {
			...
			if (
				this._item.equipRequirements["atMost"][i] > 0 &&
				this.getObjTextShow("YEPObjRequireText", "MaxParamRequire")
			) {
	...
	Window_ObjectInfo.prototype.getEquipSwitchRequirements = function () {
		...
		for (var i = 0; i < length; ++i) {
			var sw = switches[i];
			var text =
				this.getObjText("YEPObjRequireText", "SwitchRequire") +
				this.textColon() +
				this.addBasicColor($dataSystem.switches[sw]);
			text = text.replace(/<<(.*?)>>/i, "");
			this._requireData.push(text);
		}
	};
	...
	Window_ObjectInfo.prototype.getObjLimitedUses = function (skill) {
		...
		var text =
			this.getObjText("YEPSkillCostText", "LimitedUses") +
			this.textColon() +
			"\\c[" +
			Yanfly.Param.LSUTextColor +
			"]" +
			fmt.format(current, max);
	...
	Window_ObjectInfo.prototype.getObjPartyLimitCost = function (skill) {
		...
		var text =
			this.getObjText("YEPSkillCostText", "PartyLimitCost") +
			this.textColon() +
			"\\c[" +
			Yanfly.Param.PLGCostColor +
			"]" +
			fmt.format(Yanfly.Util.toGroup(cost), unit.partyLimitGaugeCurrent(), unit.partyLimitGaugeIncrements());
	...
	Window_ObjectInfo.prototype.getObjWarmup = function (skill) {
		...
		var text =
			this.getObjText("YEPSkillCostText", "Warmup") +
			this.textColon() +
			"\\c[" +
			Yanfly.Param.WUTextColor +
			"]" +
			fmt.format(Yanfly.Util.toGroup(value));
	...
	Window_ObjectInfo.prototype.getObjCooldown = function (skill) {
		...
		if (value <= 0.009) value = 0.01;
		var text =
			this.getObjText("YEPSkillCostText", "Cooldown") +
			this.textColon() +
			"\\c[" +
			Yanfly.Param.CDTextColor +
			"]" +
			fmt.format(Yanfly.Util.toGroup(value));
	...
	Window_ObjectInfo.prototype.getObjSoloItemCost = function (item, cost) {
		...
		var text =
			this.getObjText("YEPSkillCostText", "ItemCost") + this.textColon() + this.addBasicColor(fmt.format(amt1, amt2));
	...
	Window_ObjectInfo.prototype.drawLevel = function () {
		...
		var y =
			...
			this.exDataHeight(this._requireData) +
			this.stateHeight();
		this.drawHorzLine(y);
		y += this.textPadding();
	...
	Window_ObjectInfo.prototype.pushData = function (text, item) {
		...
		else if (DataManager.isState(item)) return this._stateData[this._stateData.length - 1].push(text);
	};
