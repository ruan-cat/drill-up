...
//
//			索引：	Window
//			来源：	继承于 PIXI.Container
//			实例：	无（父类，一般被继承使用）
//			应用：	作用于该脚本文件的全部子类。其他插件也可继承此类或直接实例化。
//			
//			作用域：	地图界面、战斗界面、菜单界面
//			主功能：	> 描述一个窗口，并具备窗口的底层功能。
//			子功能：
//						->窗口
//							->帧刷新基函数
//							->执行渲染
...
//							> opacity 布局透明度
//						->C文本域
//							> contents 文本域的Bitmap对象
//							> contentsOpacity 文本域透明度
//							> padding 内边距（单位像素）
//						->D开关动画
//							> openness 展开度
//							->是否已打开（isOpen）
...
//						->F背景贴图
//							> margin 外边距（单位像素）
//							> backOpacity 背景透明度（不建议用）
//						->G选项边框
//							->设置指针矩形（setCursorRect）
//						->H上下箭头
//						->I小箭头
//						->J继承PIXI
//			
//			说明：	> 此底层基于 PIXI.Container，并且提供了 帧刷新基函数，不能按照 贴图的格式 来使用。
//					> 注意透明度的区分： opacity、backOpacity、contentsOpacity
//=============================================================================
//==============================
// * 窗口底层 - 定义
//==============================
...
Window.prototype.initialize = function(){
    PIXI.Container.call(this);
    this._isWindow = true;					//窗口标记
    this._animationCount = 0;				//计时器（动画效果用）
    this.active = true;						//激活状态（此变量父类无定义，给 Window_Base 子类用 激活/取消激活）
    this.pause = false;						//暂停状态（此变量父类无定义，Window_Message子类专用）
	
	
	// > 私有属性初始化
    this._windowskin = null;				//E窗口皮肤 - Bitmap对象
	
    this.origin = new Point();				//A窗口矩形 - 当前位置（来自PIXI）
    this._width = 0;						//A窗口矩形 - 宽度
    this._height = 0;						//A窗口矩形 - 高度

    this._windowSpriteContainer = null;		//B背景容器层 - 贴图容器（注意，是PIXI.Container，不支持anchor）
	
    this._padding = 18;						//C文本域 - 内边距
    this._windowContentsSprite = null;		//C文本域 - 贴图
	
    this._openness = 255;					//D开关动画 - 展开度
	
    this._margin = 4;						//F背景贴图 - 外边距
    this._colorTone = [0, 0, 0];			//F背景贴图 - 画面色调
	this._windowBackSprite = null;			//F背景贴图 - 平铺贴图
    this._windowFrameSprite = null;			//F背景贴图 - 框架贴图
	
    this._cursorRect = new Rectangle();		//G选项边框 - 指针矩形
    this._windowCursorSprite = null;		//G选项边框 - 贴图

    this.downArrowVisible = false;			//H上下箭头 - 箭头可见 下
    this.upArrowVisible = false;			//H上下箭头 - 箭头可见 上
    this._windowArrowSprites = [];			//H上下箭头 - 箭头容器（未使用）
	
    this._windowPauseSignSprite = null;		//I小箭头 - 贴图
...
// * 窗口底层 - 帧刷新基函数
//
//			说明：	> 此函数父类无定义。这里是作为 向子贴图 传递帧刷新 的基函数。
//==============================
...
Window.prototype.updateTransform = function(){
    this._updateCursor();			//帧刷新 - G选项边框
    this._updateArrows();			//帧刷新 - H上下箭头
    this._updatePauseSign();		//帧刷新 - I小箭头
    this._updateContents();			//帧刷新 - C文本域
    PIXI.Container.prototype.updateTransform.call(this);
};
...
// * 窗口底层 - 创建部件
//
//			说明：	> 注意贴图添加顺序，先加的贴图在最底部，后加的贴图在上面。
//==============================
Window.prototype._createAllParts = function(){
	
	this._windowSpriteContainer = new PIXI.Container();		//创建贴图 - B背景容器层（注意，是PIXI.Container，不支持anchor）
	this._windowBackSprite = new Sprite();					//创建贴图 - F背景贴图 - 平铺贴图
	this._windowFrameSprite = new Sprite();					//创建贴图 - F背景贴图 - 框架贴图
	this._windowCursorSprite = new Sprite();				//创建贴图 - G选项边框 - 贴图
	this._windowContentsSprite = new Sprite();				//创建贴图 - C文本域 - 贴图
	this._downArrowSprite = new Sprite();					//创建贴图 - H上下箭头 - 下箭头贴图
	this._upArrowSprite = new Sprite();						//创建贴图 - H上下箭头 - 上箭头贴图
	this._windowPauseSignSprite = new Sprite();				//创建贴图 - I小箭头 - 贴图
	...
	// > B背景容器层 初始化
	this.addChild(this._windowSpriteContainer);				//添加到 窗口容器层 - B背景容器层
	this._windowSpriteContainer.addChild(this._windowBackSprite);		//添加到 背景容器层 - F背景贴图 - 平铺贴图
	this._windowSpriteContainer.addChild(this._windowFrameSprite);		//添加到 背景容器层 - F背景贴图 - 框架贴图
	
	this.addChild(this._windowCursorSprite);				//添加到 窗口容器层 - G选项边框 - 贴图
	this.addChild(this._windowContentsSprite);				//添加到 窗口容器层 - C文本域 - 贴图
	this.addChild(this._downArrowSprite);					//添加到 窗口容器层 - H上下箭头 - 下箭头贴图
	this.addChild(this._upArrowSprite);						//添加到 窗口容器层 - H上下箭头 - 上箭头贴图
	this.addChild(this._windowPauseSignSprite);				//添加到 窗口容器层 - I小箭头 - 贴图
};
//==============================
// * 窗口底层 - 全部部件刷新（非帧刷新）
//
...
Window.prototype._refreshAllParts = function(){
    this._refreshBack();			//部件刷新 - F背景贴图 - 平铺贴图
    this._refreshFrame();			//部件刷新 - F背景贴图 - 框架贴图
    this._refreshCursor();			//部件刷新 - G选项边框
    this._refreshContents();		//部件刷新 - C文本域
    this._refreshArrows();			//部件刷新 - H上下箭头
    this._refreshPauseSign();		//部件刷新 - I小箭头
};
...
//==============================
// * B背景容器层 - 添加贴图到 背景容器层 后面（开放函数）
//
//			说明：	> 由于 背景层容器 不支持嵌套贴图，所以提供此方法，让 新贴图 可以放在背景容器前面。
//					> 此位置在 B背景容器层 的上面，G选项边框 的下面。
//==============================
...
//==============================
// * C文本域 - 参数 - 文本域的Bitmap对象
//
//			说明：	> 此处获取的是 Bitmap 对象，而非 文本域贴图。
//==============================
...
Window.prototype._updateContents = function(){
    var w = this._width - this._padding * 2;	//（注意，这里是计算控制 贴图的矩形范围，而不是bitmap的。）
...
// * D开关动画 - 参数 - 展开度
//
//			说明：	> 注意，此处没有open和close函数，只有控制窗口展开的动画过程的参数，给子类用。
//==============================
...
// * F背景贴图 - 参数 - 背景透明度（不建议用）
//
//			说明：	> 此处的透明度修改会与 布局透明度 效果叠加。
//					> 此处参数不建议用，目前没有drill插件使用此参数。
//					  建议插件中直接通过 this._windowBackSprite.opacity 进行控制。
//==============================
...
//==============================
// * J继承PIXI - 参数 - visible（可见）
//==============================
// * J继承PIXI - 参数 - x（位置X）
//==============================
// * J继承PIXI - 参数 - y（位置Y）
//==============================
// * J继承PIXI - 参数 - children（子贴图列表）
//==============================
// * J继承PIXI - 参数 - parent（父贴图）
//==============================
//==============================
// * J继承PIXI - 函数 - addChild
//
