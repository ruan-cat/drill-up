...
export class FileProcessor {
  ...
  async processAllFiles(): Promise<void> {
    ...
    const fileInfos = jsFiles.map(filePath => 
      createFileInfo(filePath, this.config.sourceDir, this.config.outputDir)
    );
  ...
  private printSummary(fileInfos: FileInfo[]): void {
    ...
    const groupedByDir = fileInfos.reduce((acc, fileInfo) => {
