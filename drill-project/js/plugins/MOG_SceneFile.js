...
　　var Moghunter = Moghunter || {}; 
  　Moghunter.parameters = PluginManager.parameters('MOG_SceneFile');
	...
	Moghunter.scFile_Act_width = Number(Moghunter.parameters['角色窗口宽度'] || 550);	
...
Window_FileData.prototype.refresh = function() {
	 ...
	 if (this.data()) {this.drawData() 
		...
		this.drawText(Moghunter.scFile_NoData, (this.width / 2) - 80, (this.height / 2) - 50, 320, 'left'); 
	 };
};
...
Window_FileData.prototype.drawData = function() {
	  ...
	  this.drawText(String(Moghunter.scFile_PlayTimeWord), Moghunter.scFile_PlayTimeX, Moghunter.scFile_PlayTimeY, 320, 'left');
	  this.drawText(String(Moghunter.scFile_LocationWord), Moghunter.scFile_LocationX, Moghunter.scFile_LocationY , 320, 'left');
	  this.drawText(TextManager.levelA, Moghunter.scFile_levelX, Moghunter.scFile_levelY, 320, 'left'); // Level
	  if (this._gold) {this.drawText(TextManager.currencyUnit, Moghunter.scFile_goldX ,Moghunter.scFile_goldY , 320, 'left')}; // Gold
	  
	  if(Moghunter.scFile_Text_visible){
		  if(Moghunter.scFile_Par_hpmp_visible){
			this.drawText(TextManager.param(0), x, y + s * 0, 320, 'left'); // HP
			this.drawText(TextManager.param(1), x, y + s * 1, 320, 'left'); // MP
		  }
		  this.drawText(TextManager.param(2), x, y + s * 2, 320, 'left'); // Atk
		  this.drawText(TextManager.param(3), x, y + s * 3, 320, 'left'); // Def
		  this.drawText(TextManager.param(4), x, y + s * 4, 320, 'left'); // Mat
		  this.drawText(TextManager.param(5), x, y + s * 5, 320, 'left'); // Mdf
		  this.drawText(TextManager.param(6), x, y + s * 6, 320, 'left'); // Agi
		  this.drawText(TextManager.param(7), x, y + s * 7, 320, 'left'); // Luk
	  }
	  
	  this.resetTextColor();	  
	  this.drawText(this.data().playtime, Moghunter.scFile_PlayTimeX + 120, Moghunter.scFile_PlayTimeY, 320, 'left');
	  this.drawText(this.data().location, Moghunter.scFile_LocationX + 120, Moghunter.scFile_LocationY, 320, 'left');
	  this.drawText(actor[1], Moghunter.scFile_levelX + 40 , Moghunter.scFile_levelY, 320, 'left'); // Level	  
	  if (this._gold) {this.drawText(this.data().gold, Moghunter.scFile_goldX ,Moghunter.scFile_goldY , 320, 'left')}; // Gold	
	  
	  this.drawText(actor[0], Moghunter.scFile_nameX, Moghunter.scFile_nameY, 320, 'left'); // Name	  
	  if(Moghunter.scFile_Par_hpmp_visible){
		this.drawText(actor[2], x + h, y + s * 0, 320, 'left'); // HP	  
		this.drawText(actor[3], x + h, y + s * 1, 320, 'left'); // MP	  
	  }
	  this.drawText(actor[4], x + h, y + s * 2, 320, 'left'); // Atk	 	  
	  this.drawText(actor[5], x + h, y + s * 3, 320, 'left'); // Def	  
	  this.drawText(actor[6], x + h, y + s * 4, 320, 'left'); // Mat	  
	  this.drawText(actor[7], x + h, y + s * 5, 320, 'left'); // Mdf		  
	  this.drawText(actor[8], x + h, y + s * 6, 320, 'left'); // Agi	  
	  this.drawText(actor[9], x + h, y + s * 7, 320, 'left'); // Luk	  
};
